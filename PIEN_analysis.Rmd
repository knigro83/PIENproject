...
library()
rasterDownload (region='NA',res='1000m',period='Normal_1961_1990',varList="NFFD",sDir='C:/Users/KatherineNigro/Box/01. katherine.nigro Workspace/PIENproject/PIENproject/ClimateNA')
...
gcms <- c("8GCMs_ensemble_ssp126_2011-2040", "8GCMs_ensemble_ssp126_2041-2070", "8GCMs_ensemble_ssp126_2071-2100", "8GCMs_ensemble_ssp245_2011-2040", "8GCMs_ensemble_ssp245_2041-2070", "8GCMs_ensemble_ssp245_2071-2100", "8GCMs_ensemble_ssp370_2011-2040", "8GCMs_ensemble_ssp370_2041-2070", "8GCMs_ensemble_ssp370_2071-2100", "8GCMs_ensemble_ssp585_2011-2040", "8GCMs_ensemble_ssp585_2041-2070", "8GCMs_ensemble_ssp585_2071-2100")
# 
# for(i in 1:length(gcms)){
# rasterDownload(region='NA',res='4000m',period=gcms[i],varList=c("cmd","pas"),sDir='C:/Users/KatherineNigro/Box/01. katherine.nigro Workspace/ASCC/ASCC/climateNA')
# }

futures<- matrix(nrow = 12, ncol=15)
for(i in 1:length(mastervarList[-c(16,17)])){
futures[,i] <- paste(gcms,"/",mastervarList[-c(16,17)][i],".tif",sep="")
}

futures_list <- c(futures)

future_clim_rasters <- c()
for(i in 1:length(futures_list)){
  future_clim_rasters[i]<- paste("C:/Users/KatherineNigro/Box/01. katherine.nigro Workspace/ASCC/ASCC/climateNA/NA/4000m/",futures_list[i],sep="")
}

future_clim_raster_stack <- rast(future_clim_rasters)

names(future_clim_raster_stack)<- paste(names(future_clim_raster_stack),substr(futures_list, 16,26),sep="_") %>% 
  str_replace(pattern="dd_0", replacement="dd0")
...
prov_futures_long<- prov_futures %>%
  ...
  pivot_longer(cols=mat_ssp126_2011:pas_ssp585_2071) %>% 
  rowwise() %>% 
  mutate(climvar = str_extract(name, "[^_]+"), carbon = str_split(name, fixed("_"))[[1]][2], period = str_split(name, fixed("_"))[[1]][3]) %>% 
  mutate(period=ifelse(is.na(period), "1970's", 
                       ifelse(period=="2011","2020's",
                              ifelse(period=="2041","2050's",
                                     ifelse(period=="2071", "2080's",period)))), carbon=ifelse(is.na(carbon),"historic",carbon))
...
tmean = 3.4
10.7945*tmean + 88.6801

(140-98.141)/12.173


##tmean needs to be at least 3.849 for an equivalent NFFD = 145
...
ggplot(engelmann_clim, aes(x=MAT, y=map))+
  ...
  scale_fill_gradientn(colors=c("gray","black"))+
  geom_point(data=plantation_only, aes(x=MAT, y=map), size=4, col="gold", shape=18)+
  geom_point(data=plantation_future, aes(x=mat/10, y=map, col=carbon, shape=period), size=4)+
