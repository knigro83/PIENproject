...
##predictions - no gila - longitude
CCnewdata1_cone_nog_long <- expand.grid(height=mean(moniger_subset_nog$height),
                          x=seq(min(moniger_subset_nog$x),max(moniger_subset_nog$x),1))


cone_clim_mod_nog_glm <- glmmTMB(cbind(n_cones, total-n_cones) ~ scale(height) + scale(x), data= moniger_subset_nog, family = binomial)

CCnewdata2_cone_nog_long <- cbind(CCnewdata1_cone_nog_long, predict(cone_clim_mod_nog_glm, newdata = CCnewdata1_cone_nog_long, type = "link", se=TRUE))
CCnewdata3_cone_nog_long <- within(CCnewdata2_cone_nog_long, {
    PredictedProb <- plogis(fit)
    LL <- plogis(fit - (1.96 * se.fit))
    UL <- plogis(fit + (1.96 * se.fit))
})

cone_plot_long<- ggplot(CCnewdata3_cone_nog_long %>% mutate(var="Longitude"), aes(x = x, y = PredictedProb)) + 
  geom_ribbon(aes(ymin = LL,
    ymax = UL), alpha = 0.2, col=NA) + geom_line(linewidth = 1)+
  geom_line(aes(x=x, y=PredictedProb))+
  geom_rug(data=moniger_subset_nog%>% mutate(PredictedProb=.8), aes(x=x, y=PredictedProb), sides="b", linewidth=1, col="black", alpha=0.5)+
  theme_bw()+
  ylim(c(0,1))+
  xlab("Longitude")+
  ylab("Predicted Probability of Cones")+
...
##predictions - no gila - heightitude
CCnewdata1_cone_nog_height <- expand.grid(x=mean(moniger_subset_nog$x),
                          height=seq(min(moniger_subset_nog$height),max(moniger_subset_nog$height),0.1))

CCnewdata2_cone_nog_height <- cbind(CCnewdata1_cone_nog_height, predict(cone_clim_mod_nog_glm, newdata = CCnewdata1_cone_nog_height, type = "link", se=TRUE))
...
ggarrange(cone_plot_long, cone_plot_height)
...
cone_plot<- ggarrange(ggarrange(cone_plot_long, cone_plot_height, nrow=1), cone_smrpb_plot, nrow=2)
...
lat_plot <- ggplot() + 
  geom_ribbon(data=lat_plot_data, aes(x=y, ymin = lwr, ymax = upr), alpha=0.5) +
...
ht_dbh_surv_plot<- ggarrange(ggarrange(dbh_y_plot,ht_y_plot,surv_y_plot, nrow=1),surv_ind_plot,nrow=2)
ht_dbh_surv_plot
