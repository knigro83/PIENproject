...
cone_clim_mod_hty <- glmer(Cones ~ scale(height)*scale(PC1) + I(scale(PC1)^2) + scale(height)*scale(PC2) + I(scale(PC2)^2) + scale(height)*scale(y) + I(scale(y)^2)+ (1|code) + (1|year), data=moniger_subset, family="binomial")
summary(cone_clim_mod_hty)

cone_clim_mod_dbhx <- glmer(Cones ~ scale(DBH)*scale(PC1) + I(scale(PC1)^2) + scale(DBH)*scale(PC2) + I(scale(PC2)^2) + scale(DBH)*scale(x) + I(scale(x)^2)+ (1|code) + (1|year), data=moniger_subset, family="binomial")
summary(cone_clim_mod_dbhx)

cone_clim_mod_dbhy <- glmer(Cones ~ scale(DBH)*scale(PC1) + I(scale(PC1)^2) + scale(DBH)*scale(PC2) + I(scale(PC2)^2) + scale(DBH)*scale(y) + I(scale(y)^2)+ (1|code) + (1|year), data=moniger_subset, family="binomial")
...
best.mod.cone.clim <- glmer(Cones ~ scale(height)*scale(x) + scale(PC1) + I(scale(PC1)^2) + (1|code) + (1|year), data= moniger_subset, family = "binomial")
summary(best.mod.cone.clim)

cone_clim_mod_glm <- glm(Cones ~  scale(height)*scale(x) + scale(PC1) + I(scale(PC1)^2), data=moniger_subset, family="binomial")
summary(cone_clim_mod_glm)

cone_clim_mod_glm <- glm(Cones ~ scale(height)*scale(PC1) + I(scale(PC1)^2) + scale(height)*scale(PC2) + I(scale(PC2)^2) + scale(height)*scale(x) + I(scale(x)^2), data=moniger_subset, family="binomial")


##predictions
CCnewdata1 <- with(moniger_subset, data.frame(height = seq(min(height),max(height),length.out=50), 
                                              PC1=mean(PC1), 
                                              PC2 = mean(PC2), 
                                              x = rep(seq(-121, -105, 2),each=50)))

CCnewdata2 <- cbind(CCnewdata1, predict(object=cone_clim_mod_glm, newdata = CCnewdata1, type = "link", se=TRUE, predict.all=TRUE))
