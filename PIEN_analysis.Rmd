...
dbh_growth_data<- dbh %>% 
  mutate(growth96=DBH96-DBH91, growth06=DBH06-DBH96, growth14=DBH14-DBH06) %>% 
  pivot_longer(growth96:growth14)
...
dbh_increment_plot<- ggplot(dbh_growth_data, aes(x=factor(Provenance, provs_bylat$Provenance), y=value, fill=factor(Provenance, provs_bylat$Provenance)))+
  geom_boxplot()+
  facet_wrap(~factor(name, levels=c("growth96","growth06","growth14")), scales="free")+
  ylab("DBH Increment")+
  xlab("")+
  scale_fill_manual(values=unname(stepped()), name="Provenance")+
  theme(axis.text.x=element_text(angle=45, hjust=1))

dbh_growth_rel_data<- dbh %>% 
  mutate(growth96=DBH96-DBH91, growth06=DBH06-DBH96, growth14=DBH14-DBH06) %>% 
  mutate(growth96rel=growth96/DBH91*100, growth06rel=growth06/DBH96*100, growth14rel=growth14/DBH06*100) %>% 
  pivot_longer(growth96rel:growth14rel) %>% 
  filter(!is.na(value)) %>% 
  filter(!is.infinite(value))

dbh_increment_rel_plot<- ggplot(dbh_growth_rel_data, aes(x=factor(Provenance, provs_bylat$Provenance), y=value, fill=factor(Provenance, provs_bylat$Provenance)))+
  geom_boxplot()+
  facet_wrap(~factor(name, levels=c("growth96rel","growth06rel","growth14rel")), scales="free")+
  ylab("Relativized DBH Increment")+
  xlab("")+
  scale_fill_manual(values=unname(stepped()), name="Provenance")+
  theme(axis.text.x=element_text(angle=45, hjust=1))

dbh_increment_rel_plot
dbh_increment_plot

###analyze relativized height growth data
aov_rDBH <- lmer(value ~ name*Provenance + (1|BLK), data=dbh_growth_rel_data)
anova(aov_rDBH)

dbh_growth_rel_clim<- dbh_growth_rel_data %>% 
  left_join(prov_pcapts_big)

options(na.action="na.fail")
#y
mod_reldbh_pca_y_randtime <- lmer(value ~ PC1*PC2 + PC1*scale(y) + PC2*scale(y) + I(PC1^2) + I(PC2^2) + I(scale(y)^2)+ (1|Provenance) + (1|BLK) + (1|name), data=dbh_growth_rel_clim)
...
bestmod_reldbh_pca_y_randtime <- mod_reldbh_pca_y_randtime
summary(bestmod_reldbh_pca_y_randtime)

#no gila
dbh_growth_rel_clim_noG<- dbh_growth_rel_clim %>% filter(!Provenance=="Gila NF")

mod_reldbh_pca_y_randtime_noG <- lmer(value ~ PC1*PC2 + PC1*scale(y) + PC2*scale(y) + I(PC1^2) + I(PC2^2) + I(scale(y)^2)+ (1|Provenance) + (1|BLK) + (1|name), data=dbh_growth_rel_clim_noG)
...
bestmod_reldbh_pca_y_randtime_noG <- mod_reldbh_pca_y_randtime_noG
summary(bestmod_reldbh_pca_y_randtime_noG)

#x
mod_reldbh_pca_x_randtime <- lmer(value ~ PC1*PC2 + PC1*scale(x) + PC2*scale(x) + I(PC1^2) + I(PC2^2) + I(scale(x)^2)+ (1|Provenance) + (1|BLK) + (1|name), data=dbh_growth_rel_clim)
...
bestmod_reldbh_pca_x_randtime <- mod_reldbh_pca_x_randtime
summary(bestmod_reldbh_pca_x_randtime)

#no gila
mod_reldbh_pca_x_randtime_noG <- lmer(value ~ PC1*PC2 + PC1*scale(x) + PC2*scale(x) + I(PC1^2) + I(PC2^2) + I(scale(x)^2)+ (1|Provenance) + (1|BLK) + (1|name), data=dbh_growth_rel_clim_noG)
...
bestmod_reldbh_pca_x_randtime_noG <- mod_reldbh_pca_x_randtime_noG
...
##the best models were the full models but nothing was significant
```
...
dbh_growth_rel_clim_long <- dbh_growth_rel_clim %>% 
...
mod_nog <- lmer(relDBH ~ value + (1|Provenance)+ (1|BLK) + (1|year), data=dbh_growth_rel_clim_long_nog %>% filter(name==provenance_climvars[i]) %>% filter(!Provenance=="Gila NF"))
pvalue_nog<- as.data.frame(summary(mod_nog)$coefficients)[2,]$`Pr(>|t|)`
...
for(i in 1:length(provenance_climvars)){
mod <- lmer(relDBH ~ value + (1|Provenance)+ (1|BLK), data=dbh_growth_rel_clim_long %>% filter(name==provenance_climvars[i]) %>% filter(year=="growth14rel"))
pvalue<- as.data.frame(summary(mod)$coefficients)[2,]$`Pr(>|t|)`

mod_nog <- lmer(relDBH ~ value + (1|Provenance)+ (1|BLK), data=dbh_growth_rel_clim_long_nog %>% filter(name==provenance_climvars[i])  %>% filter(year=="growth14rel") %>% filter(!Provenance=="Gila NF"))
pvalue_nog<- as.data.frame(summary(mod_nog)$coefficients)[2,]$`Pr(>|t|)`
...
summary(lmer(relDBH ~ value + (1|Provenance)+ (1|BLK), data=dbh_growth_rel_clim_long %>% filter(name=="smrpb") %>% filter(year=="growth14rel")))
...
mod_nog <- lmer(relDBH ~ value + I(value^2) + (1|Provenance) + (1|BLK) + (1|year), data=dbh_growth_rel_clim_long_nog %>% filter(name==provenance_climvars[i]) %>% filter(!Provenance=="Gila NF"))
pvalues_nog<- as.data.frame(summary(mod_nog)$coefficients)[2:3,]$`Pr(>|t|)`
...
ht_increment_rel_plot<- ggplot(ht_growth_rel_data %>% filter(value<1000), aes(x=factor(Provenance,levels=provs_bylat$Provenance), y=value, fill=factor(Provenance,levels=provs_bylat$Provenance)))+
  geom_boxplot()+
...
ggplot(data=ht_growth_rel_clim_long %>% pivot_wider(), aes(x=MCMT, y=relht, col=year))+
  geom_point()+
  geom_smooth(method="lm")+
