...
SR_relhtMAT_quad_lm <- lm(log(value) ~ value + I(value^2), data=relht_SR_long %>% filter(name=="MAT"))
...
SR_MAT_plot<- ggplot()+
  ...
  xlab("MAT")+
  ylab("Relative Height Growth Rate (%/year)")+
  geom_rug(data=relht_SR_long %>% filter(name=="MAT")%>% mutate(fit=.5), aes(x=value,y=fit), linewidth=1, col="black", sides="b")+
  theme_bw()
SR_MAT_plot
