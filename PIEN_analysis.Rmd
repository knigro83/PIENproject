...
moniger_subset <- moniger_data_comb %>% 
  dplyr::select(c(SORT, HT85, HT91, HT96, HT06, DBH91, DBH96, DBH06,Cones85, Cones91, Cones96, Cones06, PC1, PC2, x, y, code)) %>% 
  filter(!(is.na(Cones85) & is.na(Cones91) & is.na(Cones96) & is.na(Cones06))) %>% 
  pivot_longer(Cones85:Cones06) 
 
cone_clim_mod <- glmer(Cones ~ scale(height)*scale(PC1) + scale(height)*scale(PC2) + scale(height)*scale(x) + (1|code), data=moniger_subset, family="binomial")
summary(cone_clim_mod)
...
##predictions
CCnewdata1 <- with(moniger_subset, data.frame(height = seq(min(height),max(height),length.out=100), PC1=mean(PC1), PC2 = mean(PC2), x = seq(min(x), max(x), length.out = 100)))
...
ggplot(CCnewdata3, aes(x = height, y = PredictedProb)) + geom_ribbon(aes(ymin = LL,
    ymax = UL, fill = x>-113), alpha = 0.2) + geom_line(aes(colour = x>-113),
    linewidth = 1)+
  geom_point(aes(x=height, y=PredictedProb))
