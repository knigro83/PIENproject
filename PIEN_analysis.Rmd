...
dates <- c("01","02","03","04","05","06","07","08","09",seq(10,31,1))
months <- c("07","08")
...
raster<- rast(paste("C:/Users/KatherineNigro/Box/01. katherine.nigro Workspace/PIENproject/prismtmp/PRISM_tmin_stable_4kmD2_",years[k],months[j],dates[i],"_bil","/PRISM_tmin_stable_4kmD2_",years[k],months[j],dates[i],"_bil.bil",sep=""))
tmin <- terra::extract(raster, adjusted_provs %>% filter(Provenance=="Plantation"), method="simple")
tmindata <- bind_rows(tmindata,data.frame(year=years[k], month=months[j], day = dates[i], tmin = tmin[,2]))}}}
...
survmod_byyear<- glm(value ~ factor(year)*Provenance, data=moniger_survival_byyr, family="binomial")
plot(survmod_byyear)
