...
####logistic
```{r}
logmod_cat1 <- glmer(value ~ BLK, data=cone_data, family="binomial")


logmod_cat <- glmer(value ~ scale(year, scale=FALSE)*factor(Provenance) + (1|BLK), data= cone_data, family="binomial")
summary(logmod_cat, corr=FALSE)
Anova(logmod_cat, type="3")
confint(logmod_cat)
...
coef_df <- data.frame(prov= names(coef(logmod_cat)[1:21]), coef=coef(logmod_cat)[1:21])

ggplot(coef_df, aes(x=prov, y=coef))+
  geom_point()+
  theme(axis.text.x=element_text(angle=45, hjust=1))
...
left_join(moniger_data_comb %>% 
  ...
  dplyr::select(-name)) %>% 
left_join(moniger_data_comb %>% 
  dplyr::select(c(SORT, PC1, PC2, PC3, x, y, code))) %>% 
...
bestmod_cone_clim_hty <- glmer(Cones ~ scale(height)*scale(y) + PC2 + I(PC2^2) + (1|code) + (1|year), data=moniger_subset, family="binomial")
summary(bestmod_cone_clim_hty)

cone_lat_plot<- interact_plot(bestmod_cone_clim_hty, pred=y, modx=height, data=moniger_subset, plot.points=TRUE, interval = TRUE, int.width = 0.95, int.type="confidence", x.label=paste("Latitude (","\u00B0","N)",sep=""), y.label="Cone Probability", legend.main="Height", colors="Purples")
...
best.mod.cone.clim <- glmer(Cones ~ scale(height)*scale(x) + PC2 + I(PC2^2) + (1|code) + (1|year), data= moniger_subset, family = "binomial")
summary(best.mod.cone.clim)

cone_long_plot<- interact_plot(best.mod.cone.clim, pred=x, modx=height, data=moniger_subset, plot.points=TRUE, interval = TRUE, int.width = 0.95, int.type="confidence", x.label=paste("Longitude (","\u00B0","W)",sep=""), y.label="Cone Probability", legend.main="Height", colors="Purples")
...
best.mod.cone.clim_nog <- glmer(Cones ~ scale(height)*scale(x) + (1|code) + (1|year), data= moniger_subset %>% filter(!code=="GiNF"), family = "binomial")
...
cone_clim_mod_glm <- glm(Cones ~ scale(height)*scale(x) + PC2 + I(PC2^2), data= moniger_subset, family = "binomial")
...
cone_dist_mod <- glmer(Cones ~ scale(dist.to.plant.3d)*scale(geo.dist.to.plant.km)*scale(height) + (1|code) + (1|year), data=cone_dist_data, family="binomial")
plot(cone_dist_mod)
summary(cone_dist_mod)

cone_dist_mod_nog <- glmer(Cones ~ scale(dist.to.plant.3d)*scale(geo.dist.to.plant.km)*scale(height) + (1|code) + (1|year), data=cone_dist_data_nog, family="binomial")
...
cone_int_plot<- interact_plot(cone_dist_bestmod, pred=geo.dist.to.plant.km, modx=height, plot.points=TRUE, data=cone_dist_data, interval=TRUE, int.type="confidence", int.width=0.95, colors="Purples", y.label="Probability of Cones", x.label="Geographic distance to plantaion (km)", legend.main="Height", legend.position="top")
...
#no g predictions
cone_dist_bestmod_nog <- glmer(Cones ~ scale(geo.dist.to.plant.km)*scale(height) + (1|code) + (1|year), data=cone_dist_data  %>% filter(!Provenance == "Gila NF"), family="binomial")
summary(cone_dist_bestmod_nog)

cone_dist_mod_glm_nog <- glm(Cones ~scale(geo.dist.to.plant.km)*scale(height) , data=cone_dist_data  %>% filter(!Provenance == "Gila NF"), family="binomial")
...
moniger_survival_byyr <- moniger_data_comb %>% 
  ...
  dplyr::select(c(SORT,Provenance,Stat74:Stat14)) %>% 
...
survmod_byyear<- glm(value ~ year*Provenance, data=moniger_survival_byyr, family="binomial")
plot(survmod_byyear)
summary(survmod_byyear)
confint(survmod_byyear)
wald.test(b = coef(survmod_byyear), Sigma = vcov(survmod_byyear), Terms=2) ##year is significant
wald.test(b = coef(survmod_byyear), Sigma = vcov(survmod_byyear), Terms=3:21) ##Provenance is significant
wald.test(b = coef(survmod_byyear), Sigma = vcov(survmod_byyear), Terms=22:40) ##interaction is significant
coef(survmod_byyear)
...
ggplot(newdata3, aes(x = factor(Provenance, levels=provs_bylat %>% arrange(PC1) %>% pull(Provenance)), y = PredictedProb)) + 
  geom_errorbar(aes(ymin = LL, ymax = UL, col = factor(Provenance, levels=provs_bylat %>% arrange(PC1) %>% pull(Provenance)))) + 
  geom_point(aes(colour = factor(Provenance, levels=provs_bylat %>% arrange(y) %>% pull(Provenance))))+
  theme(axis.text.x = element_text(angle=45, hjust=1))+
  scale_color_manual(values=unname(stepped()))+
  ylab("Predicted Probability of Survival")+
  theme_bw()+
...
##predictions by year
newdata1 <- with(moniger_survival_byyr, data.frame(year = seq(min(year),max(year),1), Provenance = rep(unique(moniger_survival_byyr$Provenance), each=41)))
...
survmod74 <- aov(value ~ Provenance, data=moniger_survival_byyr %>% filter(year==1974))
summary(survmod74)
TukeyHSD(survmod74)$Provenance %>% as.data.frame() %>% 
  filter(`p adj` < 0.05)

survmod79 <- aov(value ~ Provenance, data=moniger_survival_byyr %>% filter(year==1979))
summary(survmod79)
TukeyHSD(survmod79)$Provenance %>% as.data.frame() %>% 
  filter(`p adj` < 0.05)

survmod85 <- aov(value ~ Provenance, data=moniger_survival_byyr %>% filter(year==1985))
summary(survmod85)

survmod91 <- aov(value ~ Provenance, data=moniger_survival_byyr %>% filter(year==1991))
summary(survmod91)
TukeyHSD(survmod91)$Provenance %>% as.data.frame() %>% 
  filter(`p adj` < 0.05)

survmod96 <- aov(value ~ Provenance, data=moniger_survival_byyr %>% filter(year==1996))
summary(survmod96)

survmod06 <- aov(value ~ Provenance, data=moniger_survival_byyr %>% filter(year==2006))
summary(survmod06)

survmod14 <- aov(value ~ Provenance, data=moniger_survival_byyr %>% filter(year==2014))
summary(survmod14)
...
```{r}
##
survmod_pcx<- glmer(Stat14 ~  PC1*PC2 + scale(x)*PC1 + scale(x)*PC2 + I(PC1^2) + I(PC2^2) + scale(x) + I(scale(x)^2) + (1|Provenance), data=moniger_survival, family="binomial") 
plot(survmod_pcx)
summary(survmod_pcx)
...
dredge_survmod_pcy
bestsurvmod_pcy <- glmer(Stat14 ~ PC1*scale(y) + I(PC1^2) + PC2 + I(PC2^2)+ I(scale(y)^2) + (1|Provenance), data=moniger_survival, family="binomial")
...
moniger_survival_nog <- moniger_survival %>% filter(!Provenance%in%c("Gila NF","Okanagan"))

survmod_pcy_nog<- glmer(Stat14 ~ PC1*PC2 + scale(y)*PC1 + scale(y)*PC2 + I(PC1^2) + I(PC2^2) + scale(y) + I(scale(y)^2) + (1|Provenance), data=moniger_survival_nog, family="binomial")
...
bestsurvmod_pcy_nog <- glmer(Stat14 ~ PC1*scale(y) + I(PC1^2) + PC2 + I(PC2^2)+ I(scale(y)^2) + (1|Provenance), data=moniger_survival_nog, family="binomial")
summary(bestsurvmod_pcy_nog)

interact_plot(bestsurvmod_pcy_nog, pred=y, modx=PC1, plot.points = TRUE, interval = TRUE, int.width = 0.95, data=moniger_survival_nog)

bestsurvmod_pcy_nog_glm <- glm(Stat14 ~ PC1*scale(y) + I(PC1^2) + PC2 + I(PC2^2)+ I(scale(y)^2), data=moniger_survival_nog, family="binomial")

bestsurvmod_pcy_glm <- glm(Stat14 ~ PC1*scale(y) + I(PC1^2) + PC2 + I(PC2^2)+ I(scale(y)^2), data=moniger_survival, family="binomial")
summary(bestsurvmod_pcy_glm)
#predict
newdata1 <- with(moniger_survival_nog, data.frame(PC1 = rep(seq(min(PC1),max(PC1),2), each=50),
                                              PC2 = mean(PC2),
...
ggplot() + 
  ...
  geom_line(data=newdata3_nog, aes(x = y, y = PredictedProb, col=factor(PC1)))+
  theme(axis.text.x = element_text(angle=45, hjust=1))
...
ggplot() + 
  #geom_ribbon(aes(ymin = LL, ymax = UL), alpha=0.5) +
  geom_line(data=newdata3, aes(x = y, y = PredictedProb, col=factor(PC1)))+
  theme(axis.text.x = element_text(angle=45, hjust=1))

###varying PC2
newdata1_pc2 <- data.frame(PC1 = rep(mean(moniger_survival$PC1),222), y = c(rep(35,74), rep(42,74), rep(50,74)), PC2 = rep(seq(min(moniger_survival$PC2), max(moniger_survival$PC2), by=0.1),3))

newdata2_pc2 <- cbind(newdata1_pc2, predict(bestsurvmod_pcy_glm, newdata = newdata1_pc2, type = "link", se=TRUE))
...
surv_pc2_plot_alt <- ggplot(newdata3_pc2,aes(x = PC2, y = PredictedProb, col=factor(y), fill=factor(y))) + 
...
survmod_dist<- glmer(Stat14 ~ scale(dist.to.plant.3d)*scale(geo.dist.to.plant.km) + (1|Provenance), data=surv_pcadist_data, family = "binomial")
plot(survmod_dist)
summary(survmod_dist) 
dredge(survmod_dist)

bestmod_survmod_dist <- glmer(Stat14 ~ scale(dist.to.plant.3d)*scale(geo.dist.to.plant.km) + (1|Provenance), data=surv_pcadist_data, family = "binomial")
...
cone_int_plot+theme(legend.position="top")

survmod_dist_glm<- glm(Stat14 ~ scale(dist.to.plant.3d)*scale(geo.dist.to.plant.km), data=surv_pcadist_data, family = "binomial")

###make plot for distance model
SDnewdata1 <- with(surv_pcadist_data, data.frame(dist.to.plant.3d = seq(min(surv_pcadist_data$dist.to.plant.3d), 
                      max(surv_pcadist_data$dist.to.plant.3d), 
                      length.out = 50),
...
survmod_dist_nog<- glmer(Stat14 ~ scale(dist.to.plant.3d)*scale(geo.dist.to.plant.km) + (1|Provenance), data=surv_pcadist_data_nog, family = "binomial")
plot(survmod_dist_nog)
summary(survmod_dist_nog)
dredge(survmod_dist_nog)

best_survmod_dist_nog<- glmer(Stat14 ~ scale(dist.to.plant.3d)*scale(geo.dist.to.plant.km) + (1|Provenance), data=surv_pcadist_data_nog, family = "binomial")
summary(best_survmod_dist_nog)

survmod_dist_glm_nog<- glm(Stat14 ~ scale(dist.to.plant.3d)*scale(geo.dist.to.plant.km), data=surv_pcadist_data_nog, family = "binomial")

###make plot for distance model
SDnewdata_nog1 <- with(surv_pcadist_data_nog, data.frame(dist.to.plant.3d = seq(min(dist.to.plant.3d), 
                      max(dist.to.plant.3d), 
                      length.out = 50),
    geo.dist.to.plant.km = rep(seq(80,1600,190),each=50)))
...
#smrpb
surv_smrpb_mod <- glmer(Stat14 ~ scale(climvar.value) + (1|Provenance), data=moniger_survival_long %>% filter(climvar.name=="smrpb"), family="binomial")
...
##take random effect out to make predictions
surv_smrpb_mod_glm <- glm(Stat14 ~ scale(climvar.value), data=moniger_survival_long %>% filter(climvar.name=="smrpb"), family="binomial")

##predictions
survsmrpb_newdata1 <- with(surv_smrpb_data, data.frame(climvar.value = seq(min(climvar.value),max(climvar.value),length.out=50)))
...
#without Gila is still significant
surv_smrpb_mod_nog <- glmer(Stat14 ~ scale(climvar.value) + (1|Provenance), data=moniger_survival_long %>% filter(climvar.name=="smrpb") %>% filter(!Provenance=="Gila NF"), family="binomial")
summary(surv_smrpb_mod_nog)

##take random effect out to make predictions
surv_smrpb_mod_nog_glm <- glm(Stat14 ~ scale(climvar.value), data=moniger_survival_long %>% filter(climvar.name=="smrpb")%>% filter(!Provenance=="Gila NF"), family="binomial")
summary(surv_smrpb_mod_nog_glm)

##predictions
surv_smrpb_data_nog <- moniger_survival_long %>% filter(climvar.name=="smrpb") %>% filter(!Provenance=="Gila NF")

survsmrpb_nog_newdata1 <- with(surv_smrpb_data_nog, data.frame(climvar.value = seq(min(climvar.value),max(climvar.value),length.out=50)))
...
##MCMT
surv_MCMT_mod <- glmer(Stat14 ~ scale(climvar.value) + I(scale(climvar.value)^2) + (1|Provenance), data=moniger_survival_long %>% filter(climvar.name=="MCMT"), family="binomial")
summary(surv_MCMT_mod)

surv_MCMT_data <- moniger_survival_long %>% filter(climvar.name=="MCMT")

##take random effect out to make predictions
surv_MCMT_mod_glm <- glm(Stat14 ~ scale(climvar.value) + I(scale(climvar.value)^2), data=moniger_survival_long %>% filter(climvar.name=="MCMT"), family="binomial")

##predictions
survMCMT_newdata1 <- with(surv_MCMT_data, data.frame(climvar.value = seq(min(climvar.value),max(climvar.value),length.out=50)))

survMCMT_newdata2 <- cbind(survMCMT_newdata1, predict(surv_MCMT_mod_glm, newdata = survMCMT_newdata1, type = "link", se=TRUE))
...
#without Gila is still significant
surv_MCMT_mod_nog <- glmer(Stat14 ~ scale(climvar.value) + I(scale(climvar.value)^2) + (1|Provenance), data=moniger_survival_long %>% filter(climvar.name=="MCMT") %>% filter(!Provenance=="Gila NF"), family="binomial")
...
##Eref
surv_Eref_mod <- glmer(Stat14 ~ scale(climvar.value) + I(scale(climvar.value)^2) + (1|Provenance), data=moniger_survival_long %>% filter(climvar.name=="Eref"), family="binomial")
summary(surv_Eref_mod)

surv_Eref_data <- moniger_survival_long %>% filter(climvar.name=="Eref")

##take random effect out to make predictions
surv_Eref_mod_glm <- glm(Stat14 ~ scale(climvar.value) + I(scale(climvar.value)^2), data=moniger_survival_long %>% filter(climvar.name=="Eref"), family="binomial")

##predictions
survEref_newdata1 <- with(surv_Eref_data, data.frame(climvar.value = seq(min(climvar.value),max(climvar.value),length.out=50)))
...
#without Gila is still significant
surv_Eref_mod_nog <- glmer(Stat14 ~ scale(climvar.value) + I(scale(climvar.value)^2) + (1|Provenance), data=moniger_survival_long %>% filter(climvar.name=="Eref") %>% filter(!Provenance=="Gila NF"), family="binomial")
