...
ht_growth_data<- moniger_data_comb %>% 
  ...
  mutate(growth79rel=growth79/HT74*100, growth85rel=growth85/HT79*100, growth91rel=growth91/HT85*100, growth96rel=growth96/HT91, growth06rel=growth06/HT96, growth14rel=growth14/HT06) %>% 
...
ht_growth_rel_data<- moniger_data_comb %>% 
  dplyr::select(c(LOC,BLK,REP,Provenance,HT74:HT14,Seedling.Ht..from.Nursery)) %>% 
  mutate(growth79=HT79-HT74, growth85 = HT85-HT79, growth91 = HT91-HT85, growth96=HT96-HT91, growth06=HT06-HT96, growth14=HT14-HT06) %>% 
  mutate(growth79rel=growth79/HT74*100, growth85rel=growth85/HT79*100, growth91rel=growth91/HT85*100, growth96rel=growth96/HT91*100, growth06rel=growth06/HT96*100, growth14rel=growth14/HT06*100) %>% 
...
CLD_HGmod06 = cld(HGmod06_marg,
                  alpha=0.05,
                  Letters=letters)
CLD_HGmod06 ##Gila no longer in last place by 2006
...
ggplot(data=gila_survivors, aes(x=year, y=value, color=uid))+
  ...
  ylab("Relativized Growth Increment")
...
bestmod_relht_pca_y_randtime <- lmer(value ~  PC1*scale(y)+ I(PC1^2) + I(PC2^2) + I(scale(y)^2)+ (1|Provenance) + (1|BLK) + (1|name), data=ht_growth_rel_clim)
...
bestmod_relht_pca_y_randtime_noG <- lmer(value ~  PC2 + scale(y) + I(scale(y)^2)+ (1|Provenance) + (1|BLK) + (1|name), data=ht_growth_rel_clim_noG)
...
bestmod_relht_pca_x_randtime <- mod_relht_pca_x_randtime
...
bestmod_relht_pca_x_randtime_noG <- lmer(value ~  PC1 + scale(x) +  I(scale(x)^2)+ (1|Provenance) + (1|BLK) + (1|name), data=ht_growth_rel_clim_noG)
...
####make plot for best final model
bestmod_relht_pca_x_randtime_noG_lm <- lm(value ~  PC1 + scale(x) +  I(scale(x)^2), data=ht_growth_rel_clim_noG)

newdata_relht_x <-  expand.grid(x=seq(min(ht_growth_rel_clim_noG$x), max(ht_growth_rel_clim_noG$x), length.out=50),
                                PC1=mean(ht_growth_rel_clim_noG$PC1))
...
relht_x_plot <- ggplot()+
  ...
  geom_rug(data=ht_growth_rel_clim_noG %>% mutate(fit=70), aes(x=x,y=fit), linewidth=1, col="black", sides="b")+
  xlab("Provenance Longitude")+
  ylab("Relativized Height Growth (%)")+
  facet_grid(~var)+
  theme_bw()+
  theme(legend.position="none")
relht_x_plot
