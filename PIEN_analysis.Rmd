...
ggplot(prov_vars, aes(x=factor(Provenance,levels=prov_pcapts %>% arrange(y) %>% pull(Provenance)), y=value, color=factor(Provenance,levels=prov_pcapts %>% arrange(y) %>% pull(Provenance))))+
  geom_point(size=4)+
...
#cmd
mod_dbh_cmd <- lmer(DBH ~ cmd + (1|Provenance), data=dbh_num14%>% filter(!Provenance=="Gila NF"))
summary(mod_dbh_cmd) ##when Gila is taken out, CMD is not significant
...
obj<- detrend.series(y = prov_all_nona %>% pull(colnames(prov_good_nona)[i]), y.name="growth", method="Spline", nyrs = 100)
