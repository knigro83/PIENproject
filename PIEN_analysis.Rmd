...
```{r}
#read in files
ppt_comb_plantation<- read.csv("ppt_comb_plantation.csv") %>% dplyr::select(-X) %>% rename(X = X.1)
...
ggplot(clim_pien, aes(x=MAT, y=MAP))+
  ...
  scale_fill_gradientn(colors=c("gray","black"))+
  geom_point(data=plantation_only, aes(x=MAT, y=MAP), size=4, col="gold", shape=18)+
  geom_point(data=plantation_future, aes(x=MAT, y=MAP, col=carbon, shape=period), size=4)+
  scale_color_manual(values=carbon.colors[-1])+
  geom_point(data=prov_pcapts %>% filter(!Provenance=="Plantation"), aes(x=MAT, y=MAP), size=2)+
  ggrepel::geom_text_repel(data=prov_pcapts %>% filter(!Provenance=="Plantation"), aes(label=code))+
  theme_bw()+
  theme(text = element_text(size=16, color="black"))
```
...
ggplot()+
  geom_point(data=tree.num, aes(x=year, y=n, col=Provenance))+
  geom_line(data=tree.num, aes(x=year, y=n, col=Provenance))+
  geom_bar(data=plantation_clim, aes(x=year, y=scale(value_sm_tmean)*10), stat = "identity")
...
result <- seascorr(chrono = chron.list[[i]],
                         climate= list(aet= aet_date_wide[1:46,], cwd = cwd_date_wide[1:46,]),
...
for(i in 1:length(provs_arranged_lat[-9])){
result <- seascorr(chrono = chron.list[[i]],
                         climate= list(aet= aet_date_wide[1:46,], spei = spei_date_wide[1:46,]),
                         timespan = c(1985,2015),
                         complete= 9,
                         season_lengths=seq(4,12,1),
                         primary=1,
...
for(i in 1:length(provs_arranged_lat[-9])){
result <- seascorr(chrono = chron.list[[i]],
                         climate= list(temp= tmean_comb_plantation_wide[1:46,], precip = ppt_comb_plantation_wide[1:46,]),
                         timespan = c(1985,2015),
                         complete= 9,
                         season_lengths=seq(4,12,1),
                         primary=1,
...
for(i in 1:length(provs_arranged_lat[-9])){
result <- seascorr(chrono = chron.list[[i]],
                         climate= list(precip = ppt_comb_plantation_wide[1:46,],temp= tmean_comb_plantation_wide[1:46,]),
                         timespan = c(1985,2015),
                         complete= 9,
                         season_lengths=seq(4,12,1),
                         primary=1,
...
mod1 <- glmer(value ~ cwd_sum + (1|Provenance), data=survival_period_data, family="binomial") 
summary(mod1) ##can't really do this because survival will obviously go down with time, while temps get hotter with time
```
...
growth_climate_df <- growth.detrended.df %>% 
 ...
 dplyr::select(c(CN:year,PPTsm:PPTsp, vpdmax_sm:vpdmax_sp, FDSI, aet_sum, cwd_wy, spei_sum)) %>% 
  left_join(plantation_clim_minus1, by=c("year" = "prior_year", "CN" = "prior_CN")) %>% 
  ungroup() %>% 
  filter(year>1970) %>% 
...
sig.cors.byprov <- cor_clean %>% left_join(ttestdata %>% filter(pvalue < 0.05)) %>% 
  mutate(sig = case_when(!is.na(pvalue) ~ "*",
                         .default = "")) %>% 
  filter(climvar %in% c("aet_sum" , "cwd_wy" , "PPTsm", "vpdmax_wt", "prior_aet_sum", "prior_cwd_wy", "prior_PPTsm" , "prior_vpdmax_wt")) %>% 
  filter(!is.na(pvalue)) %>% 
  left_join(name_conversion, by=c("Provenance"="oldname"))
...
nonsig_data<- data.frame(climvar=rep(unique(sig.cors.byprov$climvar),1),newname=rep(nonsig.provs$newname, each=7), Provenance = rep(nonsig.provs$Provenance, each=7))

selected_climvars <- c("aet_sum","prior_aet_sum","cwd_wy" ,"prior_cwd_wy","PPTsm",  "prior_PPTsm","vpdmax_wt","prior_vpdmax_wt")
...
correlation_plot<- ggplot(data=sig.cors.byprov %>% bind_rows(nonsig_data) %>% left_join(prov_names, by=c("newname"="Provenance")), aes(x=factor(code, levels=provs_bylat$code), y=estimate))+
  ...
  facet_wrap(~factor(climvar, levels=selected_climvars),ncol=2)+
  xlab("")+
  theme_bw()+
...
summary(vpdmax_wt_smrpb_mod_nog) #not significant without Gila
...
FDSI_CMD_mod_nog <- lmer(cor ~ value + (1|Provenance), data=FDSI_CMD %>% filter(!Provenance=="Gila NF"))
plot(FDSI_CMD_mod_nog)
qqmath(FDSI_CMD_mod_nog)
summary(FDSI_CMD_mod_nog) #significant

#cwd_wy x PPTwt
ggplot(data=cor_provclim_long %>% filter(climvar=="cwd_wy"& name=="PPTwt"), aes(x=value, y=cor))+
  geom_point()+
  geom_smooth(method="lm")+
  xlab("PPTwt")+
  ggtitle("cwd_wy")

cwd_wy_PPTwt<- cor_provclim_long %>% filter(climvar=="cwd_wy" & name=="PPTwt") 

cwd_wy_PPTwt_mod <- lmer(cor ~ value + (1|Provenance), data=cwd_wy_PPTwt)
plot(cwd_wy_PPTwt_mod)
qqmath(cwd_wy_PPTwt_mod)
summary(cwd_wy_PPTwt_mod)

cwd_wy_PPTwt_mod_nog <- lmer(cor ~ value + (1|Provenance), data=cwd_wy_PPTwt %>% filter(!Provenance=="Gila NF"))
plot(cwd_wy_PPTwt_mod_nog)
qqmath(cwd_wy_PPTwt_mod_nog)
summary(cwd_wy_PPTwt_mod_nog) #still significant
```

```{r}

annual_climvar <- lmerdata %>% filter(pvalue.lmer < 0.05) %>% filter(!climvar %in% c("prior_value_wt_tmax","prior_value_wt_tmin","prior_value_sp_tmax","prior_value_sp_tmean")) %>% pull(climvar)
prov_climvar <- lmerdata %>% filter(pvalue.lmer < 0.05) %>% filter(!climvar %in% c("prior_value_wt_tmax","prior_value_wt_tmin","prior_value_sp_tmax","prior_value_sp_tmean")) %>% pull(name)
...
ggplot()+
  ...
  xlab("Winter Precipitation (mm)")+ ylab("Correlation")+
...
ggplot()+
  ...
  geom_smooth(data=left_join(correlation_strength_results %>% 
  filter(pvalue_nog<0.05), cor_provclim_long, by=c("annual_climvar"="climvar", "prov_climvar"="name")) %>% filter(annual_climvar %in% c("prior_aet_sum") & prov_climvar=="EMT")  , aes(x=value, y=cor), method="lm", color="darkgrey")+
  xlab("Provenance Extreme minimum temperature (C)")+ ylab("Correlation")+
  ggtitle("C) prior year AET X RWI")+
  theme_bw(),
