...
ht.order <- ht_data %>% 
  filter(name=="HT14") %>% 
  group_by(Provenance) %>% 
  summarise(med_rel_growth=median(rel.growth,na.rm=T), med_growth=median(growth.m, na.rm=T)) %>% 
  arrange(med_growth)
...
ht_data_aves <- ht_data %>% filter(!name=="HT74") %>% 
  group_by(name, Provenance,Seedling.Ht..from.Nursery) %>% 
  summarise(meangrowth = mean(growth.m), seRG = sd(growth.m)/sqrt(n()))
...
ggplot(data=ht_data_aves, aes(x=Seedling.Ht..from.Nursery, y=meangrowth))+
  geom_point()


##total height follows different pattern than "relative height growth"
ggplot(ht_data %>% filter(!name=="HT74"), aes(x=factor(Provenance, levels=ht.order$Provenance), y=value, fill=factor(Provenance, levels=ht.order$Provenance)))+
  geom_boxplot()+
  facet_wrap(~factor(name, levels=c("HT79","HT85","HT91","HT96","HT06","HT14")), scales="free")+
  scale_fill_discrete(name="Provenance")+
  scale_x_discrete(name="Provenance")+
...
ggplot(ht_data %>% filter(!name=="HT74"), aes(x=factor(Provenance, levels=ht.order$Provenance), y=nursery_ht_m, fill=factor(Provenance, levels=ht.order$Provenance)))+
  geom_boxplot()+
  scale_fill_discrete(name="Provenance")+
  scale_x_discrete(name="Provenance")+
  theme(axis.text.x = element_blank())
...
ht_mod <- lm(sqrt(rel.growth) ~ Provenance*name, data=ht_data %>% mutate(rel.growth=ifelse(rel.growth<0,0,rel.growth)))
par(mfrow=c(2,2))
plot(ht_mod)
anova(ht_mod) ##there is a significant year by provenance interaction
...
#this is the opposite effect as for DBH (middle latitudes are tallest and skinniest)
ggplot(ht_data_clim %>% filter(name=="HT14"), aes(x=y, y=rel.growth, col=Provenance))+
  geom_point()
```

```{r}
#provenance climate affect on height linear models
...
#TD
mod_totht_TD <- lmer(value ~ TD + (1|Provenance), data=ht_data_clim14)
plot(mod_totht_TD)
qqmath(mod_totht_TD)
summary(mod_totht_TD) 
summary(lmer(value ~ TD + (1|Provenance), data=ht_data_clim14_nogila)) 

ggplot(data=ht_data_clim14, aes(x=TD, y=value, col=Provenance))+
  geom_point()+
  geom_smooth(method="lm")
