...
plot(dbh_aov91)
summary(dbh_aov91)
pairwise_dbh91 <- TukeyHSD(dbh_aov91)$Provenance %>% 
  as.data.frame() %>% 
  filter(`p adj` < 0.05) %>% 
  mutate(comp=rownames(TukeyHSD(dbh_aov91)$Provenance %>% 
  as.data.frame() %>% 
  filter(`p adj` < 0.05))) %>% 
...
plot(dbh_aov96)
summary(dbh_aov96)
pairwise_dbh96 <- TukeyHSD(dbh_aov96)$Provenance %>% 
  as.data.frame() %>% 
  filter(`p adj` < 0.05) %>% 
  mutate(comp=rownames(TukeyHSD(dbh_aov96)$Provenance %>% 
  as.data.frame() %>% 
  filter(`p adj` < 0.05))) %>% 
  mutate(payette = grepl( "Payette", comp, fixed = TRUE),
         gila = grepl("Gila", comp, fixed = TRUE)) 
...
plot(dbh_aov06)
summary(dbh_aov06)
pairwise_dbh06 <- TukeyHSD(dbh_aov06)$Provenance %>% 
  as.data.frame() %>% 
  filter(`p adj` < 0.05) %>% 
  mutate(comp=rownames(TukeyHSD(dbh_aov06)$Provenance %>% 
  as.data.frame() %>% 
  filter(`p adj` < 0.05))) %>% 
  mutate(payette = grepl( "Payette", comp, fixed = TRUE),
         gila = grepl("Gila", comp, fixed = TRUE)) 
...
dbh_aov14 <- aov(DBH ~ Provenance, data =dbh_numeric14 )
plot(dbh_aov14)
summary(dbh_aov14)
...
dbh14_glht <- glht(dbh_aov14, linfct = mcp(Provenance = "Tukey"))
dbh14_cld<- cld(dbh14_glht)
dbh14_letter_df<- as.data.frame(dbh14_cld$mcletters$Letters) %>% 
  rename(letters = `dbh14_cld$mcletters$Letters`) %>% 
  mutate(Provenance = rownames(as.data.frame(dbh14_cld$mcletters$Letters)))
...
dbh_means_fig <- ggplot(dbh_aves, aes(x=factor(year,levels=c(1974,1979,1985,1991,1996,2006,2014), labels=c(1974,1979,1985,1991,1996,2006,2014)), y=meanDBH, col=factor(Provenance, levels=provs_bylat$Provenance),group=factor(Provenance, levels=provs_bylat$Provenance)))+
  ...
  scale_color_manual(values=unname(stepped()), "Provenance")+
  theme_bw()+
  xlab("Year")+
  ylab("Mean DBH (cm)")
...
mod_dbh_dist <- lmer(DBH ~ scale(geo.dist.to.plant.km)*scale(dist.to.plant.3d) + (1|Provenance), data=dbh_num14_alldist)
plot(mod_dbh_dist)
qqmath(mod_dbh_dist)
...
mod_dbh_dist_nog <- lmer(DBH ~ scale(geo.dist.to.plant.km)*scale(dist.to.plant.3d) + (1|Provenance), data=dbh_num14_alldist_noG)
plot(mod_dbh_dist_nog)
qqmath(mod_dbh_dist_nog)
summary(mod_dbh_dist_nog)
dredge(mod_dbh_dist_nog)

mod_dbh_dist2 <- lmer(DBH ~ scale(geo.dist.to.plant.km) + scale(dist.to.plant.3d) + (1|Provenance), data=dbh_num14_alldist)
plot(mod_dbh_dist2)
...
mod_dbh_dist2_noG <- lmer(DBH ~ scale(geo.dist.to.plant.km) + scale(dist.to.plant.3d) + (1|Provenance), data=dbh_num14_alldist_noG)
...
bestmod_dbh_pca_x <- lmer(DBH ~ scale(x) + (1|Provenance), data=dbh_num14_alldist)
...
bestmod_dbh_pca_y <- lmer(DBH ~ PC2*scale(y) + I(PC2^2) + I(scale(y)^2) + (1|Provenance), data=dbh_num14_alldist)
...
mod_dbh_pca_y_noO <- lmer(DBH ~ PC1*PC2 + PC1*scale(y) + PC2*scale(y) + I(PC1^2) + I(PC2^2) + I(scale(y)^2) + (1|Provenance), data=dbh_num14_alldist_noO)
plot(mod_dbh_pca_y_noO)
qqmath(mod_dbh_pca_y_noO)
summary(mod_dbh_pca_y_noO)
dredge(mod_dbh_pca_y_noO)

bestmod_dbh_pca_y_noO <- lmer(DBH ~ PC2 + I(PC2^2) + scale(y) + I(scale(y)^2) + (1|Provenance), data=dbh_num14_alldist_noO)
summary(bestmod_dbh_pca_y_noO)

dbh_num14_alldist_noGO <- dbh_num14_alldist %>% 
  filter(!Provenance%in% c("Okanogan NF", "Gila NF"))

mod_dbh_pca_y_noGO <- lmer(DBH ~ PC1*PC2 + PC1*scale(y) + PC2*scale(y) + I(PC1^2) + I(PC2^2) + I(scale(y)^2) + (1|Provenance), data=dbh_num14_alldist_noGO)
plot(mod_dbh_pca_y_noGO)
qqmath(mod_dbh_pca_y_noGO)
summary(mod_dbh_pca_y_noGO)
dredge(mod_dbh_pca_y_noGO)

bestmod_dbh_pca_y_noGO <- lmer(DBH ~ PC2  + scale(y) + I(scale(y)^2) + (1|Provenance), data=dbh_num14_alldist_noGO)
...
##PCs and x 
mod_dbh_pca_x_time <- lmer(DBH ~ PC1*PC2 + PC1*scale(x) + PC2*scale(x) + I(PC1^2) + I(PC2^2) + I(scale(x)^2) + scale(year) + scale(year)*PC1 + scale(year)*PC2 + scale(year)*scale(x) + scale(year)*I(PC1^2) + scale(year)*I(PC2^2) + scale(year)*I(scale(x)^2) + (1|Provenance), data=dbh_numeric_time)
...
bestmod_dbh_pca_x_time <- lmer(DBH ~ year + scale(x) + PC1 + I(PC1^2) + (1|Provenance), data=dbh_numeric_time)
summary(bestmod_dbh_pca_x_time)

##PCs and y 
mod_dbh_pca_y_time <- lmer(DBH ~ PC1*PC2 + PC1*scale(y) + PC2*scale(y) + I(PC1^2) + I(PC2^2) + I(scale(y)^2) + scale(year) + scale(year)*PC1 + scale(year)*PC2 + scale(year)*scale(y) + scale(year)*I(PC1^2) + scale(year)*I(PC2^2) + scale(year)*I(scale(y)^2) + (1|Provenance), data=dbh_numeric_time)
...
```{r}
##all 3 PCs
mod_dbh_pcs <- lmer(DBH ~ PC1*PC2 + PC1*PC3 + PC2*PC3 + I(PC1^2) + I(PC2^2) + I(PC3^2) + (1|Provenance), data=dbh_num14_alldist)
plot(mod_dbh_pcs)
qqmath(mod_dbh_pcs)
summary(mod_dbh_pcs)
...
mod_dbh_pcs_nog <- lmer(DBH ~ PC1*PC2 + PC1*PC3 + PC2*PC3 + I(PC1^2) + I(PC2^2) + I(PC3^2) + (1|Provenance), data=dbh_num14_alldist_noG)
...
for(i in 1:length(provenance_climvars)){
mod <- lmer(DBH ~ value + (1|Provenance), data=dbh_num91_long %>% filter(name==provenance_climvars[i]))
pvalue<- as.data.frame(summary(mod)$coefficients)[2,]$`Pr(>|t|)`

mod_nog <- lmer(DBH ~ value + (1|Provenance), data=dbh_num91_long %>% filter(name==provenance_climvars[i]) %>% filter(!Provenance=="Gila NF"))
pvalue_nog<- as.data.frame(summary(mod_nog)$coefficients)[2,]$`Pr(>|t|)`
...
summary(lmer(value ~ nursery_ht_m + (1|Provenance), data=ht_data %>% filter(name=="HT74") %>% filter(!Provenance=="Gila NF")))
...
summary(lmer(value ~ nursery_ht_m + (1|Provenance), data=ht_data %>% filter(name=="HT79") %>% filter(!Provenance=="Gila NF")))
...
summary(lmer(value ~ nursery_ht_m + (1|Provenance), data=ht_data %>% filter(name=="HT85") %>% filter(!Provenance=="Gila NF")))
...
summary(lmer(value ~ nursery_ht_m + (1|Provenance), data=ht_data %>% filter(name=="HT91") %>% filter(!Provenance=="Gila NF")))
...
summary(lmer(value ~ nursery_ht_m + (1|Provenance), data=ht_data %>% filter(name=="HT96") %>% filter(!Provenance=="Gila NF")))

#2006
nurse_ht_mod06 <- lmer(value ~ nursery_ht_m + (1|Provenance), data=ht_data %>% filter(name=="HT06") %>% filter(!Provenance=="Gila NF"))
plot(nurse_ht_mod06)
qqmath(nurse_ht_mod06)
summary(nurse_ht_mod06)  ##no effect of nursery height on total height in 2006 
summary(lmer(value ~ nursery_ht_m + (1|Provenance), data=ht_data %>% filter(name=="HT06") %>% filter(!Provenance=="Gila NF"))) #significant without Gila
...
summary(lmer(value ~ nursery_ht_m + (1|Provenance), data=ht_data %>% filter(name=="HT14") %>% filter(!Provenance=="Gila NF")))
...
totht_mod14 <- aov(value ~ Provenance, data=ht_data14)
plot(totht_mod14)
summary(totht_mod14)
...
totht14_glht <- glht(totht_mod14, linfct = mcp(Provenance="Tukey"))
totht14_cld<- cld(totht14_glht)
totht14_letter_df<- as.data.frame(totht14_cld$mcletters$Letters) %>% 
  rename(letters = `totht14_cld$mcletters$Letters`) %>% 
  mutate(Provenance = rownames(as.data.frame(totht14_cld$mcletters$Letters)))
...
## Distance model
ht_dist_mod <- lmer(value ~ scale(geo.dist.to.plant.km)*scale(dist.to.plant.3d) + (1|Provenance), data=ht_data_clim14)
plot(ht_dist_mod)
qqmath(ht_dist_mod)
summary(ht_dist_mod)
dredge(ht_dist_mod)

best_ht_dist_mod <- lmer(value ~ scale(geo.dist.to.plant.km)*scale(dist.to.plant.3d) + (1|Provenance), data=ht_data_clim14)
...
ht_dist_mod_nog <- lmer(value ~ scale(geo.dist.to.plant.km)*scale(dist.to.plant.3d) + (1|Provenance), data=ht_data_clim14_nog)
plot(ht_dist_mod_nog)
qqmath(ht_dist_mod_nog)
summary(ht_dist_mod_nog)
dredge(ht_dist_mod_nog)

best_ht_dist_mod_nog <- lmer(value ~ scale(geo.dist.to.plant.km) + scale(dist.to.plant.3d) + (1|Provenance), data=ht_data_clim14_nog)
...
bestmod_ht_pca_x <- lmer(value ~ PC1 + PC2 + I(PC2^2) + scale(x) + (1|Provenance), data=ht_data_clim14)
...
bestmod_ht_pca_y <- lmer(value ~ PC1 + PC2 + I(PC2^2) + scale(y) + (1|Provenance), data=ht_data_clim14)
...
```{r}
##all 3 PCs
mod_ht_pcs <- lmer(value ~ PC1*PC2 + PC1*PC3 + PC2*PC3 + I(PC1^2) + I(PC2^2) + I(PC3^2) + (1|Provenance), data=ht_data_clim14)
plot(mod_ht_pcs)
qqmath(mod_ht_pcs)
summary(mod_ht_pcs)
dredge(mod_ht_pcs) 
...
bestmod_ht_pcs <- lmer(value ~ PC3 + I(PC3^2) + (1|Provenance), data=ht_data_clim14)
plot(bestmod_ht_pcs)
qqmath(bestmod_ht_pcs)
summary(bestmod_ht_pcs)

mod_ht_pcs_nog <- lmer(value ~ PC1*PC2 + PC1*PC3 + PC2*PC3 + I(PC1^2) + I(PC2^2) + I(PC3^2) + (1|Provenance), data=ht_data_clim14_nog)
plot(mod_ht_pcs_nog)
...
for(i in 1:length(provenance_climvars)){
mod <- lmer(value ~ climvar.value + (1|Provenance), data=ht_data_clim91_long %>% filter(climvar.name==provenance_climvars[i]))
pvalue<- as.data.frame(summary(mod)$coefficients)[2,]$`Pr(>|t|)`

mod_nog <- lmer(value ~ climvar.value + (1|Provenance), data=ht_data_clim91_long %>% filter(climvar.name==provenance_climvars[i]) %>% filter(!Provenance=="Gila NF"))
pvalue_nog<- as.data.frame(summary(mod_nog)$coefficients)[2,]$`Pr(>|t|)`
...
ht_SR_mods %>% arrange(pvalue) ##TD & RH are significant
...
##RH
ht_RH_mod <- lmer(value ~ climvar.value + (1|Provenance), data=ht_SR_long %>% filter(climvar.name=="RH"))
plot(ht_RH_mod)
qqmath(ht_RH_mod)
summary(ht_RH_mod)
...
##PPTwt
pptwt_dbh_quad <- lmer(DBH ~ climvar.value + I(climvar.value^2) + (1|Provenance), data=dbh_SR_long %>% filter(climvar.name=="PPTwt"))
plot(pptwt_dbh_quad)
...
ggarrange(dbh_means_fig, ht_means_fig, survival_means_fig, ncol=1, nrow=3, common.legend = TRUE)
