...
cone_dist_mod_nog <- glmer(Cones ~ scale(dist.to.plant)*scale(geo.dist.to.plant.km)*scale(height) + (1|code) + (1|year), data=cone_dist_data %>% filter(!Provenance == "Gila NF"), family="binomial")
...
png("figures/cone_plot.png", width=9.5, height=6.5, units = "in", res=500)
ggarrange(ggarrange(cone_x_plot, cone_dist_plot, ncol=2),cone_sig_clim_plot, nrow=2, heights=c(1,0.8))
dev.off()
...
png("figures/survival_plot.png", width=7, height=6, units = "in", res=500)
ggarrange(ggarrange(surv_byprov, surv_dist_plot, ncol=2),surv_sig_clim_plot,nrow=2)
dev.off()
```
...
##PCA models
resis_4mod_pcx <- lmer(log(resistance) ~ PC1*PC2 + I(PC1^2) + I(PC2^2) + scale(x) + I(scale(x)^2) + (1|Provenance), data=RR4_clim)
plot(resis_4mod_pcx)
qqmath(resis_4mod_pcx)
summary(resis_4mod_pcx) 

resis_4mod_pcx_noG <- lmer(log(resistance) ~ PC1*PC2 + I(PC1^2) + I(PC2^2) + scale(x) + I(scale(x)^2)+ (1|Provenance), data=RR4_clim %>% filter(!Provenance=="Gila NF"))
plot(resis_4mod_pcx_noG)
qqmath(resis_4mod_pcx_noG)
summary(resis_4mod_pcx_noG) #nothing significant 

dredge(resis_4mod_pcx) ##null model is the best

resis_4mod_pcy <- lmer(log(resistance) ~ PC1*PC2 + I(PC1^2) + I(PC2^2) + scale(y) + I(scale(y)^2) + (1|Provenance), data=RR4_clim)
plot(resis_4mod_pcy)
qqmath(resis_4mod_pcy)
summary(resis_4mod_pcy) 

resis_4mod_pcy_noG <- lmer(log(resistance) ~ PC1*PC2 + I(PC1^2) + I(PC2^2) + scale(y) + I(scale(y)^2)+ (1|Provenance), data=RR4_clim %>% filter(!Provenance=="Gila NF"))
plot(resis_4mod_pcy_noG)
qqmath(resis_4mod_pcy_noG)
summary(resis_4mod_pcy_noG) 

dredge(resis_4mod_pcy) ##null model is the best 

#distance model
resis_4mod_dist <- lmer(log(resistance) ~ scale(dist.to.plant)*scale(geo.dist.to.plant.km) + (1|Provenance), data=RR4_clim)
plot(resis_4mod_dist)
qqmath(resis_4mod_dist)
summary(resis_4mod_dist) 

resis_4mod_dist_noG <- lmer(log(resistance) ~ scale(dist.to.plant)*scale(geo.dist.to.plant.km) + (1|Provenance), data=RR4_clim %>% filter(!Provenance=="Gila NF"))
plot(resis_4mod_dist_noG)
qqmath(resis_4mod_dist_noG)
summary(resis_4mod_dist_noG) #nothing significant when gila removed

dredge(resis_4mod_dist) ##null model is best 
```
