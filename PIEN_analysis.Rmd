...
dplyr::select(c(CN:vpdmax_sp, FDSI, aet_sum, cwd_sum, cwd_wy, spei_sum)) %>% 
  ...
  mutate(type = ifelse(name %in% c("PPTsp","PPTsm","PPT_at","ppt_wt"), "PPT",
                       ifelse(name %in% c("value_sp_tmean","value_sm_tmean","value_fa_tmean","value_wt_tmean"),"TEMP",NA)),
         season = ifelse(name %in% c("PPTsp","value_sp_tmean"), "Spring", 
                         ifelse(name %in% c("PPTsm","value_sm_tmean"), "Summer",
                                ifelse(name %in% c("ppt_wt","value_wt_tmean"),"Winter",
                                                   ifelse(name %in% c("PPT_at","value_fa_tmean"),"Fall",NA)))))
...
# engelmann_points <- spatSample(pien_trim, size=100000000, method="regular", replace=FALSE, na.rm=TRUE, as.points=TRUE, values=TRUE, xy=TRUE)
# 
#engelmann_clim <- terra::extract(clim_raster_stack, engelmann_points %>% project(crs(clim_raster_stack)), method="simple", weights=TRUE)
# 
...
#   rast <- rast(raster)
#   ext.poly <- terra::extract(rast,engelmann_points %>% project(crs(clim_raster_stack)), method="simple", weights = TRUE, df=TRUE)
#   ppt<-ext.poly[,2]
...
ggplot(clim_pien, aes(x=MAT, y=MAP))+
  geom_hex(alpha=0.7)+
  scale_fill_gradientn(colors=c("black","grey"))+
...
pca2_clim_vars <- c("CMD","Eref","MCMT","MWMT","NFFD","PAS","PPTsm","PPTsp","PPTat","PPTwt","RH","TD","SMRPB_NA")
...
loadings_pc1pc2 <- as.data.frame(clim_pca_big$rotation)[1:3][c("PAS","PPTsp","NFFD"),]
loadings_pc1pc3 <- as.data.frame(clim_pca_big$rotation)[1:3][c("PAS","PPTsp","PPTsm","SMRPB_NA"),]
...
pc1_pc2_niche_plot <- ggplot(as.data.frame(clim_pca_big$x[-(1:5),]), aes(x=PC1, y=PC2))+
  geom_hex(alpha=0.7, na.rm=TRUE, bins=100)+
  ...
  ggnewscale::new_scale_fill()+
  scale_fill_manual(values=unname(stepped()), name="Provenance")+
  geom_point(data=plant_pca_scores, aes(x=PC1, y=PC2, shape=period), col="red4", size=2)+
...
png("figures/pc1_pc2_niche_plot.png", width=3, height=3, res=300, units = "in")
pc1_pc2_niche_plot
dev.off()
...
pc1_pc3_niche_plot <- ggplot(as.data.frame(clim_pca_big$x[-(1:5),]), aes(x=PC1, y=PC3))+
  ...
  ggnewscale::new_scale_fill()+
  scale_fill_manual(values=unname(stepped()), name="Provenance")+
  geom_point(data=plant_pca_scores, aes(x=PC1, y=PC3, shape=period), col="red4", size=2)+
  scale_shape_manual(values=c(19,15,8,3))+
  geom_point(data=prov_pcapts_big, aes(x=PC1, y=PC3, fill=factor(Provenance, levels=provs_bylat$Provenance)), size=1.5, shape=24)+
  ggrepel::geom_text_repel(data=prov_pcapts_big, aes(label=code), size=1.5)+
  xlim(-10,8)+
...
png("figures/map_and_bigpca.png", width=8, height=3.5, res=300, units = "in")
ggarrange(map,ggarrange(pc1_pc2_niche_plot, pc1_pc3_niche_plot, nrow=2, ncol=1, labels=c("B)","C)")),figpca_legend, labels=c("A)","",""), ncol=3, nrow=1, widths=c(1,1,.5))
dev.off()
...
bestmod_dbh_pca_x <- lmer(DBH ~ scale(x) + (1|Provenance) + (1|BLK), data=dbh_num14_alldist)
...
AICc(mod_dbh_pca_x, mod_dbh_pca_y) #lat is better

bestmod_dbh_pca_y <- lmer(DBH ~ PC2*scale(y) + I(PC2^2) + I(scale(y)^2) + (1|Provenance) + (1|BLK), data=dbh_num14_alldist)
...
dbh_pca_int_plot<- interact_plot(bestmod_dbh_pca_y, pred=y, modx=PC2, plot.points = TRUE, interval = TRUE, int.width = 0.95, data=dbh_num14_alldist)
...
interact_plot(mod_dbh_pca_y_nog, pred=y, modx=PC2, plot.points=TRUE, interval = TRUE, int.width = 0.95, data=dbh_num14_alldist_noG)

bestmod_dbh_pca_y_nog <- lmer(DBH ~ scale(y) + I(scale(y)^2) + (1|Provenance) + (1|BLK), data=dbh_num14_alldist_noG)
summary(bestmod_dbh_pca_y_nog) #quadratic y effect is significant when you take out Gila

dbh_num14_alldist_noO <- dbh_num14_alldist %>% 
  filter(!Provenance%in% c("Okanogan NF"))

mod_dbh_pca_y_noO <- lmer(DBH ~ PC1*PC2 + PC1*scale(y) + PC2*scale(y) + I(PC1^2) + I(PC2^2) + I(scale(y)^2) + (1|Provenance) + (1|BLK), data=dbh_num14_alldist_noO)
plot(mod_dbh_pca_y_noO)
qqmath(mod_dbh_pca_y_noO)
summary(mod_dbh_pca_y_noO)
dredge(mod_dbh_pca_y_noO)

bestmod_dbh_pca_y_noO <- lmer(DBH ~ PC2 + I(PC2^2) + scale(y) + I(scale(y)^2) + (1|Provenance) + (1|BLK), data=dbh_num14_alldist_noO)
summary(bestmod_dbh_pca_y_noO)

dbh_num14_alldist_noGO <- dbh_num14_alldist %>% 
  filter(!Provenance%in% c("Okanogan NF", "Gila NF"))

mod_dbh_pca_y_noGO <- lmer(DBH ~ PC1*PC2 + PC1*scale(y) + PC2*scale(y) + I(PC1^2) + I(PC2^2) + I(scale(y)^2) + (1|Provenance) + (1|BLK), data=dbh_num14_alldist_noGO)
plot(mod_dbh_pca_y_noGO)
qqmath(mod_dbh_pca_y_noGO)
summary(mod_dbh_pca_y_noGO)
dredge(mod_dbh_pca_y_noGO)

bestmod_dbh_pca_y_noGO <- lmer(DBH ~ PC2  + scale(y) + I(scale(y)^2) + (1|Provenance) + (1|BLK), data=dbh_num14_alldist_noGO)
summary(bestmod_dbh_pca_y_noGO)
...
#saveRDS(mod_dbh_pca_x_time_dredge,"mod_dbh_pca_x_time_dredge.RDS")
mod_dbh_pca_x_time_dredge <- readRDS("mod_dbh_pca_x_time_dredge.RDS")

bestmod_dbh_pca_x_time <- lmer(DBH ~ year + scale(x) + PC1 + I(PC1^2) + (1|Provenance) + (1|BLK), data=dbh_numeric_time)
...
#saveRDS(mod_dbh_pca_y_time_dredge,"mod_dbh_pca_y_time_dredge.RDS")
mod_dbh_pca_y_time_dredge <- readRDS("mod_dbh_pca_y_time_dredge.RDS")

bestmod_dbh_pca_y_time <- lmer(DBH ~ year + scale(y) + (1|Provenance) + (1|BLK), data=dbh_numeric_time)
summary(bestmod_dbh_pca_y_time) #year is only thing significant

AICc(mod_dbh_pca_x_time, mod_dbh_pca_y_time) #lat is better
...
bestmod_dbh_pca_y_randtime <- lmer(DBH ~ scale(y) + (1|Provenance) + (1|BLK) + (1|year), data=dbh_numeric_time)
...
qqmath(mod_dbh_pcs)
summary(mod_dbh_pcs)
dredge(mod_dbh_pcs) ##null model is the best model 
...
mod <- lmer(DBH ~ value + (1|Provenance)+ (1|BLK), data=dbh_num14_long %>% filter(name==provenance_climvars[i]))
pvalue<- as.data.frame(summary(mod)$coefficients)[2,]$`Pr(>|t|)`

mod_nog <- lmer(DBH ~ value + (1|Provenance)+ (1|BLK), data=dbh_num14_long %>% filter(name==provenance_climvars[i]) %>% filter(!Provenance=="Gila NF"))
...
for(i in 1:length(provenance_climvars)){
mod <- lmer(DBH ~ value + I(value^2) + (1|Provenance) + (1|BLK), data=dbh_num14_long %>% filter(name==provenance_climvars[i]))
pvalues<- as.data.frame(summary(mod)$coefficients)[2:3,]$`Pr(>|t|)`

mod_nog <- lmer(DBH ~ value + I(value^2) + (1|Provenance) + (1|BLK), data=dbh_num14_long %>% filter(name==provenance_climvars[i]) %>% filter(!Provenance=="Gila NF"))
...
```{r}
#look at DBH in other years

dbh_numeric91 <- dbh_numeric %>% filter(year=="DBH91") %>% 
  mutate(Provenance=as.factor(Provenance))

dbh_num91_long <- dbh_numeric91 %>% 
  pivot_longer(c(AHM:SHM, smrpb, monsoonality))

dbh_mods91<- data.frame()
dbh_num91_nog <- dbh_numeric91 %>% filter(!Provenance=="Gila NF")
for(i in 1:length(provenance_climvars)){
mod <- lmer(DBH ~ value + (1|Provenance) + (1|BLK), data=dbh_num91_long %>% filter(name==provenance_climvars[i]))
pvalue<- as.data.frame(summary(mod)$coefficients)[2,]$`Pr(>|t|)`

mod_nog <- lmer(DBH ~ value + (1|Provenance) + (1|BLK), data=dbh_num91_long %>% filter(name==provenance_climvars[i]) %>% filter(!Provenance=="Gila NF"))
pvalue_nog<- as.data.frame(summary(mod_nog)$coefficients)[2,]$`Pr(>|t|)`
...
for(i in 1:length(nursery_climvars)){
  mod <- lm(nursery_ht_m ~ value, data=nursery_ht_clim %>% filter(name == nursery_climvars[i]))
  pvalue <- as.data.frame(summary(mod)$coefficients)$`Pr(>|t|)`[2]
  nursery_lms <- bind_rows(nursery_lms, data.frame(nursery_climvars=nursery_climvars[i],pvalue))
}
nursery_lms %>% arrange(pvalue)
...
nurse_ht_mod <- lmer(log(value) ~ nursery_ht_m*factor(year) + (1|Provenance) + (1|BLK), data=ht_data_year)
plot(nurse_ht_mod)
qqmath(nurse_ht_mod)
...
##no Gila
nurse_ht_mod_nog <- lmer(log(value) ~ nursery_ht_m*factor(year) + (1|Provenance) + (1|BLK), data=ht_data_year %>% filter(!Provenance=="Gila NF"))
plot(nurse_ht_mod_nog)
qqmath(nurse_ht_mod_nog)
summary(nurse_ht_mod_nog)  
Anova(nurse_ht_mod_nog, type="3")
...
summary(lmer(value ~ nursery_ht_m + (1|Provenance)+ (1|BLK), data=ht_data %>% filter(name=="HT79") %>% filter(!Provenance=="Gila NF")))
...
summary(lmer(value ~ nursery_ht_m + (1|Provenance) + (1|BLK), data=ht_data %>% filter(name=="HT85") %>% filter(!Provenance=="Gila NF")))
...
summary(lmer(value ~ nursery_ht_m + (1|Provenance) + (1|BLK), data=ht_data %>% filter(name=="HT91") %>% filter(!Provenance=="Gila NF")))
...
summary(lmer(value ~ nursery_ht_m + (1|Provenance) + (1|BLK), data=ht_data %>% filter(name=="HT96") %>% filter(!Provenance=="Gila NF")))
...
##PCs and x 
mod_ht_pca_x <- lmer(value ~ PC1*PC2 + PC1*x + PC2*x + I(PC1^2) + I(PC2^2) + I(scale(x)^2) + (1|Provenance) + (1|BLK), data=ht_data_clim14)
plot(mod_ht_pca_x)
qqmath(mod_ht_pca_x)
...
bestmod_ht_pca_x <- lmer(value ~ PC1 + PC2 + I(PC2^2) + I(scale(x)^2) + (1|Provenance) + (1|BLK), data=ht_data_clim14)
summary(bestmod_ht_pca_x)

ggplot(data=ht_data_clim14, aes(x=x, y=value))+
  geom_point()

mod_ht_pca_x_nog <- lmer(value ~ PC1*PC2 + PC1*x + PC2*x + I(PC1^2) + I(PC2^2) + I(scale(x)^2) + (1|Provenance) + (1|BLK), data=ht_data_clim14_nog)
...
##PCs and y 
mod_ht_pca_y <- lmer(value ~ PC1*PC2 + PC1*y + PC2*y + I(PC1^2) + I(PC2^2) + I(scale(y)^2) + (1|Provenance) + (1|BLK), data=ht_data_clim14)
plot(mod_ht_pca_y)
qqmath(mod_ht_pca_y)
...
interact_plot(bestmod_ht_pca_y, data=ht_data_clim14, pred=y, modx=PC2, plot.points=TRUE, interval=TRUE, int.type="confidence")
...
mod_ht_pca_y_nog <- lmer(value ~ PC1*PC2 + PC1*y + PC2*y + I(PC1^2) + I(PC2^2) + I(scale(y)^2) + (1|Provenance) + (1|BLK), data=ht_data_clim14_nog)
plot(mod_ht_pca_y_nog)
qqmath(mod_ht_pca_y_nog)
summary(mod_ht_pca_y_nog)
dredge(mod_ht_pca_y_nog)

bestmod_ht_pca_y_nog <- lmer(value ~ y + I(scale(y)^2) + (1|Provenance) + (1|BLK), data=ht_data_clim14_nog)
...
```{r}
##all 3 PCs
mod_ht_pcs <- lmer(value ~ PC1*PC2 + PC1*PC3 + PC2*PC3 + I(PC1^2) + I(PC2^2) + I(PC3^2) + (1|Provenance) + (1|BLK), data=ht_data_clim14)
plot(mod_ht_pcs)
qqmath(mod_ht_pcs)
summary(mod_ht_pcs)
dredge(mod_ht_pcs) 

AICc(mod_ht_pca_x, mod_ht_pca_y, mod_ht_pcs) ##PC3 model has the lowest AICc

bestmod_ht_pcs <- lmer(value ~ PC3 + I(PC3^2) + (1|Provenance) + (1|BLK), data=ht_data_clim14)
plot(bestmod_ht_pcs)
qqmath(bestmod_ht_pcs)
summary(bestmod_ht_pcs)

mod_ht_pcs_nog <- lmer(value ~ PC1*PC2 + PC1*PC3 + PC2*PC3 + I(PC1^2) + I(PC2^2) + I(PC3^2) + (1|Provenance) + (1|BLK), data=ht_data_clim14_nog)
plot(mod_ht_pcs_nog)
qqmath(mod_ht_pcs_nog)
summary(mod_ht_pcs_nog)
dredge(mod_ht_pcs_nog) ##null model is best

AICc(mod_ht_pca_x_nog, mod_ht_pca_y_nog, mod_ht_pcs_nog) ##PC3 model still has the lowest AICc
...
for(i in 1:length(provenance_climvars)){
mod <- lmer(value ~ climvar.value + (1|Provenance)+(1|BLK), data=ht_data_clim14_long %>% filter(climvar.name==provenance_climvars[i]))
pvalue<- as.data.frame(summary(mod)$coefficients)[2,]$`Pr(>|t|)`

mod_nog <- lmer(value ~ climvar.value + (1|Provenance) + (1|BLK), data=ht_data_clim14_long %>% filter(climvar.name==provenance_climvars[i]) %>% filter(!Provenance=="Gila NF"))
pvalue_nog<- as.data.frame(summary(mod_nog)$coefficients)[2,]$`Pr(>|t|)`
...
for(i in 1:length(provenance_climvars)){
mod <- lmer(value ~ climvar.value + I(climvar.value^2) + (1|Provenance) + (1|BLK), data=ht_data_clim14_long %>% filter(climvar.name==provenance_climvars[i]))
pvalues<- as.data.frame(summary(mod)$coefficients)[2:3,]$`Pr(>|t|)`

mod_nog <- lmer(value ~ climvar.value + I(climvar.value^2) + (1|Provenance) + (1|BLK), data=ht_data_clim14_long %>% filter(climvar.name==provenance_climvars[i]) %>% filter(!Provenance=="Gila NF"))
pvalues_nog<- as.data.frame(summary(mod_nog)$coefficients)[2:3,]$`Pr(>|t|)`
...
```{r}
##try heights in other years

ht_data_clim91 <- ht_data_clim %>% 
  filter(name=="HT91") %>% 
  left_join(pca_dist_df %>% dplyr::select(code, dist.to.plant,dist.to.plant.3d)) %>% 
  left_join(geo.dist.df)

ht_data_clim91_long <- ht_data_clim91 %>% 
  pivot_longer(c(AHM:SHM, smrpb, monsoonality), names_to="climvar.name", values_to = "climvar.value")

ht_mods91<- data.frame()

for(i in 1:length(provenance_climvars)){
mod <- lmer(value ~ climvar.value + (1|Provenance) + (1|BLK), data=ht_data_clim91_long %>% filter(climvar.name==provenance_climvars[i]))
pvalue<- as.data.frame(summary(mod)$coefficients)[2,]$`Pr(>|t|)`

mod_nog <- lmer(value ~ climvar.value + (1|Provenance) + (1|BLK), data=ht_data_clim91_long %>% filter(climvar.name==provenance_climvars[i]) %>% filter(!Provenance=="Gila NF"))
pvalue_nog<- as.data.frame(summary(mod_nog)$coefficients)[2,]$`Pr(>|t|)`
...
ht_mods91 %>% arrange(pvalue_nog) ##lots significant with Gila, but when it's taken out, nothing is significant

ggplot(ht_data_clim91_long %>% filter(climvar.name=="RH"), aes(x=climvar.value, y=value))+
  geom_point()+
  geom_smooth(method="lm")
```
...
for(i in 1:length(provenance_climvars)){
mod <- lmer(value ~ climvar.value + (1|Provenance) + (1|BLK), data=ht_SR_long %>% filter(climvar.name==provenance_climvars[i]))
pvalue<- as.data.frame(summary(mod)$coefficients)[2,]$`Pr(>|t|)`
...
cor(ht_SR %>% dplyr::select(c(AHM:SHM,elev,x,y,smrpb,monsoonality)))
#TD is the main differentiating climate factor in the southern rockies, this might be why it's coming out. 

##TD
ht_TD_mod <- lmer(value ~ climvar.value + (1|Provenance) + (1|BLK), data=ht_SR_long %>% filter(climvar.name=="TD"))
plot(ht_TD_mod)
qqmath(ht_TD_mod)
summary(ht_TD_mod)
...
##RH
ht_RH_mod <- lmer(value ~ climvar.value + (1|Provenance) + (1|BLK), data=ht_SR_long %>% filter(climvar.name=="RH"))
plot(ht_RH_mod)
qqmath(ht_RH_mod)
summary(ht_RH_mod)

ggplot(data=ht_SR_long %>% filter(climvar.name=="RH"), aes(x=climvar.value, y=value))+
  geom_point()+
  geom_smooth(method="lm")

##smrpb
ht_smrpb_mod <- lmer(value ~ climvar.value + (1|Provenance) + (1|BLK), data=ht_SR_long %>% filter(climvar.name=="smrpb"))
plot(ht_smrpb_mod)
qqmath(ht_smrpb_mod)
summary(ht_smrpb_mod)

ggplot(data=ht_SR_long %>% filter(climvar.name=="smrpb"), aes(x=climvar.value, y=value))+
  geom_point()+
  geom_smooth(method="lm")
...
for(i in 1:length(provenance_climvars)){
mod <- lmer(value ~ climvar.value + I(climvar.value^2) + (1|Provenance) + (1|BLK), data=ht_SR_long %>% filter(climvar.name==provenance_climvars[i]))
pvalues<- as.data.frame(summary(mod)$coefficients)[2:3,]$`Pr(>|t|)`
...
##MAP
ht_map_quad <- lmer(value ~ climvar.value + I(climvar.value^2) + (1|Provenance) + (1|BLK), data=ht_SR_long %>% filter(climvar.name=="MAP"))
plot(ht_map_quad)
qqmath(ht_map_quad)
summary(ht_map_quad)
...
##Latitude
htmod_SR <- lmer(value ~ y + (1|Provenance) + (1|BLK), data=ht_SR)
plot(htmod_SR)
qqmath(htmod_SR)
summary(htmod_SR)
...
#nursery height
totht_mod_nursery <- lmer(value ~ Seedling.Ht..from.Nursery + (1|Provenance) + (1|BLK), data=ht_data_clim14)
plot(totht_mod_nursery)
qqmath(totht_mod_nursery)
summary(totht_mod_nursery) 

totht_mod_nursery_nog <- lmer(value ~ Seedling.Ht..from.Nursery + (1|Provenance) + (1|BLK), data=ht_data_clim14_nog)
plot(totht_mod_nursery_nog)
qqmath(totht_mod_nursery_nog)
summary(totht_mod_nursery_nog) ##marginally significant effect of nursery height on total height in 2014 when Gila is removed
...
ggplot(data=dbh_SR_long %>% filter(climvar.name=="TD"), aes(x=climvar.value, y=DBH))+
  geom_point()+
  geom_smooth(method="lm")
...
for(i in 1:length(provenance_climvars)){
mod <- lmer(value ~ climvar.value + (1|Provenance) + (1|BLK), data=ht_NR_long %>% filter(climvar.name==provenance_climvars[i]))
pvalue<- as.data.frame(summary(mod)$coefficients)[2,]$`Pr(>|t|)`
...
ht_NR_mods %>% arrange(pvalue) ##eFFP, SHM, EMT, PPTsm, NFFD...and many more

##TD
ht_SHM_mod <- lmer(value ~ climvar.value + (1|Provenance) + (1|BLK), data=ht_NR_long %>% filter(climvar.name=="SHM"))
plot(ht_SHM_mod)
qqmath(ht_SHM_mod)
summary(ht_SHM_mod)
...
for(i in 1:length(provenance_climvars)){
mod <- lmer(value ~ climvar.value + I(climvar.value^2) + (1|Provenance) + (1|BLK), data=ht_NR_long %>% filter(climvar.name==provenance_climvars[i]))
pvalues<- as.data.frame(summary(mod)$coefficients)[2:3,]$`Pr(>|t|)`
...
ht_NR_mods_quad %>% arrange(pvalue2) #MAP and many more have significant quadratic effect
```
...
#dredge.cone.clim.hty <- dredge(cone_clim_mod_hty)
#saveRDS(dredge.cone.clim.hty, "dredge.cone.clim.hty.RDS")
dredge.cone.clim.hty <- readRDS("dredge.cone.clim.hty.RDS")
bestmod_cone_clim_hty <- glmmTMB(cbind(n_cones, total-n_cones) ~ PC1 + PC2 + I(PC2^2) + scale(height) + (1|code) + (1|year) + (1|BLK), data=moniger_subset, family=binomial)
summary(bestmod_cone_clim_hty)
...
best.mod.cone.clim <- glmmTMB(cbind(n_cones, total-n_cones) ~ scale(height) + PC1 + PC2 + I(PC2^2) + (1|code) + (1|year) + (1|BLK), data= moniger_subset, family = binomial)
summary(best.mod.cone.clim)

##predictions
CCnewdata1_cone <- expand.grid(height=seq(min(moniger_subset %>% pull(height)),
                                     max(moniger_subset %>% pull(height)),1),
                          PC2 = seq(min(moniger_subset %>% pull(PC2)),
                                    max(moniger_subset  %>% pull(PC2)),1),
                          PC1 = mean(moniger_subset  %>% pull(PC1)))


cone_clim_mod_glm1 <- glmmTMB(cbind(n_cones, total-n_cones) ~ scale(height) + PC1 + PC2 + I(PC2^2), data= moniger_subset, family = binomial)
...
ggplot(CCnewdata3_cone, aes(x = PC2, y = PredictedProb, col= factor(height))) + 
  geom_ribbon(aes(ymin = LL,
    ymax = UL, fill = factor(height)), alpha = 0.2, col=NA) + geom_line(aes(colour = factor(height)),
    linewidth = 1)+
  geom_line(aes(x=PC2, y=PredictedProb, col=factor(height)))+
  scale_color_manual(values=viridis::mako(n=19), name = "height")+
  scale_fill_manual(values=viridis::mako(n=19),name="height")+
  theme_bw()+
  xlab("PC2")+
...
best.mod.cone.clim_nog <- glmmTMB(cbind(n_cones, total-n_cones) ~ scale(height)*scale(x) + scale(height)*PC2 + scale(height)*PC1 + (1|code) + (1|year) + (1|BLK), data= moniger_subset %>% filter(!code=="GiNF"), family = binomial)
summary(best.mod.cone.clim_nog)

##predictions - no gila
CCnewdata1 <- expand.grid(height=seq(min(moniger_subset %>% filter(!code=="GiNF") %>% pull(height)),
                                     max(moniger_subset %>% filter(!code=="GiNF") %>% pull(height)),1),
                          PC1 = seq(min(moniger_subset %>% filter(!code=="GiNF") %>% pull(PC1)),
                                    max(moniger_subset %>% filter(!code=="GiNF") %>% pull(PC1)),1),
                          x=mean(moniger_subset %>% filter(!code=="GiNF") %>% pull(x)),
                          PC2 = mean(moniger_subset %>% filter(!code=="GiNF") %>% pull(PC2)))


cone_clim_mod_glm <- glmmTMB(cbind(n_cones, total-n_cones) ~ scale(height)*scale(x) + scale(height)*PC2 + scale(height)*PC1, data= moniger_subset %>% filter(!code=="GiNF"), family = binomial)
...
ggplot(CCnewdata3, aes(x = height, y = PredictedProb, col= factor(PC1))) + 
  geom_ribbon(aes(ymin = LL,
    ymax = UL, fill = factor(PC1)), alpha = 0.2, col=NA) + geom_line(aes(colour = factor(PC1)),
    linewidth = 1)+
  geom_line(aes(x=height, y=PredictedProb, col=factor(PC1)))+
  scale_color_manual(values=viridis::mako(n=13), name = "PC1")+
  scale_fill_manual(values=viridis::mako(n=13),name="PC1")+
  theme_bw()+
  xlab("Height (m)")+
  ylab("Predicted Probability of Cones")+
  theme(legend.position = "bottom")
...
##predictions - no gila PC2
CCnewdata1_pc2 <- expand.grid(height=seq(min(moniger_subset %>% filter(!code=="GiNF") %>% pull(height)),
                                     max(moniger_subset %>% filter(!code=="GiNF") %>% pull(height)),1),
                          PC2 = seq(min(moniger_subset %>% filter(!code=="GiNF") %>% pull(PC2)),
                                    max(moniger_subset %>% filter(!code=="GiNF") %>% pull(PC2)),1),
                          x=mean(moniger_subset %>% filter(!code=="GiNF") %>% pull(x)),
                          PC1 = mean(moniger_subset %>% filter(!code=="GiNF") %>% pull(PC1)))

CCnewdata2_pc2 <- cbind(CCnewdata1_pc2, predict(cone_clim_mod_glm, newdata = CCnewdata1_pc2, type = "link", se=TRUE))
CCnewdata3_pc2 <- within(CCnewdata2_pc2, {
    PredictedProb <- plogis(fit)
    LL <- plogis(fit - (1.96 * se.fit))
    UL <- plogis(fit + (1.96 * se.fit))
})

ggplot(CCnewdata3_pc2, aes(x = height, y = PredictedProb, col= factor(PC2))) + 
  geom_ribbon(aes(ymin = LL,
    ymax = UL, fill = factor(PC2)), alpha = 0.2, col=NA) + geom_line(aes(colour = factor(PC2)),
    linewidth = 1)+
  geom_line(aes(x=height, y=PredictedProb, col=factor(PC2)))+
  scale_color_manual(values=viridis::mako(n=13), name = "PC2")+
  scale_fill_manual(values=viridis::mako(n=13),name="PC2")+
  theme_bw()+
  xlab("Height (m)")+
  ylab("Predicted Probability of Cones")+
  theme(legend.position = "bottom")
...
cone_mods %>% arrange(pvalue) ##smrpb & PPTsm are significant for both
...
cone_smrpb_mod_nog <- glmmTMB(cbind(n_cones,total-n_cones) ~ climvar.value + (1|year) + (1|Provenance), data=moniger_provclim %>% filter(climvar.name=="smrpb") %>% filter(!Provenance=="Gila NF"), family=binomial)
...
##
survmod_pcx<- glmmTMB(cbind(alive,died) ~  PC1*PC2 + scale(x)*PC1 + scale(x)*PC2 + I(PC1^2) + I(PC2^2) + scale(x) + I(scale(x)^2) + (1|Provenance) + (1|BLK), data=moniger_survival_byyr_binom_2014, family=binomial) 
summary(survmod_pcx)

survmod_pcy<- glmmTMB(cbind(alive,died) ~ PC1*PC2 + scale(y)*PC1 + scale(y)*PC2 + I(PC1^2) + I(PC2^2) + scale(y) + I(scale(y)^2) + (1|Provenance) + (1|BLK), data=moniger_survival_byyr_binom_2014, family=binomial)
summary(survmod_pcy)
...
dredge_survmod_pcy
bestsurvmod_pcy <- glmmTMB(cbind(alive,died) ~ PC1*scale(y) + I(PC1^2) + PC2 + I(PC2^2)+ I(scale(y)^2) + (1|Provenance) + (1|BLK), data=moniger_survival_byyr_binom_2014, family=binomial)
...
moniger_survival_nog <- moniger_survival_byyr_binom_2014 %>% filter(!Provenance%in%c("Gila NF"))

survmod_pcy_nog<- glmmTMB(cbind(alive,died) ~ PC1*PC2 + scale(y)*PC1 + scale(y)*PC2 + I(PC1^2) + I(PC2^2) + scale(y) + I(scale(y)^2) + (1|Provenance) + (1|BLK), data=moniger_survival_nog, family=binomial)
...
bestsurvmod_pcy_nog <- glmmTMB(cbind(alive,died) ~ PC1*scale(y) + I(PC1^2) + PC2 + I(PC2^2)+ I(scale(y)^2) + (1|Provenance) + (1|BLK), data=moniger_survival_nog, family=binomial)
summary(bestsurvmod_pcy_nog)

#predict
newdata1 <- expand.grid(PC1 = c(mean(moniger_survival_nog$PC1)+sd(moniger_survival_nog$PC1),mean(moniger_survival_nog$PC1),mean(moniger_survival_nog$PC1)-sd(moniger_survival_nog$PC1)),
                        PC2 = mean(moniger_survival_nog$PC2),
                        y = seq(min(moniger_survival_nog$y),max(moniger_survival_nog$y),2),
                        BLK= "C",
                        Provenance="Pike NF")
newdata2_nog <- cbind(newdata1, predict(bestsurvmod_pcy_nog, newdata = newdata1, type = "link", se=TRUE))
...
ggplot(data=newdata3_nog, aes(x = y, y = PredictedProb, col=factor(PC1))) + 
  geom_line()+
  geom_ribbon(aes(ymin = LL, ymax = UL, fill=factor(PC1)), alpha=0.5) +
  theme(axis.text.x = element_text(angle=45, hjust=1))

##with gila
newdata2 <- cbind(newdata1, predict(bestsurvmod_pcy, newdata = newdata1, type = "link", se=TRUE))
newdata3 <- within(newdata2, {
    PredictedProb <- plogis(fit)
    LL <- plogis(fit - (1.96 * se.fit))
    UL <- plogis(fit + (1.96 * se.fit))
...
ggplot(data=newdata3, aes(x = y, y = PredictedProb, col=factor(PC1))) + 
  geom_line()+
  geom_ribbon(aes(ymin = LL, ymax = UL, fill=factor(PC1)), alpha=0.5) +
  theme(axis.text.x = element_text(angle=45, hjust=1))

###varying PC2
newdata1_pc2 <- expand.grid(PC2 = seq(min(moniger_survival_nog$PC2),max(moniger_survival_nog$PC2),.5),
                        PC1 = mean(moniger_survival_nog$PC1),
                        y = c(mean(moniger_survival_nog$y) + sd(moniger_survival_nog$y), mean(moniger_survival_nog$y), mean(moniger_survival_nog$y) - sd(moniger_survival_nog$y)),
                        BLK= "C",
                        Provenance="Pike NF")

newdata2_pc2 <- cbind(newdata1_pc2, predict(bestsurvmod_pcy_nog, newdata = newdata1_pc2, type = "link", se=TRUE))
newdata3_pc2 <- within(newdata2_pc2, {
    PredictedProb <- plogis(fit)
    LL <- plogis(fit - (1.96 * se.fit))
    UL <- plogis(fit + (1.96 * se.fit))
})

ggplot(newdata3_pc2,aes(x = PC2, y = PredictedProb, col=factor(y), fill=factor(y))) + 
...
#ggarrange(ht_dbh_lat_plot, ggarrange(surv_pc1_plot ,surv_pc2_plot , cone_lat_plot, nrow=3, ncol=1),nrow=1)
```
...
```{r}
moniger_survival_long <- moniger_survival_byyr_binom_2014 %>% 
  pivot_longer(c(AHM:SHM, smrpb, monsoonality), names_to="climvar.name", values_to = "climvar.value")

surv_mods<- data.frame()

for(i in 1:length(provenance_climvars)){
mod <- glmmTMB(cbind(alive,died) ~ climvar.value + (1|Provenance) + (1|BLK), data=moniger_survival_long %>% filter(climvar.name==provenance_climvars[i]), family=binomial)
pvalue<- as.data.frame(summary(mod)$coefficients$cond)[2,]$`Pr(>|z|)`

mod_nog <- glmmTMB(cbind(alive,died) ~ climvar.value + (1|Provenance)+ (1|BLK), data=moniger_survival_long %>% filter(climvar.name==provenance_climvars[i]) %>% filter(!Provenance=="Gila NF"), family=binomial)
...
#NFFD
surv_NFFD_mod <- glmmTMB(cbind(alive,died) ~ scale(climvar.value) + (1|Provenance) + (1|BLK), data=moniger_survival_long %>% filter(climvar.name=="NFFD"), family="binomial")
...
##predictions
survNFFD_newdata1 <- expand.grid(climvar.value = seq(min(surv_NFFD_data$climvar.value),max(surv_NFFD_data$climvar.value),length.out=50),
                                 Provenance = "Pike NF",
                                 BLK = "C")

survNFFD_newdata2 <- cbind(survNFFD_newdata1, predict(surv_NFFD_mod, newdata = survNFFD_newdata1, type = "link", se=TRUE))
...
surv_NFFD_mod_nog <- glmmTMB(cbind(alive,died) ~ scale(climvar.value) + (1|Provenance) + (1|BLK), data=moniger_survival_long %>% filter(climvar.name=="NFFD") %>% filter(!Provenance=="Gila NF"), family=binomial)
summary(surv_NFFD_mod_nog)
############


#bFFP
surv_bFFP_mod <- glmmTMB(cbind(alive,died) ~ scale(climvar.value) + (1|Provenance)+ (1|BLK), data=moniger_survival_long %>% filter(climvar.name=="bFFP"), family=binomial)
...
#without Gila is still significant
surv_bFFP_mod_nog <- glmmTMB(cbind(alive,died) ~ scale(climvar.value) + (1|Provenance)+ (1|BLK), data=moniger_survival_long %>% filter(climvar.name=="bFFP") %>% filter(!Provenance=="Gila NF"), family=binomial)
summary(surv_bFFP_mod_nog)
#########

#eFFP
surv_eFFP_mod <- glmmTMB(cbind(alive,died) ~ scale(climvar.value) + (1|Provenance)+ (1|BLK), data=moniger_survival_long %>% filter(climvar.name=="eFFP"), family="binomial")
...
#without Gila is still significant
surv_eFFP_mod_nog <- glmmTMB(cbind(alive,died) ~ scale(climvar.value) + (1|Provenance)+ (1|BLK), data=moniger_survival_long %>% filter(climvar.name=="eFFP") %>% filter(!Provenance=="Gila NF"), family="binomial")
summary(surv_eFFP_mod_nog)
#########

#smrpb
surv_smrpb_mod <- glmmTMB(cbind(alive,died) ~ scale(climvar.value) + (1|Provenance)+ (1|BLK), data=moniger_survival_long %>% filter(climvar.name=="smrpb"), family="binomial")
...
survsmrpb_newdata2 <- cbind(survsmrpb_newdata1, predict(surv_smrpb_mod, newdata = survsmrpb_newdata1, type = "link", se=TRUE))
...
#without Gila is still significant
surv_smrpb_mod_nog <- glmmTMB(cbind(alive,died) ~ scale(climvar.value) + (1|Provenance)+(1|BLK), data=moniger_survival_long %>% filter(climvar.name=="smrpb") %>% filter(!Provenance=="Gila NF"), family="binomial")
...
for(i in 1:length(provenance_climvars)){
mod <- glmmTMB(cbind(alive,died) ~ scale(climvar.value) + I(scale(climvar.value)^2) + (1|Provenance) + (1|BLK), data=moniger_survival_long %>% filter(climvar.name==provenance_climvars[i]), family="binomial")
pvalues<- as.data.frame(summary(mod)$coefficients$cond)[2:3,]$`Pr(>|z|)`

mod_nog <- glmmTMB(cbind(alive,died) ~ scale(climvar.value) + I(scale(climvar.value)^2) + (1|Provenance) + (1|BLK), data=moniger_survival_long %>% filter(climvar.name==provenance_climvars[i]) %>% filter(!Provenance=="Gila NF"), family="binomial")
pvalues_nog<- as.data.frame(summary(mod_nog)$coefficients$cond)[2:3,]$`Pr(>|z|)`
...
##MCMT
surv_MCMT_mod <- glmmTMB(cbind(alive,died) ~ scale(climvar.value) + I(scale(climvar.value)^2) + (1|Provenance) + (1|BLK), data=moniger_survival_long %>% filter(climvar.name=="MCMT"), family="binomial")
summary(surv_MCMT_mod)

surv_MCMT_data <- moniger_survival_long %>% filter(climvar.name=="MCMT")

##predictions
survMCMT_newdata1 <- expand.grid(climvar.value = seq(min(surv_MCMT_data$climvar.value),max(surv_MCMT_data$climvar.value),length.out=50),
                                 Provenance = "Pike NF",
                                 BLK = "C")

survMCMT_newdata2 <- cbind(survMCMT_newdata1, predict(surv_MCMT_mod, newdata = survMCMT_newdata1, type = "link", se=TRUE))
survMCMT_newdata3 <- within(survMCMT_newdata2, {
    PredictedProb <- plogis(fit)
    LL <- plogis(fit - (1.96 * se.fit))
    UL <- plogis(fit + (1.96 * se.fit))
})

surv_MCMT_plot<- ggplot(survMCMT_newdata3, aes(x = climvar.value, y = PredictedProb)) + geom_ribbon(aes(ymin = LL,
    ymax = UL), alpha = 0.2) + 
  geom_line(linewidth = 1)+
  xlab("MCMT")+
    ylab("Predicted Probability")+
...
#without Gila is still significant
surv_MCMT_mod_nog <- glmmTMB(cbind(alive,died) ~ scale(climvar.value) + I(scale(climvar.value)^2) + (1|Provenance) + (1|BLK), data=moniger_survival_long %>% filter(climvar.name=="MCMT") %>% filter(!Provenance=="Gila NF"), family="binomial")
summary(surv_MCMT_mod_nog)
#########

##cmd
surv_cmd_mod <- glmmTMB(cbind(alive,died) ~ scale(climvar.value) + I(scale(climvar.value)^2) + (1|Provenance) + (1|BLK), data=moniger_survival_long %>% filter(climvar.name=="CMD"), family="binomial")
...
survcmd_newdata2 <- cbind(survcmd_newdata1, predict(surv_cmd_mod, newdata = survcmd_newdata1, type = "link", se=TRUE))
...
#without Gila is still significant
surv_cmd_mod_nog <- glmmTMB(cbind(alive,died) ~ scale(climvar.value) + I(scale(climvar.value)^2) + (1|Provenance) + (1|BLK), data=moniger_survival_long %>% filter(climvar.name=="CMD") %>% filter(!Provenance=="Gila NF"), family="binomial")
summary(surv_cmd_mod_nog)


##Eref
surv_Eref_mod <- glmmTMB(cbind(alive,died) ~ scale(climvar.value) + I(scale(climvar.value)^2) + (1|Provenance) + (1|BLK), data=moniger_survival_long %>% filter(climvar.name=="Eref"), family="binomial")
...
survEref_newdata2 <- cbind(survEref_newdata1, predict(surv_Eref_mod, newdata = survEref_newdata1, type = "link", se=TRUE))
...
#without Gila is still significant
surv_Eref_mod_nog <- glmmTMB(cbind(alive,died) ~ scale(climvar.value) + I(scale(climvar.value)^2) + (1|Provenance) + (1|BLK), data=moniger_survival_long %>% filter(climvar.name=="Eref") %>% filter(!Provenance=="Gila NF"), family="binomial")
...
ggplot(growth.detrended.byprov, aes(x=year, y=meanrwi-1))+
  ...
  geom_point(aes( col=Provenance), size=2)+
  geom_ribbon(aes(x=year, ymin=meanrwi-se-1, ymax=meanrwi+se-1, fill=Provenance), alpha=0.3)+
...
selected_climvars <-  c("vpdmax_wt","value_wt_tmean", "aet_sum","prior_PPTsm")
...
correlation_plot<- ggplot(data=sig.cors.byprov %>% filter(climvar %in% selected_climvars) %>% left_join(prov_names, by=c("newname"="Provenance")), aes(x=factor(code, levels=provs_bylat$code), y=estimate))+
  ...
  facet_wrap(~factor(climvar, levels=selected_climvars),ncol=1)+
  xlab("")+
  theme_bw()+
