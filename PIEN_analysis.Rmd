...
provenance_climvars <- dbh_num14 %>% 
  dplyr::select(MAT:PPTsm, smrpb, monsoonality) %>% 
  colnames()

dbh_num14_long <- dbh_num14 %>% 
  pivot_longer(c(MAT:PPTsm, smrpb, monsoonality))
...
cor(ht_SR %>% dplyr::select(c(MAT:PPTsm,elev,x,y,smrpb,monsoonality)))
...
ht_NR_mods %>% arrange(pvalue) ##eFFP, SHM, EMT, PPTsm, NFFD
