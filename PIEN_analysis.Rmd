...
for(i in 1:length(climvars)){
  for(j in 1:length(names)){
    ...
    mod <- lmer(cor ~ value + (1|Provenance), data=data)
    pvalue.lmer <- as.data.frame(summary(mod)$coefficients)[2,] %>% pull(`Pr(>|t|)`)
    r2.lmer <- performance::r2(mod)
    
lmerdata_SR <- bind_rows(lmerdata_SR, data.frame(climvar=climvars[i], name=names[j], pvalue.lmer = pvalue.lmer, r2.lmer=r2.lmer)) 
  }}
...
lmerdata_pcs_SR <- bind_rows(lmerdata_pcs_SR, data.frame(climvar=climvars[i], pc1value.lmer = pc1pvalue.lmer, pc2value.lmer = pc2pvalue.lmer, pc1pc2value.lmer = pc1pc2pvalue.lmer, r2.lmer=r2.lmer)) 
}
