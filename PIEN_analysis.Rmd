...
interact_plot(bestmod_dbh_pca_y, pred=y, modx=PC2, plot.points = TRUE, interval = TRUE, int.width = 0.95, colors="blue2", data=dbh_num14_alldist)
...
##predict model
mod_ht_dist_lm <- lm(value ~ scale(geo.dist.to.plant.km)+scale(dist.to.plant.3d), data=ht_data_clim14_nog)

ht_dist_newdata <- with(ht_data_clim14_nog, data.frame(geo.dist.to.plant.km = seq(min(geo.dist.to.plant.km),max(geo.dist.to.plant.km),length.out=100), 
                                                       dist.to.plant.3d = mean(dist.to.plant.3d)))
...
interact_plot(bestsurvmod_pcy, pred=y, modx=PC1, plot.points = TRUE, interval = TRUE, int.width = 0.95, colors="blue2", data=moniger_survival)
...
interact_plot(bestsurvmod_pcy_nog, pred=y, modx=PC1, plot.points = TRUE, interval = TRUE, int.width = 0.95, colors="blue2", data=moniger_survival_nog)
...
#predict
newdata1 <- with(moniger_survival, data.frame(PC1 = rep(seq(min(PC1),max(PC1),2), each=50),
                                              PC2 = mean(PC2),
...
###varying PC2
newdata1_pc2 <- with(moniger_survival, data.frame(PC1 = mean(PC1),
                                              PC2 = seq(min(PC2),max(PC2),length.out=50),
                                              y = mean(y)))

newdata2_pc2 <- cbind(newdata1_pc2, predict(bestsurvmod_pcy_nog_glm, newdata = newdata1_pc2, type = "link", se=TRUE))
...
ggplot(newdata3_pc2,aes(x = PC2, y = PredictedProb)) + 
  geom_ribbon(aes(ymin = LL, ymax = UL), alpha=0.5) +
  geom_line(aes(x = PC2, y = PredictedProb))+
  theme(axis.text.x = element_text(angle=45, hjust=1))
...
interact_plot(bestmod_survmod_dist, pred=dist.to.plant.3d, modx=geo.dist.to.plant.km, plot.points = TRUE, interval = TRUE, int.width = 0.95, colors="blue2", data=surv_pcadist_data)
