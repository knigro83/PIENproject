...
all_seascorr <- seascorr(chrono = chron.growth.detrended.all,
                         climate= list(ppt_comb_plantation_wide, tmean_comb_plantation_wide),
                         var_names=c("ppt","tmean"))


test2<- seascorr(chrono = chron.growth.detrended.all,
                         climate= list(tmean_comb_plantation_wide, ppt_comb_plantation_wide),
                         var_names=c("tmean","ppt"))
...
dataset1 <- output[[1]]$Dataset
ConfidenceSet <- dataset1[which(cumsum(dataset1$ModWeight) <= 0.95),]
...
head(output[[2]]$Dataset)
summary(output[[2]]$BestModel)

##Although these results point to the presence of a strong climate signal in F. coelebs laying date, we cannot be sure that this result has not occurred due to chance. To test this possibility, we next run the randomisation procedure using the function randwin, with repeats = 5.
...
pvalue(dataset = output[[1]]$Dataset, datasetrand = output.rand[[1]], metric = "C", sample.size = 20)  ##high pvalue means that this climate signal is not very strong (high probability that it occurred by chance)
...
pvalue(dataset = output[[1]]$Dataset, datasetrand = output.randk[[1]], metric = "C", sample.size = 20)
