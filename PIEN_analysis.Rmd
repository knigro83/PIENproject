...
```{r}
##Look at just southern Rockies, excluding Gila
ht_NR <- ht_data_clim14_nogila %>% filter(y > 44)
...
ave_dbhs <- moniger_data_comb %>% 
  dplyr::select(c(SORT,Provenance,dbh85:dbh06)) %>% 
  pivot_longer(dbh85:dbh06) %>% 
  mutate(year=case_when(name == "dbh85" ~ 1985,
                        name == "dbh91" ~ 1991,
                        name == "dbh96" ~ 1996,
                        name == "dbh06" ~ 2006)) %>% 
  filter(!is.na(value)) %>% 
...
ggplot(moniger_data_comb, aes(x=dbh85, y=Cones85))+
...
cone_clim_mod_htx <- glmer(Cones ~ scale(height)*scale(PC1) + I(scale(PC1)^2) + scale(height)*scale(PC2) + I(scale(PC2)^2) + scale(height)*scale(x) + I(scale(x)^2)+ (1|code) + (1|year), data=moniger_subset, family="binomial")
...
AICc(cone_clim_mod_hty, cone_clim_mod_dbhx, cone_clim_mod_dbhy,cone_clim_mod_htx)
...
cone_dist_mod_glm <- glm(Cones ~ scale(dist.to.plant)*scale(geo.dist.to.plant.km)*scale(height), data=cone_dist_data, family="binomial")

```
#START HERE 7/9/24
```{r}
##predictions
CDnewdata1 <- with(cone_dist_data, data.frame(geo.dist.to.plant.km = seq(min(geo.dist.to.plant.km), max(geo.dist.to.plant.km), length.out=50), 
                                              dist.to.plant=mean(dist.to.plant), 
...
ggplot(CDnewdata3, aes(x = geo.dist.to.plant.km, y = PredictedProb, col=height>5)) + geom_ribbon(aes(ymin = LL,
    ymax = UL, fill=height>5), alpha = 0.2) + geom_line(linewidth = 1, aes(col=height>5))+
  geom_point()
...
cone_smrpb_mod_nog <- glmer(Cones ~ scale(climvar.value) + (1|Provenance), data=moniger_provclim %>% filter(climvar.name=="smrpb") %>% filter(!Provenance=="Gila NF"), family="binomial")
