...
```{r}
##PCs and x 
mod_dbh_pca_x <- lmer(DBH ~ PC1*PC2 + PC1*x + PC2*x + I(PC1^2) + I(PC2^2) + I(scale(x)^2) + (1|Provenance), data=dbh_num14_alldist)
plot(mod_dbh_pca_x)
qqmath(mod_dbh_pca_x)
summary(mod_dbh_pca_x)
dredge(mod_dbh_pca_x)

bestmod_dbh_pca_x <- lmer(DBH ~ PC1 + I(scale(x)^2) + (1|Provenance), data=dbh_num14_alldist)
summary(bestmod_dbh_pca_x)

mod_dbh_pca_x_nog <- lmer(DBH ~ PC1*PC2 + PC1*x + PC2*x + I(PC1^2) + I(PC2^2) + I(scale(x)^2) + (1|Provenance), data=dbh_num14_alldist_noG)
plot(mod_dbh_pca_x_nog)
qqmath(mod_dbh_pca_x_nog)
summary(mod_dbh_pca_x_nog)
dredge(mod_dbh_pca_x_nog)

bestmod_dbh_pca_x_nog <- lmer(DBH ~ I(scale(x)^2) + (1|Provenance), data=dbh_num14_alldist_noG)
summary(bestmod_dbh_pca_x_nog) #no real patterns when you take out Gila

##PCs and y 
mod_dbh_pca_y <- lmer(DBH ~ PC1*PC2 + PC1*y + PC2*y + I(PC1^2) + I(PC2^2) + I(scale(y)^2) + (1|Provenance), data=dbh_num14_alldist)
plot(mod_dbh_pca_y)
qqmath(mod_dbh_pca_y)
summary(mod_dbh_pca_y)
dredge(mod_dbh_pca_y)

bestmod_dbh_pca_y <- lmer(DBH ~ PC1 + I(scale(y)^2) + (1|Provenance), data=dbh_num14_alldist)
summary(bestmod_dbh_pca_y)

ggplot(dbh_num14_alldist, aes(x=y, y=DBH))+
  geom_point()

mod_dbh_pca_y_nog <- lmer(DBH ~ PC1*PC2 + PC1*y + PC2*y + I(PC1^2) + I(PC2^2) + I(scale(y)^2) + (1|Provenance), data=dbh_num14_alldist_noG)
plot(mod_dbh_pca_y_nog)
qqmath(mod_dbh_pca_y_nog)
summary(mod_dbh_pca_y_nog)
dredge(mod_dbh_pca_y_nog)

bestmod_dbh_pca_y_nog <- lmer(DBH ~ I(scale(y)^2) + (1|Provenance), data=dbh_num14_alldist_noG)
summary(bestmod_dbh_pca_y_nog) #quadratic y effect stil significant when you take out Gila
...
#Longitude
mod_dbh_lon <- lmer(DBH ~ scale(x) + I(scale(x)^2)+(1|Provenance), data=dbh_num14)
plot(mod_dbh_lon)
qqmath(mod_dbh_lon)
summary(mod_dbh_lon) #no significant effects of longitude

  #take out Gila
mod_dbh_lon_nogila <- lmer(DBH ~ scale(x) + I(scale(x)^2) + (1|Provenance), data=dbh_num14_nogila)
...
ggplot(ht_SR, aes(x=map, y=value))+
  geom_point()
...
totht_mod_nursery_nog <- lmer(value ~ Seedling.Ht..from.Nursery + (1|Provenance), data=ht_data_clim14_nogila)
...
cone_clim_mod_htx <- glmer(Cones ~ scale(height)*scale(PC1) + I(scale(PC1)^2) + scale(height)*scale(PC2) + I(scale(PC2)^2) + scale(height)*scale(x) + I(scale(x)^2)+ (1|code), data=moniger_subset, family="binomial")
summary(cone_clim_mod_htx)

cone_clim_mod_hty <- glmer(Cones ~ scale(height)*scale(PC1) + I(scale(PC1)^2) + scale(height)*scale(PC2) + I(scale(PC2)^2) + scale(height)*scale(y) + I(scale(y)^2)+ (1|code), data=moniger_subset, family="binomial")
summary(cone_clim_mod_hty)

cone_clim_mod_dbhx <- glmer(Cones ~ scale(DBH)*scale(PC1) + I(scale(PC1)^2) + scale(DBH)*scale(PC2) + I(scale(PC2)^2) + scale(DBH)*scale(x) + I(scale(x)^2)+ (1|code), data=moniger_subset, family="binomial")
summary(cone_clim_mod_dbhx)

cone_clim_mod_dbhy <- glmer(Cones ~ scale(DBH)*scale(PC1) + I(scale(PC1)^2) + scale(DBH)*scale(PC2) + I(scale(PC2)^2) + scale(DBH)*scale(y) + I(scale(y)^2)+ (1|code), data=moniger_subset, family="binomial")
summary(cone_clim_mod_dbhy)

AICc(cone_clim_mod_htx, cone_clim_mod_hty, cone_clim_mod_dbhx, cone_clim_mod_dbhy)

cone_clim_mod_glm <- glm(Cones ~ scale(height)*scale(PC1) + scale(height)*scale(PC2) + scale(height)*scale(x), data=moniger_subset, family="binomial")
summary(cone_clim_mod_glm)

##predictions
CCnewdata1 <- with(moniger_subset, data.frame(height = seq(min(height),max(height),length.out=50), PC1=mean(PC1), PC2 = mean(PC2), x = rep(seq(-121, -105, 1),each=50)))
...
ggplot(CCnewdata3, aes(x = height, y = PredictedProb, col= factor(x))) + geom_ribbon(aes(ymin = LL,
    ymax = UL, fill = factor(x)), alpha = 0.2) + geom_line(aes(colour = factor(x)),
    linewidth = 1)+
  geom_point(aes(x=height, y=PredictedProb, col=factor(x)))
...
```{r}
cone_dist_data <- moniger_data_comb %>% 
  dplyr::select(c(SORT, Cones85, Cones91, Cones96, Cones06, code)) %>%
  pivot_longer(Cones85:Cones06, values_to = "Cones") %>% 
    mutate(year=case_when(name == "Cones85" ~ 1985,
                        name == "Cones91" ~ 1991,
...
cone_dist_mod <- glmer(Cones ~ scale(dist.to.plant)*scale(geo.dist.to.plant.km) + (1|code), data=cone_dist_data, family="binomial")
plot(cone_dist_mod)
summary(cone_dist_mod)

cone_dist_mod_glm <- glm(Cones ~ scale(dist.to.plant)*scale(geo.dist.to.plant.km), data=cone_dist_data, family="binomial")


##predictions
CDnewdata1 <- with(cone_dist_data, data.frame(dist.to.plant = seq(min(dist.to.plant), max(dist.to.plant), by=0.5), geo.dist.to.plant.km=mean(geo.dist.to.plant.km)))
...
ggplot(CDnewdata3, aes(x = dist.to.plant, y = PredictedProb)) + geom_ribbon(aes(ymin = LL,
    ymax = UL), alpha = 0.2) + geom_line(linewidth = 1)+
  geom_point()
...
for(i in 1:length(provenance_climvars)){
mod <- glmer(Cones ~ climvar.value*factor(year) + (1|code), data=moniger_provclim %>% filter(climvar.name==provenance_climvars[i]), family="binomial")
pvalue<- as.data.frame(summary(mod)$coefficients)[2,]$`Pr(>|z|)`

mod_nog <- glmer(Cones ~ climvar.value*factor(year) + (1|code), data=moniger_provclim %>% filter(climvar.name==provenance_climvars[i]) %>% filter(!code=="GiNF"), family="binomial")
...
cone_mods %>% arrange(pvalue_nog) ##smrpb is significant for both

###smrpb
cone_smrpb_mod <- glmer(Cones ~ scale(climvar.value) + (1|code), data=moniger_provclim %>% filter(climvar.name=="smrpb"), family="binomial")
...
cone_smrpb_mod_nog <- glmer(Stat14 ~ scale(climvar.value) + (1|Provenance), data=moniger_coneival_long %>% filter(climvar.name=="smrpb") %>% filter(!Provenance=="Gila NF"), family="binomial")
...
for(i in 1:length(provenance_climvars)){
mod <- glmer(Cones ~ scale(climvar.value) + I(scale(climvar.value)^2) + (1|code), data=moniger_provclim %>% filter(climvar.name==provenance_climvars[i]), family="binomial")
pvalues<- as.data.frame(summary(mod)$coefficients)[2:3,]$`Pr(>|z|)`

mod_nog <- glmer(Cones ~ scale(climvar.value) + I(scale(climvar.value)^2) + (1|code), data=moniger_provclim %>% filter(climvar.name==provenance_climvars[i]) %>% filter(!code=="GiNF"), family="binomial")
...
###TD
cone_TD_mod <- glmer(Cones ~ scale(climvar.value) + I(scale(climvar.value)^2) + (1|code), data=moniger_provclim %>% filter(climvar.name=="TD"), family="binomial")
summary(cone_TD_mod)
...
ggplot(coneTD_newdata3, aes(x = climvar.value, y = PredictedProb)) + geom_ribbon(aes(ymin = LL,
    ymax = UL), alpha = 0.2) + 
  geom_line(linewidth = 1)+
  geom_point(aes(x=climvar.value, y=PredictedProb))+
  xlab("TD")
...
#without Gila is still significant
cone_TD_mod_nog <- glmer(Cones ~ scale(climvar.value) + I(scale(climvar.value)^2) + (1|code), data=moniger_provclim %>% filter(climvar.name=="TD") %>% filter(!Provenance=="Gila NF"), family="binomial")
...
ggplot(newdata3, aes(x = Provenance, y = PredictedProb)) + 
  geom_errorbar(aes(ymin = LL, ymax = UL, col = Provenance)) + 
  geom_point(aes(colour = Provenance))+
  theme(axis.text.x = element_text(angle=45, hjust=1))
```
...
survmod_pcy<- glmer(Stat14 ~ PC1+ I(PC1^2) + PC2 + I(PC2^2) + scale(y) + I(scale(y)^2) + (1|Provenance), data=moniger_survival, family="binomial")
plot(survmod_pcy)
summary(survmod_pcy)
```
...
SDnewdata1 <- with(surv_pcadist_data, data.frame(dist.to.plant = seq(min(surv_pcadist_data$dist.to.plant), 
                      ...
                      length.out = 50),
    geo.dist.to.plant.km = rep(seq(87,1593,100),each=50)))
...
ggplot(SDnewdata3, aes(x = dist.to.plant, y = PredictedProb, col= factor(geo.dist.to.plant.km))) + geom_ribbon(aes(ymin = LL,
    ymax = UL, fill = factor(geo.dist.to.plant.km)), alpha = 0.2) + geom_line(aes(colour = factor(geo.dist.to.plant.km)),
    linewidth = 1)+
  geom_point()
...
newdata_MWMT_surv <- data.frame(
    climvar.value = seq(min(surv_MWMT_data$climvar.value), 
                      max(surv_MWMT_data$climvar.value), 
                      length.out = 100
                      ))
...
ggplot(survMWMT_newdata3, aes(x = climvar.value, y = PredictedProb)) + geom_ribbon(aes(ymin = LL,
    ymax = UL), alpha = 0.2) + 
  geom_line(linewidth = 1)+
  geom_point(aes(x=climvar.value, y=PredictedProb))+
  xlab("MWMT")
...
newdata_cmd_surv <- data.frame(
    climvar.value = seq(min(surv_cmd_data$climvar.value), 
                      max(surv_cmd_data$climvar.value), 
                      length.out = 100
                      ))
...
ggplot(survcmd_newdata3, aes(x = climvar.value, y = PredictedProb)) + geom_ribbon(aes(ymin = LL,
    ymax = UL), alpha = 0.2) + 
  geom_line(linewidth = 1)+
  geom_point(aes(x=climvar.value, y=PredictedProb))+
  xlab("cmd")
