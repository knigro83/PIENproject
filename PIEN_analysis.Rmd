...
####make plot 
SR_TD_ht_mod_lm <- lm(log(value) ~  climvar.value + I(climvar.value^2), data=ht_SR_long %>% filter(climvar.name=="TD"))
...
SR_TD_plot<- ggplot(data=ht_SR_long %>% filter(climvar.name=="TD"), aes(x=climvar.value, y=value))+
  geom_point()+
  geom_smooth(method="lm")+
  ylab("log(Height)")+
...
SR_RH_plot<- ggplot(data=ht_SR_long %>% filter(climvar.name=="RH"), aes(x=climvar.value, y=value))+
  geom_point()+
  geom_smooth(method="lm")+
  ylab("log(Height)")+
  xlab("Provenance Relative Humidity")
...
SR_smrpb_plot <- ggplot(data=ht_SR_long %>% filter(climvar.name=="smrpb"), aes(x=climvar.value, y=value))+
  geom_point()+
  geom_smooth(method="lm")+
  ylab("log(Height)")+
...
ggarrange(SR_TD_plot, SR_RH_plot, SR_smrpb_plot, ncol=3)
...
##MAP
ht_map_quad <- lmer(log(value) ~ climvar.value + I(climvar.value^2) + (1|Provenance) + (1|BLK) + (1|year), data=ht_SR_long %>% filter(climvar.name=="MAP"))
plot(ht_map_quad)
qqmath(ht_map_quad)
summary(ht_map_quad)

ht_map_quad_lm <- lm(log(value) ~ climvar.value + I(climvar.value^2), data=ht_SR_long %>% filter(climvar.name=="MAP"))
...
##PPTwt
ht_PPTwt_quad <- lmer(log(value) ~ climvar.value + I(climvar.value^2) + (1|Provenance) + (1|BLK)+(1|year), data=ht_SR_long %>% filter(climvar.name=="PPTwt"))
plot(ht_PPTwt_quad)
qqmath(ht_PPTwt_quad)
summary(ht_PPTwt_quad)

ht_PPTwt_quad_lm <- lm(value ~ climvar.value + I(climvar.value^2), data=ht_SR_long %>% filter(climvar.name=="PPTwt"))
newdata_ht_PPTwt <- with(ht_SR_long %>% filter(climvar.name=="PPTwt"), data.frame(climvar.value=seq(min(climvar.value),max(climvar.value),length.out=100)))

ht_PPTwt_predict <- predict(ht_PPTwt_quad_lm, newdata = newdata_ht_PPTwt, interval="confidence")

predictdata_ht_PPTwt <- cbind(newdata_ht_PPTwt, ht_PPTwt_predict)

SR_PPTwt_plot<- ggplot()+
  geom_point(data=ht_SR_long %>% filter(climvar.name=="PPTwt"), aes(x=climvar.value, y=value, col=factor(Provenance, levels=provs_bylat$Provenance)))+
  geom_ribbon(data=predictdata_ht_PPTwt, aes(x=climvar.value, ymin=lwr, ymax=upr), fill="black", alpha=0.2)+
  geom_line(data=predictdata_ht_PPTwt, aes(x=climvar.value,y=fit), color="black")+
  xlab("PPTwt")+
  ylab("log(Height)")+
  scale_color_manual(values=unname(stepped()),name="Provenance")+
  theme_bw()
