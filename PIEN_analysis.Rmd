...
ggplot()+
  ...
  facet_wrap(~factor(Provenance, levels=c(provs_bylat$Provenance,"Plantation")))+
  theme_bw()+
...
bestmod_dbh_pca_x_nog <- lmer(DBH ~ scale(x) + (1|Provenance), data=dbh_num14_alldist_noG)
summary(bestmod_dbh_pca_x_nog) #null model was better than this though so not a strong effect. 

ggplot(dbh_num14_alldist_noG, aes(x=x, y=DBH))+
  geom_point()+
  geom_smooth(method="lm")
...
bestmod_dbh_pca_y <- lmer(DBH ~ PC2 + I(PC2^2) + scale(y) + I(scale(y)^2) + (1|Provenance), data=dbh_num14_alldist)
...
ggplot(data=ht_data_clim14, aes(x=PC2, y=value))+
  geom_point()
...
ggplot(ht_data_clim14, aes(x=y, y=value))+
  geom_point()
ggplot(ht_data_clim14, aes(x=PC2, y=value))+
  geom_point()
