...
bestmod_dbh_pca_y_randtime_lm <- lm(DBH ~ I(scale(y)^2) + PC2, data=dbh_numeric_time)

newdata_dbh_y <-  expand.grid(y=seq(min(dbh_numeric_time$y), max(dbh_numeric_time$y), length.out=50),
            PC2=mean(dbh_numeric_time$PC2))
dbh_predict_y <- predict(bestmod_dbh_pca_y_randtime_lm, newdata = newdata_dbh_y, interval="confidence")

predictdatadbh_y <- cbind(newdata_dbh_y, dbh_predict_y)

dbh_y_plot <- ggplot()+
  geom_point(data=dbh_numeric_time, aes(x=y, y=DBH, col=factor(Provenance, levels=provs_bylat$Provenance)))+
  geom_ribbon(data=predictdatadbh_y, aes(x=y, ymin=lwr, ymax=upr), fill="black", alpha=0.2)+
  geom_line(data=predictdatadbh_y, aes(x=y,y=fit), color="black")+
  xlab("Latitude")+
  scale_color_manual(values=unname(stepped()),name="Provenance")+
  facet_wrap(~year, scales="free")+
  geom_text(data= data.frame(x=32.99908, year=c(1991,1996,2006,2014), y=c(5,8,13,17)), aes(x=x, y=y, label="*"), size=10, col=unname(stepped(n=9))[1])+
  theme_bw()+
  theme(legend.position="none")
dbh_y_plot
...
totht_mod <- lmer(log(value) ~ Provenance*name + (1|BLK), data=ht_data)
plot(totht_mod)
qqmath(totht_mod)
Anova(totht_mod, type="3") ##there is a significant year by provenance interaction

#no gila
totht_mod_nog <- lmer(log(value) ~ Provenance*name + (1|BLK), data=ht_data %>% filter(!Provenance=="Gila NF"))
plot(totht_mod_nog)
