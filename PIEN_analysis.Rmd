...
ggplot(as.data.frame(clim_pca_big$x[-(1:5),]), aes(x=PC1, y=PC2))+
  geom_hex(alpha=0.7)+
  scale_fill_gradientn(colors=c("black","grey"))+
  geom_point(data=plant_pca_scores, aes(x=PC1, y=PC2, shape=period), col="gold", size=5)+
  geom_point(data=prov_pcapts_big, aes(x=PC1, y=PC2, color=factor(Provenance, levels=provs_bylat$Provenance)), size=4)+
  scale_color_manual(values=unname(stepped()), name="Provenance")+
  ggrepel::geom_text_repel(data=prov_pcapts_big, aes(label=code))+
  theme_bw()+
  theme(text = element_text(size=16, color="black"))


ggplot(as.data.frame(clim_pca_big$x[-(1:5),]), aes(x=PC1, y=PC3))+
  geom_hex(alpha=0.7)+
  scale_fill_gradientn(colors=c("black","grey"))+
  geom_point(data=plant_pca_scores, aes(x=PC1, y=PC3, shape=period), col="gold", size=5)+
  geom_point(data=prov_pcapts_big, aes(x=PC1, y=PC3, color=factor(Provenance, levels=provs_bylat$Provenance)), size=4)+
  scale_color_manual(values=unname(stepped()), name="Provenance")+
  ggrepel::geom_text_repel(data=prov_pcapts_big, aes(label=code))+
  theme_bw()+
  theme(text = element_text(size=16, color="black"))
