...
slpasp_plantation <- terra::extract(slpasp, adjusted_provs %>% filter(Provenance=="Plantation") %>% terra::project(slpasp), method="simple")
...
zoom(canada, e=ext(westernus), new=FALSE)

canada_crop <- crop(canada, ext(-126.92, -97.23, 41.6769, 52))

ggplot()+
  geom_spatvector(data=westernus, col="grey")+
  geom_spatvector(data=canada_crop, col="grey")+
  geom_spatvector(data=adjusted_provs %>% left_join(prov_names) %>% filter(!Provenance == "Plantation"), color="black", shape="\u2605", size=4)+
  geom_spatvector(data=adjusted_provs %>% left_join(prov_names) %>% filter(Provenance == "Plantation"), color="darkred", size=3)+
  geom_spatvector_text(data=adjusted_provs %>% left_join(prov_names)%>% filter(!Provenance == "Plantation"), aes(label=code), hjust=c(rep(c(-0.2,1),2),-.2,-.2,-.2,0,rep(c(-.2,1),5),1,1), vjust=c(rep(0,7),1.4,rep(0,12)))+
    geom_spatvector_text(data=adjusted_provs %>% left_join(prov_names)%>% filter(Provenance == "Plantation"), aes(label="Plantation"), hjust = 1, vjust=-0.6, col="darkred")+
  theme_bw()
...
exp(cbind(OR = coef(logmod_cat), confint(logmod_cat)))
...
ggplot(newdata3, aes(x = year, y = PredictedProb)) + geom_ribbon(aes(ymin = LL,
    ymax = UL, fill = Provenance), alpha = 0.2) + geom_line(aes(colour = Provenance),
    linewidth = 1)
...
newdata_pcdist_surv <- data.frame(
    dist.to.plant = seq(min(surv_pcadist_data$dist.to.plant), 
                      max(surv_pcadist_data$dist.to.plant), 
                      length.out = 50),
    geo.dist.to.plant = rep(seq(87,1593,100),each=50)
    )
...
##PCA models
resis_mod_pc <- lmer(log(resistance) ~ PC1*PC2 + I(PC1^2) + I(PC2^2) + (1|Provenance), data=RR4_clim)
plot(resis_mod_pc)
qqmath(resis_mod_pc)
summary(resis_mod_pc) 

resis_mod_pc_noG <- lmer(log(resistance) ~ PC1*PC2 + I(PC1^2) + I(PC2^2) + (1|Provenance), data=RR4_clim %>% filter(!Provenance=="Gila NF"))
plot(resis_mod_pc_noG)
qqmath(resis_mod_pc_noG)
summary(resis_mod_pc_noG) #nothing significant when Gila removed
...
```{r}
###resilience
resil_mod_pc <- lmer(resilience ~ PC1*PC2 + I(PC1^2) + I(PC2^2) + (1|Provenance), data=RR4_clim)
plot(resil_mod_pc)
qqmath(resil_mod_pc)
...
resil_mod_pc_noG <- lmer(resilience ~ PC1*PC2 + I(PC1^2) + I(PC2^2) + (1|Provenance), data=RR4_clim_noG)
plot(resil_mod_pc_noG)
qqmath(resil_mod_pc_noG)
summary(resil_mod_pc_noG)

dredge(resil_mod_pc) #PC1 is only term when Gila is included
