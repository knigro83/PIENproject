...
prov_clim_adjusted_bi_long<- prov_clim_adjusted_bi %>% 
  ...
  dplyr::select(c(Provenance,MAT:PPTsm, smrpb, monsoonality)) %>% 
...
dbh_growth_data<- dbh %>% 
  mutate(areagrowth96=(pi*(DBH96/2)^2)-(pi*(DBH91/2)^2), areagrowth06=(pi*(DBH06/2)^2)- (pi*(DBH96/2)^2), areagrowth14=(pi*(DBH14/2)^2)- (pi*(DBH06/2)^2)) %>% 
...
bestmod_reldbh_pca_y_randtime <- lmer(sqrt(value) ~  PC2 + scale(y)+ (1|Provenance) + (1|BLK) + (1|name), data=dbh_growth_clim)
...
bestmod_reldbh_pca_y_randtime_noG <- lmer(sqrt(value) ~  I(scale(y)^2)+ (1|Provenance) + (1|BLK) + (1|name), data=dbh_growth_clim_noG)
...
mod_reldbh_pca_x_randtime_dredge
...
mod_reldbh_pca_x_randtime_noG_dredge

bestmod_reldbh_pca_x_randtime_noG <- lmer(sqrt(value) ~ PC2 + scale(x) + (1|Provenance) + (1|BLK) + (1|name), data=dbh_growth_clim_noG)
summary(bestmod_reldbh_pca_x_randtime_noG) ##Null model was better fit than this one so I don't think these effects are very strong. 
...
bestmod_reldbh_pca_y_randtime_noG_lm <- lm(sqrt(value) ~ I(scale(y)^2), data=dbh_growth_clim_noG)
...
reldbh_y_plot <- ggplot()+
  ...
  geom_rug(data=dbh_growth_clim_noG %>% mutate(fit=49), aes(x=y,y=fit), linewidth=1, col="black", sides="b")+
  xlab("Provenance Latitude")+
  ylab("Basal Area Growth (cm^2)")+
  facet_grid(~var)+
  theme_bw()+
  theme(legend.position="none")
reldbh_y_plot
...
dbh_growth_rel_clim_long <- dbh_growth_data_rel %>% 
  dplyr::rename(year=name, relDBH=value) %>% 
  pivot_longer(c(AHM:SHM, smrpb, monsoonality)) %>% 
  filter(!is.infinite(relDBH))


reldbh_mods<- data.frame()
dbh_growth_rel_clim_long_nog <- dbh_growth_rel_clim_long %>% filter(!Provenance=="Gila NF")
for(i in 1:length(provenance_climvars)){
mod <- lmer(relDBH ~ value + (1|Provenance)+ (1|BLK) + (1|year), data=dbh_growth_rel_clim_long %>% filter(name==provenance_climvars[i]))
pvalue<- as.data.frame(summary(mod)$coefficients)[2,]$`Pr(>|t|)`

mod_nog <- lmer(relDBH ~ value + (1|Provenance)+ (1|BLK) + (1|year), data=dbh_growth_rel_clim_long_nog %>% filter(name==provenance_climvars[i]))
pvalue_nog<- as.data.frame(summary(mod_nog)$coefficients)[2,]$`Pr(>|t|)`
...
reldbh_mods %>% arrange(pvalue_nog)

######just 2014 #######
reldbh_mods14<- data.frame()
for(i in 1:length(provenance_climvars)){
mod <- lmer(relDBH ~ value + (1|Provenance)+ (1|BLK), data=dbh_growth_rel_clim_long %>% filter(name==provenance_climvars[i]) %>% filter(year=="relareagrowth14"))
pvalue<- as.data.frame(summary(mod)$coefficients)[2,]$`Pr(>|t|)`

mod_nog <- lmer(relDBH ~ value + (1|Provenance)+ (1|BLK), data=dbh_growth_rel_clim_long_nog %>% filter(name==provenance_climvars[i])  %>% filter(year=="relareagrowth14") )
pvalue_nog<- as.data.frame(summary(mod_nog)$coefficients)[2,]$`Pr(>|t|)`

reldbh_mods14 <- bind_rows(reldbh_mods14, data.frame(var=provenance_climvars[i], pvalue = pvalue, pvalue_nog = pvalue_nog))

}

reldbh_mods14 %>% arrange(pvalue_nog)

summary(lmer(relDBH ~ value + (1|Provenance)+ (1|BLK), data=dbh_growth_rel_clim_long %>% filter(name=="smrpb") %>% filter(year=="relareagrowth14") ))
...
for(i in 1:length(provenance_climvars)){
mod <- lmer(relDBH ~ value + I(value^2) + (1|Provenance) + (1|BLK) + (1|year), data=dbh_growth_rel_clim_long %>% filter(name==provenance_climvars[i]))
pvalues<- as.data.frame(summary(mod)$coefficients)[2:3,]$`Pr(>|t|)`

mod_nog <- lmer(relDBH ~ value + I(value^2) + (1|Provenance) + (1|BLK) + (1|year), data=dbh_growth_rel_clim_long_nog %>% filter(name==provenance_climvars[i]))
pvalues_nog<- as.data.frame(summary(mod_nog)$coefficients)[2:3,]$`Pr(>|t|)`
...
SR_dbh_TD_plot <- ggplot(data=dbh_SR_long %>% filter(climvar.name=="TD"), aes(x=climvar.value, y=DBH))+
  geom_point()+
  geom_smooth(method="lm")

ggplot(data=dbh_SR_long %>% filter(climvar.name=="smrpb"), aes(x=climvar.value, y=DBH))+
  geom_point()+
  geom_smooth(method="lm")
...
ggplot()+
  geom_point(data=dbh_SR_long %>% filter(climvar.name=="PPTwt"), aes(x=climvar.value, y=DBH, col=factor(Provenance, levels=provs_bylat$Provenance)))+
...
##MAT
NR_relhtMAT_quad <- lmer(relht ~ value + I(value^2) + (1|Provenance) + (1|BLK) + (1|year), data=relht_NR_long %>% filter(name=="MAT"))
plot(NR_relhtMAT_quad)
qqmath(NR_relhtMAT_quad)
summary(NR_relhtMAT_quad)

NR_relhtMAT_quad_lm <- lm(relht ~ value + I(value^2), data=relht_NR_long %>% filter(name=="MAT"))
newdata_NR_relhtMAT <- with(relht_NR_long %>% filter(name=="MAT"), data.frame(value=seq(min(value),max(value),length.out=100)))

NR_relhtMAT_predict <- predict(NR_relhtMAT_quad_lm, newdata = newdata_NR_relhtMAT, interval="confidence")

predictdata_NR_relhtMAT <- cbind(newdata_NR_relhtMAT, NR_relhtMAT_predict)

NR_MAT_plot<- ggplot()+
  geom_ribbon(data=predictdata_NR_relhtMAT, aes(x=value, ymin=lwr, ymax=upr), fill="black", alpha=0.2)+
  geom_line(data=predictdata_NR_relhtMAT, aes(x=value,y=fit), color="black")+
  xlab("MAT")+
  ylab("Relative Height Growth Rate (%/year)")+
...
##Tavewt
NR_relhtTavewt_quad <- lmer(relht ~ value + I(value^2) + (1|Provenance) + (1|BLK) + (1|year), data=relht_NR_long %>% filter(name=="Tavewt"))
plot(NR_relhtTavewt_quad)
qqmath(NR_relhtTavewt_quad)
summary(NR_relhtTavewt_quad)

NR_relhtTavewt_quad_lm <- lm(relht ~ value + I(value^2), data=relht_NR_long %>% filter(name=="Tavewt"))
newdata_NR_relhtTavewt <- with(relht_NR_long %>% filter(name=="Tavewt"), data.frame(value=seq(min(value),max(value),length.out=100)))

NR_relhtTavewt_predict <- predict(NR_relhtTavewt_quad_lm, newdata = newdata_NR_relhtTavewt, interval="confidence")

predictdata_NR_relhtTavewt <- cbind(newdata_NR_relhtTavewt, NR_relhtTavewt_predict)

NR_Tavewt_plot<- ggplot()+
  geom_ribbon(data=predictdata_NR_relhtTavewt, aes(x=value, ymin=lwr, ymax=upr), fill="black", alpha=0.2)+
  geom_line(data=predictdata_NR_relhtTavewt, aes(x=value,y=fit), color="black")+
  xlab("Tavewt")+
  ylab("Relative Height Growth Rate (%/year)")+
  geom_rug(data=relht_NR_long %>% filter(name=="Tavewt")%>% mutate(fit=10), aes(x=value,y=fit), linewidth=1, col="black", sides="b")+
  theme_bw()
NR_Tavewt_plot

##DD0
NR_relhtDD0_quad <- lmer(relht ~ value + I(value^2) + (1|Provenance) + (1|BLK) + (1|year), data=relht_NR_long %>% filter(name=="DD0"))
plot(NR_relhtDD0_quad)
qqmath(NR_relhtDD0_quad)
summary(NR_relhtDD0_quad)

NR_relhtDD0_quad_lm <- lm(relht ~ value + I(value^2), data=relht_NR_long %>% filter(name=="DD0"))
newdata_NR_relhtDD0 <- with(relht_NR_long %>% filter(name=="DD0"), data.frame(value=seq(min(value),max(value),length.out=100)))

NR_relhtDD0_predict <- predict(NR_relhtDD0_quad_lm, newdata = newdata_NR_relhtDD0, interval="confidence")

predictdata_NR_relhtDD0 <- cbind(newdata_NR_relhtDD0, NR_relhtDD0_predict)

NR_DD0_plot<- ggplot()+
  geom_ribbon(data=predictdata_NR_relhtDD0, aes(x=value, ymin=lwr, ymax=upr), fill="black", alpha=0.2)+
  geom_line(data=predictdata_NR_relhtDD0, aes(x=value,y=fit), color="black")+
  xlab("DD0")+
  ylab("Relative Height Growth Rate (%/year)")+
  geom_rug(data=relht_NR_long %>% filter(name=="DD0")%>% mutate(fit=10), aes(x=value,y=fit), linewidth=1, col="black", sides="b")+
  theme_bw()
NR_DD0_plot

##DD18
NR_relhtDD18_quad <- lmer(relht ~ value + I(value^2) + (1|Provenance) + (1|BLK) + (1|year), data=relht_NR_long %>% filter(name=="DD18"))
plot(NR_relhtDD18_quad)
qqmath(NR_relhtDD18_quad)
summary(NR_relhtDD18_quad)

NR_relhtDD18_quad_lm <- lm(relht ~ value + I(value^2), data=relht_NR_long %>% filter(name=="DD18"))
newdata_NR_relhtDD18 <- with(relht_NR_long %>% filter(name=="DD18"), data.frame(value=seq(min(value),max(value),length.out=100)))

NR_relhtDD18_predict <- predict(NR_relhtDD18_quad_lm, newdata = newdata_NR_relhtDD18, interval="confidence")

predictdata_NR_relhtDD18 <- cbind(newdata_NR_relhtDD18, NR_relhtDD18_predict)

NR_DD18_plot<- ggplot()+
  geom_ribbon(data=predictdata_NR_relhtDD18, aes(x=value, ymin=lwr, ymax=upr), fill="black", alpha=0.2)+
  geom_line(data=predictdata_NR_relhtDD18, aes(x=value,y=fit), color="black")+
  xlab("DD18")+
  ylab("Relative Height Growth Rate (%/year)")+
  geom_rug(data=relht_NR_long %>% filter(name=="DD18")%>% mutate(fit=10), aes(x=value,y=fit), linewidth=1, col="black", sides="b")+
  theme_bw()
NR_DD18_plot
```
...
bestNR_mod_ht_pca_y_randtime <- lmer(value ~ y + (1|Provenance) + (1|BLK) + (1|year), data=NR_ht_data_year_clim)
summary(bestNR_mod_ht_pca_y_randtime) #negative effect of latitude
...
AICc(bestNR_mod_ht_pca_x_randtime,bestNR_mod_ht_pca_y_randtime) ##null is best
