...
mod_nog <- glmmTMB(cbind(alive,died) ~ climvar.value + (1|Provenance)+ (1|BLK) + (1|year), data=SR_moniger_survival_long %>% filter(climvar.name==provenance_climvars[i]) %>% filter(!Provenance=="Gila NF"), family=binomial)
pvalue_nog<- as.data.frame(summary(mod_nog)$coefficients$cond)[2,]$`Pr(>|z|)`

SR_surv_mods <- bind_rows(SR_surv_mods, data.frame(var=provenance_climvars[i], pvalue = pvalue, pvalue_nog = pvalue_nog))}

SR_surv_mods %>% arrange(pvalue_nog) ##length of growing season seems to be influential both with and without Gila, also smrpb
...
SR_surveFFP_newdata2 <- cbind(SR_surveFFP_newdata1, predict(SR_surv_eFFP_mod, newdata = SR_surveFFP_newdata1, type = "link", se=TRUE))
...
ggplot(SR_surveFFP_newdata3, aes(x = climvar.value, y = PredictedProb)) + geom_ribbon(aes(ymin = LL,
    ymax = UL), alpha = 0.2) + 
  geom_line(linewidth = 1)+
  geom_point(aes(x=climvar.value, y=PredictedProb))+
  xlab("eFFP")
...
#smrpb
surv_smrpb_mod <- glmmTMB(cbind(alive,died) ~ scale(climvar.value) + (1|Provenance)+ (1|BLK) + (1|year), data=SR_moniger_survival_long %>% filter(climvar.name=="smrpb"), family="binomial")
summary(surv_smrpb_mod)

surv_smrpb_data <- SR_moniger_survival_long %>% filter(climvar.name=="smrpb")
...
surv_smrpb_mod_glm <- glmmTMB(cbind(alive,died) ~ scale(climvar.value), data=SR_moniger_survival_long %>% filter(climvar.name=="smrpb"), family="binomial")
...
#without Gila is still significant
surv_smrpb_mod_nog <- glmmTMB(cbind(alive,died) ~ scale(climvar.value) + (1|Provenance)+(1|BLK) + (1|year), data=SR_moniger_survival_long %>% filter(climvar.name=="smrpb") %>% filter(!Provenance=="Gila NF"), family="binomial")
summary(surv_smrpb_mod_nog)

##predictions
surv_smrpb_data_nog <- SR_moniger_survival_long %>% filter(climvar.name=="smrpb") %>% filter(!Provenance=="Gila NF")
...
mod_nog <- glmmTMB(cbind(alive,died) ~ scale(climvar.value) + I(scale(climvar.value)^2) + (1|Provenance) + (1|BLK) + (1|year), data=SR_moniger_survival_long %>% filter(climvar.name==provenance_climvars[i]) %>% filter(!Provenance=="Gila NF"), family="binomial")
pvalues_nog<- as.data.frame(summary(mod_nog)$coefficients$cond)[2:3,]$`Pr(>|z|)`
...
##Eref
surv_Eref_mod <- glmmTMB(cbind(alive,died) ~ scale(climvar.value) + I(scale(climvar.value)^2) + (1|Provenance) + (1|BLK) + (1|year), data=SR_moniger_survival_long %>% filter(climvar.name=="Eref"), family="binomial")
summary(surv_Eref_mod)

surv_Eref_data <- SR_moniger_survival_long %>% filter(climvar.name=="Eref")

##predictions
survEref_newdata1 <- expand.grid(climvar.value = seq(min(surv_Eref_data$climvar.value),max(surv_Eref_data$climvar.value),length.out=50))

surv_Eref_mod_glm <- glmmTMB(cbind(alive,died) ~ scale(climvar.value) + I(scale(climvar.value)^2), data=SR_moniger_survival_long %>% filter(climvar.name=="Eref"), family="binomial")
...
#without Gila is still significant
surv_Eref_mod_nog <- glmmTMB(cbind(alive,died) ~ scale(climvar.value) + I(scale(climvar.value)^2) + (1|Provenance) + (1|BLK) + (1|year), data=SR_moniger_survival_long %>% filter(climvar.name=="Eref") %>% filter(!Provenance=="Gila NF"), family="binomial")
summary(surv_Eref_mod_nog)
#########

##cmd
surv_cmd_mod <- glmmTMB(cbind(alive,died) ~ scale(climvar.value) + I(scale(climvar.value)^2) + (1|Provenance) + (1|BLK) + (1|year), data=SR_moniger_survival_long %>% filter(climvar.name=="CMD"), family="binomial")
summary(surv_cmd_mod)

surv_cmd_data <- SR_moniger_survival_long %>% filter(climvar.name=="CMD")

##predictions
survcmd_newdata1 <- expand.grid(climvar.value = seq(min(surv_cmd_data$climvar.value),max(surv_cmd_data$climvar.value),length.out=50))

surv_cmd_mod_glm <- glmmTMB(cbind(alive,died) ~ scale(climvar.value) + I(scale(climvar.value)^2), data=SR_moniger_survival_long %>% filter(climvar.name=="CMD"), family="binomial")
...
#without Gila is still significant
surv_cmd_mod_nog <- glmmTMB(cbind(alive,died) ~ scale(climvar.value) + I(scale(climvar.value)^2) + (1|Provenance) + (1|BLK) + (1|year), data=SR_moniger_survival_long %>% filter(climvar.name=="CMD") %>% filter(!Provenance=="Gila NF"), family="binomial")
summary(surv_cmd_mod_nog)
```
