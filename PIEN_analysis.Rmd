...
ggplot(gini_byprov, aes(x=Provenance, y=gini))+
  geom_boxplot()+
  theme(axis.text.x = element_text(angle=45, hjust=1))


gini_df_raw = data.frame()
for(i in 1:length(trees)){
gini = dbh_age_good %>% filter(year > 1990 & year < 2011) %>% filter(tree==trees[i]) %>% pull(rw) %>% 
  Gini(weights=NULL, unbiased = TRUE, conf.level=TRUE, R=10000, type="perc", na.rm=TRUE)
gini_df_raw = bind_rows(gini_df_raw, data.frame(gini=gini[1], lwr = gini[2], upr=gini[3], tree=trees[i]))
}

gini_df_raw


gini_raw_byprov<- gini_df_raw %>% 
  left_join(dbh_age_summary, by="tree")

ggplot(gini_raw_byprov, aes(x=Provenance, y=gini))+
  geom_boxplot()+
  theme(axis.text.x = element_text(angle=45, hjust=1))
...
chron.growth.detrended.all <- chron(growth.detrended.all)
...
spei_cwd_seascorr <- seascorr(chrono = chron.growth.detrended.all,
                         climate= list(spei= spei_date_wide, cwd = cwd_date_wide),
                         timespan = c(1971,2015),
                         complete= 9,
                         season_lengths=seq(4,12,1),
                         primary=1,
                         secondary=2,
                         ci=0.05)
plot(spei_cwd_seascorr)


spei_aet_seascorr <- seascorr(chrono = chron.growth.detrended.all,
                         climate= list(spei= spei_date_wide, aet = aet_date_wide),
                         timespan = c(1971,2015),
                         complete= 9,
                         season_lengths=seq(4,12,1),
                         primary=1,
                         secondary=2,
                         ci=0.05)
plot(spei_aet_seascorr)

aet_cwd_seascorr <- seascorr(chrono = chron.growth.detrended.all,
                         ...
                         secondary=2,
                         ci=0.05)
plot(aet_cwd_seascorr)

aet_spei_seascorr <- seascorr(chrono = chron.growth.detrended.all,
                         ...
                         secondary=2,
                         ci=0.05)
plot(aet_spei_seascorr)


cwd_spei_seascorr <- seascorr(chrono = chron.growth.detrended.all,
                         climate= list(cwd= cwd_date_wide, spei = spei_date_wide),
                         timespan = c(1971,2015),
                         complete= 9,
                         season_lengths=seq(4,12,1),
                         primary=1,
                         secondary=2,
                         ci=0.05)
plot(cwd_spei_seascorr)

cwd_aet_seascorr <- seascorr(chrono = chron.growth.detrended.all,
                         climate= list(cwd= cwd_date_wide, aet = aet_date_wide),
                         timespan = c(1971,2015),
                         complete= 9,
                         season_lengths=seq(4,12,1),
                         primary=1,
                         secondary=2,
                         ci=0.05)
plot(cwd_aet_seascorr)


###do for all provenances separately
...
result <- seascorr(chrono = chron.list[[i]],
                         ...
                         secondary=2,
                         ci=0.05)
spei_cwd_list[[i]]<- result
}

inner_result_spei_cwd<- list()
outer_result_spei_cwd<- list()

for(i in 1:length(provs_arranged_lat[-9])){
for(j in 1:length(seq(4,12,1))){
result<- bind_rows(spei_cwd_list[[i]]$coef[[j]]$primary %>% bind_cols(data.frame(month=c("Aug","Sep","Oct","Nov","Dec","JAN","FEB","MAR","APR","MAY","JUN","JUL","AUG","SEP"), var="primary", seasonl = seq(4,12,1)[j])) %>% 
  filter(significant==TRUE),
spei_cwd_list[[i]]$coef[[j]]$secondary %>% 
bind_cols(data.frame(month=c("Aug","Sep","Oct","Nov","Dec","JAN","FEB","MAR","APR","MAY","JUN","JUL","AUG","SEP"), var = "secondary", seasonl = seq(4,12,1)[j])) %>% 
  filter(significant==TRUE) 
)
 inner_result_spei_cwd[[j]]<- result}

  inner_result_spei_cwd2 <- inner_result_spei_cwd[sapply(inner_result_spei_cwd, function(x) dim(x)[1])>0]
  inner_result_spei_cwd3 <- bind_rows(inner_result_spei_cwd2)
  outer_result_spei_cwd[[i]]<- inner_result_spei_cwd3}
names(outer_result_spei_cwd)<- provs_arranged_lat[-9]

spei_cwd_seas <- bind_rows(outer_result_spei_cwd, .id="Provenance") 

spei_cwd_seas_allp <- spei_cwd_seas%>% 
  bind_rows(data.frame(Provenance = data.frame(Provenance=provs_arranged_lat[-9]) %>% filter(!Provenance %in% spei_cwd_seas$Provenance),coef=0,significant=TRUE,month="AUG",var="primary",seasonl=4))

ggplot(spei_cwd_seas_allp, aes(x=factor(month, levels=c("Aug","Sep","Oct","Nov","Dec","JAN","FEB","MAR","APR","MAY","JUN","JUL","AUG","SEP")), y=coef, fill=factor(Provenance, levels=provs_arranged_lat[-9])))+
  geom_bar(stat="identity", position = position_dodge())+
  facet_grid(rows=vars(factor(var, levels=c("primary","secondary"),labels=c("pri_spei","sec_cwd"))),cols=vars(seasonl), )+
  scale_fill_manual(values=unname(stepped()), name="Provenance")+
  theme_bw()+
  ggtitle("spei, cwd")+
  theme(axis.text.x = element_text(angle=45, hjust=1))
...
result <- seascorr(chrono = chron.list[[i]],
                         ...
                         secondary=2,
                         ci=0.05)
spei_aet_list[[i]]<- result
}

inner_result_spei_aet<- list()
outer_result_spei_aet<- list()

for(i in 1:length(provs_arranged_lat[-9])){
for(j in 1:length(seq(4,12,1))){
result<- bind_rows(spei_aet_list[[i]]$coef[[j]]$primary %>% bind_cols(data.frame(month=c("Aug","Sep","Oct","Nov","Dec","JAN","FEB","MAR","APR","MAY","JUN","JUL","AUG","SEP"), var="primary", seasonl = seq(4,12,1)[j])) %>% 
  filter(significant==TRUE),
spei_aet_list[[i]]$coef[[j]]$secondary %>% 
bind_cols(data.frame(month=c("Aug","Sep","Oct","Nov","Dec","JAN","FEB","MAR","APR","MAY","JUN","JUL","AUG","SEP"), var = "secondary", seasonl = seq(4,12,1)[j])) %>% 
  filter(significant==TRUE) 
)
 inner_result_spei_aet[[j]]<- result}

  inner_result_spei_aet2 <- inner_result_spei_aet[sapply(inner_result_spei_aet, function(x) dim(x)[1])>0]
  inner_result_spei_aet3 <- bind_rows(inner_result_spei_aet2)
  outer_result_spei_aet[[i]]<- inner_result_spei_aet3}
names(outer_result_spei_aet)<- provs_arranged_lat[-9]

spei_aet_seas <- bind_rows(outer_result_spei_aet, .id="Provenance") 

spei_aet_seas_allp <- spei_aet_seas%>% 
  bind_rows(data.frame(Provenance = data.frame(Provenance=provs_arranged_lat[-9]) %>% filter(!Provenance %in% spei_aet_seas$Provenance),coef=0,significant=TRUE,month="AUG",var="primary",seasonl=4))

ggplot(spei_aet_seas_allp, aes(x=factor(month, levels=c("Aug","Sep","Oct","Nov","Dec","JAN","FEB","MAR","APR","MAY","JUN","JUL","AUG","SEP")), y=coef, fill=factor(Provenance, levels=provs_arranged_lat[-9])))+
  geom_bar(stat="identity", position = position_dodge())+
  facet_grid(rows=vars(factor(var, levels=c("primary","secondary"),labels=c("pri_spei","sec_aet"))),cols=vars(seasonl), )+
  scale_fill_manual(values=unname(stepped()), name="Provenance")+
  theme_bw()+
  ggtitle("spei, aet")+
  theme(axis.text.x = element_text(angle=45, hjust=1))
...
result <- seascorr(chrono = chron.list[[i]],
                         ...
                         secondary=2,
                         ci=0.05)
cwd_aet_list[[i]]<- result
}

inner_result_cwd_aet<- list()
outer_result_cwd_aet<- list()

for(i in 1:length(provs_arranged_lat[-9])){
for(j in 1:length(seq(4,12,1))){
result<- bind_rows(cwd_aet_list[[i]]$coef[[j]]$primary %>% bind_cols(data.frame(month=c("Aug","Sep","Oct","Nov","Dec","JAN","FEB","MAR","APR","MAY","JUN","JUL","AUG","SEP"), var="primary", seasonl = seq(4,12,1)[j])) %>% 
  filter(significant==TRUE),
cwd_aet_list[[i]]$coef[[j]]$secondary %>% 
bind_cols(data.frame(month=c("Aug","Sep","Oct","Nov","Dec","JAN","FEB","MAR","APR","MAY","JUN","JUL","AUG","SEP"), var = "secondary", seasonl = seq(4,12,1)[j])) %>% 
  filter(significant==TRUE) 
)
 inner_result_cwd_aet[[j]]<- result}

  inner_result_cwd_aet2 <- inner_result_cwd_aet[sapply(inner_result_cwd_aet, function(x) dim(x)[1])>0]
  inner_result_cwd_aet3 <- bind_rows(inner_result_cwd_aet2)
  outer_result_cwd_aet[[i]]<- inner_result_cwd_aet3}
names(outer_result_cwd_aet)<- provs_arranged_lat[-9]

cwd_aet_seas <- bind_rows(outer_result_cwd_aet, .id="Provenance") 

cwd_aet_seas_allp <- cwd_aet_seas%>% 
  bind_rows(data.frame(Provenance = data.frame(Provenance=provs_arranged_lat[-9]) %>% filter(!Provenance %in% cwd_aet_seas$Provenance),coef=0,significant=TRUE,month="AUG",var="primary",seasonl=4))

ggplot(cwd_aet_seas_allp, aes(x=factor(month, levels=c("Aug","Sep","Oct","Nov","Dec","JAN","FEB","MAR","APR","MAY","JUN","JUL","AUG","SEP")), y=coef, fill=factor(Provenance, levels=provs_arranged_lat[-9])))+
  geom_bar(stat="identity", position = position_dodge())+
  facet_grid(rows=vars(factor(var, levels=c("primary","secondary"),labels=c("pri_cwd","sec_aet"))),cols=vars(seasonl), )+
  scale_fill_manual(values=unname(stepped()), name="Provenance")+
  theme_bw()+
  ggtitle("cwd, aet")+
  theme(axis.text.x = element_text(angle=45, hjust=1))
...
result <- seascorr(chrono = chron.list[[i]],
                         ...
                         secondary=2,
                         ci=0.05)
cwd_spei_list[[i]]<- result
}

inner_result_cwd_spei<- list()
outer_result_cwd_spei<- list()

for(i in 1:length(provs_arranged_lat[-9])){
for(j in 1:length(seq(4,12,1))){
result<- bind_rows(cwd_spei_list[[i]]$coef[[j]]$primary %>% bind_cols(data.frame(month=c("Aug","Sep","Oct","Nov","Dec","JAN","FEB","MAR","APR","MAY","JUN","JUL","AUG","SEP"), var="primary", seasonl = seq(4,12,1)[j])) %>% 
  filter(significant==TRUE),
cwd_spei_list[[i]]$coef[[j]]$secondary %>% 
bind_cols(data.frame(month=c("Aug","Sep","Oct","Nov","Dec","JAN","FEB","MAR","APR","MAY","JUN","JUL","AUG","SEP"), var = "secondary", seasonl = seq(4,12,1)[j])) %>% 
  filter(significant==TRUE) 
)
 inner_result_cwd_spei[[j]]<- result}

  inner_result_cwd_spei2 <- inner_result_cwd_spei[sapply(inner_result_cwd_spei, function(x) dim(x)[1])>0]
  inner_result_cwd_spei3 <- bind_rows(inner_result_cwd_spei2)
  outer_result_cwd_spei[[i]]<- inner_result_cwd_spei3}
names(outer_result_cwd_spei)<- provs_arranged_lat[-9]

cwd_spei_seas <- bind_rows(outer_result_cwd_spei, .id="Provenance") 

cwd_spei_seas_allp <- cwd_spei_seas%>% 
  bind_rows(data.frame(Provenance = data.frame(Provenance=provs_arranged_lat[-9]) %>% filter(!Provenance %in% cwd_spei_seas$Provenance),coef=0,significant=TRUE,month="AUG",var="primary",seasonl=4))

ggplot(cwd_spei_seas_allp, aes(x=factor(month, levels=c("Aug","Sep","Oct","Nov","Dec","JAN","FEB","MAR","APR","MAY","JUN","JUL","AUG","SEP")), y=coef, fill=factor(Provenance, levels=provs_arranged_lat[-9])))+
  geom_bar(stat="identity", position = position_dodge())+
  facet_grid(rows=vars(factor(var, levels=c("primary","secondary"),labels=c("pri_cwd","sec_spei"))),cols=vars(seasonl), )+
  scale_fill_manual(values=unname(stepped()), name="Provenance")+
  theme_bw()+
  ggtitle("cwd, spei")+
  theme(axis.text.x = element_text(angle=45, hjust=1))
