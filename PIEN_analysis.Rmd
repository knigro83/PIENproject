...
bestmod_cone_clim_hty <- glmmTMB(cbind(n_cones, total-n_cones) ~ PC1 + I(PC1^2) + scale(height) + (1|code) + (1|year) + (1|BLK), data=moniger_subset, family=binomial)
...
AICc(best.mod.cone.clim, bestmod_cone_clim_hty)
...
summary(cone_clim_mod_hty_nog)

# dredge.cone.clim.nog.y <- dredge(cone_clim_mod_hty_nog)
# saveRDS(dredge.cone.clim.nog.y, "dredge.cone.clim.nog.y.RDS")
dredge.cone.clim.nog.y <- readRDS("dredge.cone.clim.nog.y.RDS")
dredge.cone.clim.nog.y

best.mod.cone.clim_y_nog <- glmmTMB(cbind(n_cones, total-n_cones) ~ scale(height) + scale(y) + I(scale(y)^2) + (1|code) + (1|year) + (1|BLK), data= moniger_subset %>% filter(!code=="GiNF"), family = binomial)
summary(best.mod.cone.clim_y_nog)

AICc(cone_clim_mod_htx_nog, cone_clim_mod_hty_nog)
AICc(best.mod.cone.clim_y_nog, best.mod.cone.clim_nog) ##long model is better

##predictions - no gila 
CCnewdata1 <- expand.grid(height=seq(min(moniger_subset %>% filter(!code=="GiNF") %>% pull(height)),max(moniger_subset %>% filter(!code=="GiNF") %>% pull(height)),1),
                          x=seq(min(moniger_subset %>% filter(!code=="GiNF") %>% pull(x)),max(moniger_subset %>% filter(!code=="GiNF") %>% pull(x)),1))


cone_clim_mod_glm <- glmmTMB(cbind(n_cones, total-n_cones) ~ scale(height) + scale(x), data= moniger_subset %>% filter(!code=="GiNF"), family = binomial)
...
ggplot(CCnewdata3, aes(x = height, y = PredictedProb, col= factor(x))) + 
  geom_ribbon(aes(ymin = LL,
    ymax = UL, fill = factor(x)), alpha = 0.2, col=NA) + geom_line(aes(colour = factor(x)),
    linewidth = 1)+
  geom_line(aes(x=height, y=PredictedProb, col=factor(x)))+
  scale_color_manual(values=viridis::mako(n=17), name = "x")+
  scale_fill_manual(values=viridis::mako(n=17),name="x")+
  theme_bw()+
  xlab("Height (m)")+
  ylab("Predicted Probability of Cones")+
  theme(legend.position = "bottom")
...
newdata1 <- expand.grid(y = seq(min(moniger_survival_nog$y),max(moniger_survival_nog$y),1),
                        BLK= "C",
                        Provenance="Pike NF")
...
ggplot(data=newdata3_nog, aes(x = y, y = PredictedProb)) + 
  geom_line()+
...
survFFP_newdata2 <- cbind(survFFP_newdata1, predict(surv_FFP_mod, newdata = survFFP_newdata1, type = "link", se=TRUE))
