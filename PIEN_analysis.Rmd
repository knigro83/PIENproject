...
for(i in 1:length(climvars)){
    ...
    Mr2.lmer <- performance::r2(mod)$R2_marginal
       ...
       if(!is.na(Cr2.lmer)) (
lmerdata_pcs <- bind_rows(lmerdata_pcs, data.frame(climvar=climvars[i], pc1value.lmer = pc1pvalue.lmer, pc2value.lmer = pc2pvalue.lmer, pc1pc2value.lmer = pc1pc2pvalue.lmer, r2.lmer=Cr2.lmer, r2type="C"))  ) 

    if(is.na(Cr2.lmer)) (
lmerdata_pcs <- bind_rows(lmerdata_pcs, data.frame(climvar=climvars[i], pc1value.lmer = pc1pvalue.lmer, pc2value.lmer = pc2pvalue.lmer, pc1pc2value.lmer = pc1pc2pvalue.lmer, r2.lmer=Mr2.lmer, r2type="M")) 
 ) 
...
dbhmods <- bind_rows(dbhmods, data.frame(climvar = climvars[i], pvalue = as.data.frame(summary(mod)$coefficients)[2,] %>% pull(`Pr(>|t|)`), r2=performance::r2(mod)$R2_))}
...
for(i in 1:length(climvars)){
  for(j in 1:length(names)){
    ...
    if(!is.na(Cr2.lmer)) (
lmerdata_SR <- bind_rows(lmerdata_SR, data.frame(climvar=climvars[i], name=names[j], pvalue.lmer = pvalue.lmer, r2.lmer=Cr2.lmer, r2type="C")) )
    
    if(is.na(Cr2.lmer)) (
lmerdata_SR <- bind_rows(lmerdata_SR, data.frame(climvar=climvars[i], name=names[j], pvalue.lmer = pvalue.lmer, r2.lmer=Mr2.lmer, r2type="M")) )
...
for(i in 1:length(climvars)){
    ...
    if(!is.na(Cr2.lmer)) (
lmerdata_pcs_SR <- bind_rows(lmerdata_pcs_SR, data.frame(climvar=climvars[i], pc1value.lmer = pc1pvalue.lmer, pc2value.lmer = pc2pvalue.lmer, pc1pc2value.lmer = pc1pc2pvalue.lmer, r2.lmer=Cr2.lmer, r2type="C")) ) 

    if(is.na(Cr2.lmer)) (
lmerdata_pcs_SR <- bind_rows(lmerdata_pcs_SR, data.frame(climvar=climvars[i], pc1value.lmer = pc1pvalue.lmer, pc2value.lmer = pc2pvalue.lmer, pc1pc2value.lmer = pc1pc2pvalue.lmer, r2.lmer=Mr2.lmer, r2type="M")) ) 
...
pdf("figures/SR_pyear_trends.pdf", width=11, height=11)
SR_pyear_trends
dev.off()
##########individual model results

##current year - pick most representative variables to display

SR_corplots5 <- ggarrange(
ggplot()+
    geom_hline(yintercept=0, col="black")+
  scale_color_manual(values=unname(stepped()), name="Provenance")+
  geom_point(data=left_join(lmerdata_SR, cor_provclim_long_SR, by=c("climvar","name")) %>% filter(climvar =="day_of_first_fall_frost" & name=="PPTsm")  , aes(x=value, y=cor, col=factor(Provenance, levels=provs_bylat$Provenance)))+
  geom_smooth(data= left_join(lmerdata_SR, cor_provclim_long_SR, by=c("climvar","name")) %>% filter(climvar =="day_of_first_fall_frost" & name=="PPTsm"), aes(x=value, y=cor), method="lm", col="darkgrey")+
  xlab("Provenance Summer Precip (mm)")+ ylab("Correlation")+
  ggtitle("A) Day of first fall frost X RWI")+
  theme_bw(),

ggplot()+
    geom_hline(yintercept=0, col="black")+
  scale_color_manual(values=unname(stepped()), name="Provenance")+
  geom_point(data=left_join(lmerdata_SR, cor_provclim_long_SR, by=c("climvar","name")) %>% filter(climvar =="day_of_last_spring_frost" & name=="bFFP")  , aes(x=value, y=cor, col=factor(Provenance, levels=provs_bylat$Provenance)))+
  geom_smooth(data= left_join(lmerdata_SR, cor_provclim_long_SR, by=c("climvar","name")) %>% filter(climvar =="day_of_last_spring_frost" & name=="bFFP"), aes(x=value, y=cor), method="lm", col="darkgrey")+
  xlab("Provenance beginning of frost-free period")+ ylab("Correlation")+
  ggtitle("A) Day of last spring frost X RWI")+
  theme_bw(),

ggplot()+
    geom_hline(yintercept=0, col="black")+
  scale_color_manual(values=unname(stepped()), name="Provenance")+
  geom_point(data=left_join(lmerdata_SR, cor_provclim_long_SR, by=c("climvar","name")) %>% filter(climvar =="day_of_last_spring_frost" & name=="MWMT")  , aes(x=value, y=cor, col=factor(Provenance, levels=provs_bylat$Provenance)))+
  geom_smooth(data= left_join(lmerdata_SR, cor_provclim_long_SR, by=c("climvar","name")) %>% filter(climvar =="day_of_last_spring_frost" & name=="MWMT"), aes(x=value, y=cor), method="lm", col="darkgrey")+
  xlab("Provenance mean warmest month temp (C)")+ ylab("Correlation")+
  ggtitle("A) Day of last spring frost X RWI")+
  theme_bw(),
...
ggplot()+
    geom_hline(yintercept=0, col="black")+
  scale_color_manual(values=unname(stepped()), name="Provenance")+
  geom_point(data=left_join(lmerdata_SR, cor_provclim_long_SR, by=c("climvar","name")) %>% filter(climvar =="value_sm_tmin" & name=="PPTat")  , aes(x=value, y=cor, col=factor(Provenance, levels=provs_bylat$Provenance)))+
  geom_smooth(data= left_join(lmerdata_SR, cor_provclim_long_SR, by=c("climvar","name")) %>% filter(climvar =="value_sm_tmin" & name=="PPTat"), aes(x=value, y=cor), method="lm", col="darkgrey")+
  xlab("Provenance Fall Precip (mm)")+ ylab("Correlation")+
  ggtitle("A) Summer Tmin X RWI")+
  theme_bw(),

ggplot()+
    geom_hline(yintercept=0, col="black")+
  scale_color_manual(values=unname(stepped()), name="Provenance")+
  geom_point(data=left_join(lmerdata_SR, cor_provclim_long_SR, by=c("climvar","name")) %>% filter(climvar =="value_wt_tmin" & name=="CMD")  , aes(x=value, y=cor, col=factor(Provenance, levels=provs_bylat$Provenance)))+
  geom_smooth(data= left_join(lmerdata_SR, cor_provclim_long_SR, by=c("climvar","name")) %>% filter(climvar =="value_wt_tmin" & name=="CMD"), aes(x=value, y=cor), method="lm", col="darkgrey")+
  xlab("Provenance Climate Moisture Deficit")+ ylab("Correlation")+
  ggtitle("A) Winter Tmin X RWI")+
  theme_bw(), common.legend=TRUE, ncol=3, nrow=2)
...
for(i in 1:length(climvars)){
  for(j in 1:length(names)){
    ...
    mod <- lmer(cor ~ value + (1|Provenance), data=data)
    pvalue.lmer <- as.data.frame(summary(mod)$coefficients)[2,] %>% pull(`Pr(>|t|)`)
    r2 <-  performance::r2(mod)$R2_conditional
    
lmerdata_NR <- bind_rows(lmerdata_NR, data.frame(climvar=climvars[i], name=names[j], pvalue.lmer = pvalue.lmer, r2.lmer = r2)) 
  }}
