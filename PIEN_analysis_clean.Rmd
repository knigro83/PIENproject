...
##seasonal ppt
prior_ppt_sp <- ppt_comb_plantation %>% 
  filter(month %in% c(3,4,5)) %>% 
  mutate(year = year+1) %>% 
  group_by(CN, year) %>% 
  summarise(prior_ppt_sp = sum(ppt)) %>% 
  filter(year>1969)
...
ppt_seasonal_plantation <- left_join(prior_ppt_sp, prior_ppt_sm) %>% 
  left_join(prior_ppt_fa) %>% 
...
prior_tmean_sp <- all_temps %>% filter(month %in% c(3,4,5) & name=="tmean") %>% 
  mutate(year = year+1) %>% 
  group_by(CN, year) %>% 
  summarise(prior_tmean_sp = mean(value))
prior_tmin_sp <- all_temps %>% filter(month %in% c(3,4,5) & name=="tmin") %>% 
  mutate(year = year+1) %>% 
  group_by(CN, year) %>% 
  summarise(prior_tmin_sp = mean(value))
prior_tmax_sp <- all_temps %>% filter(month %in% c(3,4,5) & name=="tmax") %>% 
  mutate(year = year+1) %>% 
  group_by(CN, year) %>% 
  summarise(prior_tmax_sp = mean(value))
...
plantation_seasonal_temps<- plyr::join_all(list(prior_tmean_sp,prior_tmin_sp,prior_tmax_sp,prior_tmean_sm,prior_tmin_sm,prior_tmax_sm,prior_tmean_fa,prior_tmin_fa,prior_tmax_fa,prior_tmean_wt,prior_tmin_wt,prior_tmax_wt,tmean_sp,tmin_sp,tmax_sp,tmean_sm,tmin_sm,tmax_sm), type="left")
...
vpdmax_seasonal <- as.data.frame(vpdmax_comb_plantation) %>% 
 ...
 dplyr::summarise(vpdmax_sp = mean(vpdmax))) %>% 
  ...
  filter(month %in% c(3,4,5)) %>% 
  group_by(CN, year) %>% 
 dplyr::summarise(prior_vpdmax_sp = mean(vpdmax))) %>% 
  left_join(as.data.frame(vpdmax_comb_plantation) %>% 
  mutate(vpdmax = as.numeric(vpdmax), year=as.integer(year+1)) %>%
  filter(month %in% c(6,7,8)) %>% 
  group_by(CN, year) %>% 
...
priorsp_cwd<- plantation_cwd %>% 
  mutate(date= as.Date(paste(month,"01",year,sep="/"), "%m/%d/%Y")) %>% 
  filter(month%in%c(3,4,5)) %>%
  group_by(site, year) %>% 
  summarise(prior_cwd_sp = sum(cwd)) %>% 
  mutate(year=year+1) %>% 
  filter(year>1970)
...
cwd_seasonal <- left_join(priorsp_cwd, priorsm_cwd) %>% 
  left_join(priorfa_cwd) %>% 
  left_join(wt_cwd) %>% 
  left_join(sp_cwd) %>% 
  left_join(sm_cwd)
...
```{r}
priorsp_aet<- plantation_cwd %>% 
  mutate(date= as.Date(paste(month,"01",year,sep="/"), "%m/%d/%Y")) %>% 
  filter(month%in%c(3,4,5)) %>%
  group_by(site, year) %>% 
  summarise(prior_aet_sp = sum(aet)) %>% 
  mutate(year=year+1) %>% 
  filter(year>1970)
...
aet_seasonal <- left_join(priorsp_aet, priorsm_aet) %>% 
  left_join(priorfa_aet) %>% 
  left_join(wt_aet) %>% 
  left_join(sp_aet) %>% 
  left_join(sm_aet)
...
plantation_climatebalance_seasonal <- plantation_climatebalance %>% 
  filter(! (year==1970 & month %in% c(1,2))) %>% 
...
plantation_spei_seasonal <- spei(plantation_climatebalance_seasonal %>% pull(ppt_pet), scale=3)
plot(plantation_spei_seasonal)
length(plantation_spei_seasonal$fitted)

seasonal_spei<- data.frame(year = plantation_climatebalance_seasonal$year, month=plantation_climatebalance_seasonal$month, spei3 = plantation_spei_seasonal$fitted) %>% 
  filter(month%in%c(5,8,11,2)) %>% 
  mutate(season = case_when(month==5 ~ "sp",
                            month==8 ~ "sm",
                            month==11 ~ "fa",
                            month==2 ~ "wt")) %>% 
  mutate(date= as.Date(paste(month,"01",year,sep="/"), "%m/%d/%Y")) %>% 
  mutate(year=ifelse(season=="fa", year+1, year))

seasonal_spei_prior_spsm<- seasonal_spei %>% 
  filter(season%in%c("sp","sm")) %>% 
  mutate(year=year+1, season=paste0("prior_spei_",season))

seasonal_spei_wide <- seasonal_spei %>% 
  bind_rows(seasonal_spei_prior_spsm) %>% 
  dplyr::select(-c(month, date)) %>% 
  pivot_wider(names_from=season, values_from=spei3) %>% 
  rename(spei_sp = sp, spei_sm = sm, prior_spei_fa = fa, prior_spei_wt = wt) %>% 
...
plantation_clim <- ppt_seasonal_plantation %>% 
  ...
  left_join(seasonal_spei_wide)

colnames(plantation_clim)

plantation_clim_long <- plantation_clim %>% 
  dplyr::select(-site) %>% 
  pivot_longer(prior_ppt_sp:prior_spei_sm) %>% 
  separate(name, into=c("name1","name2","name3"), sep="_",remove=FALSE, fill="left") %>% 
  mutate(statistic = case_when(name2=="tmax" ~ "max",
                               name2=="tmin"~ "min",
                               name2=="tmean" ~ "mean",
                               name2=="ppt" ~ "sum",
                               TRUE ~ NA),
         type = case_when(name2=="tmax" ~ "TEMP",
                               name2=="tmin"~ "TEMP",
                               name2=="tmean" ~ "TEMP",
                               name2=="ppt" ~ "PPT",
                               TRUE ~ NA))
...
plantation_clim_long_currentyr <- plantation_clim_long %>% 
  filter(name1=="prior") %>% 
  mutate(year = year-1) %>% 
  dplyr::select(c(CN, year, name, name2, name3, value, statistic, type))
  

ggplot(plantation_clim_long_currentyr %>% filter(name2 %in% c("ppt","tmin","tmean","tmax")), aes(x=year, y=value, col=type))+
...
##sm tmin
mod_smtmin <- lm(value ~ year, data=plantation_clim_all_studyperiod %>% filter(name=="prior_tmin_sm"))
par(mfrow=c(2,2))
plot(mod_smtmin)
summary(mod_smtmin)

##fa tmin
mod_fatmin <- lm(value ~ year, data=plantation_clim_all_studyperiod %>% filter(name=="prior_tmin_fa"))
par(mfrow=c(2,2))
plot(mod_fatmin)
summary(mod_fatmin)

##aet
mod_aet <- lm(value ~ year, data=plantation_clim_all_studyperiod %>% filter(name=="prior_aet_fa"))
par(mfrow=c(2,2))
plot(mod_aet)
summary(mod_aet)

ggplot(plantation_clim_all_studyperiod %>% filter(name %in% c("prior_aet_fa","prior_tmin_sm","prior_tmin_fa")), aes(x=year, y=value, col=name, group=name))+
  geom_point()+
...
ggplot(plantation_clim_long_qs %>% 
  filter(name2 %in% c("ppt","aet","spei")), aes(x=year, y=value, color=name))+
  geom_point()+
  geom_line()+
  facet_wrap(~name, scales="free")+
  geom_line(data=plantation_clim_long_qs %>% 
  filter(name2 %in% c("ppt","aet","spei")), aes(x=year, y=out.low), col="black")+
  geom_line(data=plantation_clim_long_qs %>% 
  filter(name2 %in% c("ppt","aet","spei")), aes(x=year, y=out.hi), col="black")+
...
##latitude model - with Gila
lat_dbh_gr_mod <- lmer(growth ~ value + I(value^2) + (1|Provenance) + (1|BLK) + (1|year), data=dbh_growth_clim_long  %>% filter(name=="y"))
plot(lat_dbh_gr_mod)
qqmath(lat_dbh_gr_mod)
summary(lat_dbh_gr_mod)
r2(lat_dbh_gr_mod)

lat_dbh_gr_mod_lm <- lm(growth ~ value + I(value^2), data=dbh_growth_clim_long %>% filter(name=="y"))
...
ggplot()+
  geom_ribbon(data=predictdata_lat_dbh_gr_all, aes(x=value, ymin=lwr, ymax=upr, fill=model), alpha=0.2)+
  geom_line(data=predictdata_lat_dbh_gr_all, aes(x=value,y=fit, color=model))+
...
lat_dbh_relgrowth_mod <- lmer(relgrowth ~ value + I(value^2) + (1|Provenance) + (1|BLK) + (1|year), data=dbh_relgrowth_clim_long %>% filter(name=="y"))
plot(lat_dbh_relgrowth_mod)
qqmath(lat_dbh_relgrowth_mod)
summary(lat_dbh_relgrowth_mod)
r2(lat_dbh_relgrowth_mod)

lat_dbh_relgrowth_mod_lm <- lm(relgrowth ~ value + I(value^2), data=dbh_relgrowth_clim_long %>% filter(name=="y"))
...
ggplot()+
  geom_ribbon(data=predictdata_lat_dbh_relgrowth_all, aes(x=value, ymin=lwr, ymax=upr, fill=model), alpha=0.2)+
  geom_line(data=predictdata_lat_dbh_relgrowth_all, aes(x=value,y=fit, color=model))+
  geom_rug(data=dbh_relgrowth_clim_long %>% filter(name=="y") %>% mutate(fit=6), aes(x=value,y=fit), linewidth=1, col="black", sides="b")+
  scale_color_manual(values=c("darkgrey","black"))+
  scale_fill_manual(values = c("darkgrey","black"))+
  xlab("Provenance Latitude")+
...
ht_mods_quad06 %>% arrange(pvalue2_nog) #just y
...
moniger_survival_byyr_binom_zeros_2014 %>% 
  group_by(Provenance) %>% 
...
dbh_lat_data<- bind_rows(lat_dbh_predictdata_all %>% mutate(metric="absolute", trait="DBH"),predictdata_lat_dbh_gr_all %>% mutate(metric="increment", trait="DBH"),predictdata_lat_dbh_relgrowth_all %>% mutate(metric="relative increment", trait="DBH") )

ht_lat_data <- bind_rows(predictdata_lat_ht_all %>% rename(value=climvar.value) %>% mutate(metric="absolute", trait="Height"),predictdata_y_htgrowth_all %>% mutate(metric="increment", trait="Height"),predictdata_y_relht_all %>% mutate(metric="relative increment", trait="Height")) 
...
all_lat_data<- bind_rows(dbh_lat_data, ht_lat_data, surv_lat_data) %>% 
  ...
  mutate(ypos = case_when(units == "DBH (cm)" & model=="with Gila NF" ~ 8.25,
                          ...
                          units=="Height (m)"& model=="with Gila NF" ~ 0.5,
                          units=="Height (m)"& model=="without Gila NF" ~ 0.55,
                          units == "% Survival to 2014" & model=="with Gila NF" ~ 0.635,
                          units == "% Survival to 2014" & model=="without Gila NF" ~ 0.65,
                          units=="Basal area growth (cm^2/yr)" & model=="with Gila NF" ~ 5.25,
                          units=="Basal area growth (cm^2/yr)" & model=="without Gila NF" ~ 5.5,
                          units=="Height growth (m/yr)" & model=="with Gila NF" ~ .15,
                          units=="Height growth (m/yr)" & model=="without Gila NF" ~ .158,
                          units=="% Survival each year" & model=="with Gila NF" ~ 0.9275,
                          units=="% Survival each year" & model=="without Gila NF" ~ 0.9325,
                          units=="Basal area relative growth (%/yr)" & model=="with Gila NF" ~ 14.8,
                          units=="Basal area relative growth (%/yr)" & model=="without Gila NF" ~ 16.5,
                          units=="Height relative growth (%/yr)" & model=="with Gila NF"~10.35,
...
ggplot(all_lat_data, aes(x=value, y=fit))+
  ...
  facet_wrap(~factor(units, levels=c("% Survival to 2014","DBH (cm)","Height (m)","% Survival each year","Basal area growth (cm^2/yr)","Height growth (m/yr)","Basal area relative growth (%/yr)","Height relative growth (%/yr)")), scales="free_y")+
  geom_text(aes(x=35,y=ypos,label=paste0("p = ",round(pvalue,2)), col=model), size=3.5)+
  scale_color_manual(values=c("darkgrey","black"))+
  scale_fill_manual(values=c("darkgrey","black"))+
...
plantation_clim_cor<- plantation_clim %>% 
 ...
 dplyr::select(c(prior_ppt_sp, prior_ppt_sm, ppt_sp, ppt_sm,prior_tmean_sp:prior_tmax_sm, tmean_sp:vpdmax_sm, vpdmax_sp:prior_vpdmax_sm, prior_cwd_sp:prior_cwd_sm, cwd_sp:prior_aet_sm, aet_sp:spei_sm, prior_spei_sp, prior_spei_sm)) %>% 
  cor()
...
growth_climate_df <- growth.detrended.df %>% 
  ...
  dplyr::select(-site) %>% 
  rename(dofff_sm = day_of_first_fall_frost, dolsf_sp= day_of_last_spring_frost, prior_dofff_sm = prior_day_of_first_fall_frost, prior_dolsf_sp= prior_day_of_last_spring_frost)

climvars <- colnames(growth_climate_df %>% dplyr::select(c(prior_ppt_sp:prior_dolsf_sp)))
...
##########final plot
##just going to plot the significant relationship with winter tmean because that was correlated with growth for a lot of provenances

wt_tmean_CMD_data<- left_join(lmerdata %>% filter(climvar=="value_wt_tmean" & pvalue.lmer_nog<0.05), cor_provclim_long)
wt_tmean_CMD_mod <- lmer(cor ~ value + (1|Provenance) + (1|BLK), data=wt_tmean_CMD_data)
summary(wt_tmean_CMD_mod)
wt_tmean_CMD_mod_nog <- lmer(cor ~ value + (1|Provenance) + (1|BLK), data=wt_tmean_CMD_data %>% filter(!Provenance=="Gila NF"))
summary(wt_tmean_CMD_mod_nog)

##predictions - with Gila
wt_tmean_CMD_mod_lm <- lm(cor ~ value, data=wt_tmean_CMD_data)
newdata_wt_tmean_CMD <- data.frame(value= seq(min(wt_tmean_CMD_data$value),max(wt_tmean_CMD_data$value), length.out=50))

predict_wt_tmean_CMD <- predict(wt_tmean_CMD_mod_lm, newdata_wt_tmean_CMD, interval="confidence")

predict_data_wt_tmean_CMD <- as.data.frame(bind_cols(newdata_wt_tmean_CMD,predict_wt_tmean_CMD))

##predictions - without Gila
wt_tmean_CMD_mod_lm_nog <- lm(cor ~ value, data=wt_tmean_CMD_data %>% filter(!Provenance=="Gila NF"))
newdata_wt_tmean_CMD_nog <- data.frame(value= seq(min(wt_tmean_CMD_data %>% filter(!Provenance=="Gila NF") %>% pull(value)),max(wt_tmean_CMD_data %>% filter(!Provenance=="Gila NF") %>% pull(value)), length.out=50))

predict_wt_tmean_CMD_nog <- predict(wt_tmean_CMD_mod_lm_nog, newdata_wt_tmean_CMD_nog, interval="confidence")

predict_data_wt_tmean_CMD_nog <- as.data.frame(bind_cols(newdata_wt_tmean_CMD_nog,predict_wt_tmean_CMD_nog))

predict_data_wt_tmean_CMD_all <- bind_rows(predict_data_wt_tmean_CMD_nog %>% mutate(model="without Gila NF"), predict_data_wt_tmean_CMD %>% mutate(model="with Gila NF"))

wt_tmean_CMD_plot <- ggplot()+
  geom_point(data=wt_tmean_CMD_data, aes(x=value, y=cor), color="black")+
  geom_line(data=predict_data_wt_tmean_CMD_all, aes(x=value, y=fit, col=model))+
  geom_ribbon(data=predict_data_wt_tmean_CMD_all, aes(x=value, y=fit, ymin=lwr, ymax=upr, fill=model), alpha=0.3)+
  scale_color_manual(values=c("darkgrey","black"))+
  scale_fill_manual(values=c("darkgrey","black"))+
  geom_hline(yintercept=0)+
  xlab("Provenance CMD")+
  ylab("Correlation between TMEAN WT & RWI")+
  theme_bw()+
  theme(legend.position="top")

ggarrange(correlation_plot, wt_tmean_CMD_plot, nrow=1, ncol=2, labels=c("A)","B)"))
