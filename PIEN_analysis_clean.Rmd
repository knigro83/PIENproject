...
moniger_subset <- left_join(moniger_data_comb %>% 
  filter(! (LOC==30 & BLK=="D" & REP==3)) %>%  ##excluding Gila tree that was outlier for height in 1996
  dplyr::select(c(Provenance, SORT, LOC, BLK, REP, HT85, HT91, HT96, HT06, code)) %>%
...
moniger_data_comb %>% 
  filter(! (LOC==30 & BLK=="D" & REP==3)) %>%  ##excluding Gila tree that was outlier for height in 1996
  dplyr::select(c(Provenance, SORT, LOC, BLK, REP, DBH91, DBH96, DBH06, code)) %>%
...
left_join(moniger_data_comb %>% 
  filter(! (LOC==30 & BLK=="D" & REP==3)) %>%  ##excluding Gila tree that was outlier for height in 1996
  dplyr::select(c(SORT, LOC, BLK, REP, PC1, PC2, PC3, x, y, code))) %>% 
...
best.mod.cone.clim_nog <- glmmTMB(cbind(n_cones, total-n_cones) ~ PC2*scale(height) + (1|code) + (1|year) + (1|BLK), data= moniger_subset %>% filter(!code=="GiNF"), family = binomial)
...
newmod <- glmmTMB(cbind(n_cones, total-n_cones) ~  scale.ht*PC2, data= moniger_subset_nog, family = binomial)
summary(newmod)

CCnewdata1_cone_nog_height <- expand.grid(PC2=mean(unique(moniger_subset_nog$PC2)),
...
```{r}
##just height model
ht_cone_mod <- glmmTMB(cbind(n_cones, total-n_cones) ~ height, data= moniger_subset, family = binomial)
summary(ht_cone_mod)
x = (log(.5) + 1.45593)/.44148
x
```
...
moniger_provclim<- moniger_subset %>% 
  left_join(as.data.frame(moniger_data_comb) %>% dplyr::select(c(Provenance,SORT,LOC,BLK, REP,code,PC1,PC2,AHM:SHM,smrpb,monsoonality))) %>% 
  pivot_longer(c(PC1,PC2,AHM:SHM, x:monsoonality), names_to="climvar.name", values_to = "climvar.value") %>% 
  filter(!is.na(total))
...
#longitude
cone_x_mod <- glmmTMB(cbind(n_cones,total-n_cones) ~ climvar.value + (1|year) + (1|code) + (1|BLK), data=moniger_provclim %>% filter(climvar.name=="x"), family=binomial)
summary(cone_x_mod)
r2(cone_x_mod)

cone_x_data <- moniger_provclim %>% filter(climvar.name=="x")

newdata_x_cone <- data.frame(
    climvar.value = seq(min(cone_x_data$climvar.value), 
                      max(cone_x_data$climvar.value), 
                      length.out = 100
                      ))

##take random effect out to make predictions
cone_x_mod_glm <- glmmTMB(cbind(n_cones,total-n_cones) ~ scale(climvar.value), data=moniger_provclim %>% filter(climvar.name=="x"), family=binomial)

##predictions
conex_newdata1 <- with(cone_x_data, data.frame(climvar.value = seq(min(climvar.value),max(climvar.value),length.out=50)))

conex_newdata2 <- cbind(conex_newdata1, predict(cone_x_mod_glm, newdata = conex_newdata1, type = "link", se=TRUE))
conex_newdata3 <- within(conex_newdata2, {
    PredictedProb <- plogis(fit)
    LL <- plogis(fit - (1.96 * se.fit))
    UL <- plogis(fit + (1.96 * se.fit))
})
...
#no Gila
cone_x_mod_nog <- glmmTMB(cbind(n_cones,total-n_cones) ~ climvar.value + (1|year) + (1|Provenance)+(1|BLK), data=moniger_provclim %>% filter(climvar.name=="x") %>% filter(!Provenance=="Gila NF"), family=binomial)
summary(cone_x_mod_nog)
r2(cone_x_mod_nog)

cone_x_data_nog <- moniger_provclim %>% filter(climvar.name=="x") %>% filter(!Provenance=="Gila NF")

#prediction
##take random effect out to make predictions
cone_x_mod_nog_glm <- glmmTMB(cbind(n_cones,total-n_cones) ~ scale(climvar.value), data=cone_x_data_nog, family=binomial)

##predictions
conex_nog_newdata1 <- with(cone_x_data_nog, data.frame(climvar.value = seq(min(climvar.value),max(climvar.value),length.out=50)))

conex_nog_newdata2 <- cbind(conex_nog_newdata1, predict(cone_x_mod_nog_glm, newdata = conex_nog_newdata1, type = "link", se=TRUE))
conex_nog_newdata3 <- within(conex_nog_newdata2, {
    PredictedProb <- plogis(fit)
    LL <- plogis(fit - (1.96 * se.fit))
    UL <- plogis(fit + (1.96 * se.fit))
})

cone_x_plot_data<- conex_newdata3 %>% 
  mutate(model="with Gila NF") %>% 
  bind_rows(conex_nog_newdata3 %>% mutate(model="without Gila NF")) %>% 
  mutate(var="Longitude")

cone_x_plot<- ggplot(cone_x_plot_data, aes(x = climvar.value, y = PredictedProb)) + 
  geom_ribbon(aes(ymin = LL,ymax = UL, fill=model), alpha = 0.2) + 
  geom_line(linewidth = 1, aes(linetype=model))+
  geom_rug(data=moniger_provclim %>% filter(climvar.name=="x") %>% mutate(PredictedProb=0.6), sides="b", linewidth=1, color="black", alpha=0.5)+
  scale_linetype_manual(values=c(2,1))+
  scale_fill_manual(values=c("darkgray","black"))+
  ylim(c(0,1))+
  xlab("Longitude")+
...
ggarrange(cone_x_plot, cone_smrpb_plot, common.legend = TRUE)

##correlation between longitude and smrpb - not very strong
moniger_subset %>% 
  left_join(as.data.frame(moniger_data_comb)) %>% 
  dplyr::select(c(x,smrpb)) %>% 
  unique() %>% 
  cor()
...
cone_climdist_df <- cone_clim_long_transfer %>% 
  dplyr::select(Provenance, LOC,BLK, REP, year, n_cones, total, climvar.name, diff_plant) %>% 
  filter(climvar.name %in% c("PC1","PC2")) %>% 
  pivot_wider(names_from=climvar.name,values_from = diff_plant) %>% 
  mutate(dist_2d = sqrt(PC1^2 + PC2^2))
...
moniger_survival_byyr_binom<- moniger_survival_byyr %>% 
  group_by(Provenance,LOC,BLK,REP,year) %>% 
  summarise(alive=sum(value)) %>% 
  left_join(alive_blk_yr) %>% 
  mutate(died=prev.yr.alive-alive) %>% 
  full_join(surv_df_zero) %>% 
  left_join(moniger_survival %>% dplyr::select(c(Provenance,LOC,BLK,REP,PC1,PC2,AHM:SHM,x,y,smrpb,monsoonality)) %>% unique()) 
...
##distance in 2d climate space -- START HERE

surv_climdist_df <- surv_clim_long_transfer %>% 
  dplyr::select(Provenance, LOC,BLK, REP, year, alive, died, climvar.name, diff_plant) %>% 
  filter(climvar.name %in% c("PC1","PC2")) %>% 
  pivot_wider(names_from=climvar.name,values_from = diff_plant) %>% 
  mutate(dist_2d = sqrt(PC1^2 + PC2^2))
  
surv_climdist_mod <- glmmTMB(cbind(alive,died) ~ dist_2d + (1|Provenance) + (1|year) + (1|BLK), data=surv_climdist_df, family=binomial)
summary(surv_climdist_mod) # the closer in climate to the plantation, the higher probability of survs. 
