...
#####y
summary(lmer(relht ~ value + (1|Provenance)+ (1|BLK) + (1|year), data=ht_growth_rel_clim_long %>% filter(name=="y")))
summary(lmer(relht ~ value + (1|Provenance)+ (1|BLK) + (1|year), data=ht_growth_rel_clim_long %>% filter(name=="y") %>% filter(!Provenance=="Gila NF")))
...
predictdata_y_relht_all <- predictdata_y_relht %>% 
  mutate(model="with Gila NF") %>% 
  bind_rows(predictdata_y_relht_nog %>% mutate(model="without Gila NF"))

ggplot()+
  geom_vline(xintercept = prov_clim_adjusted_bi_long %>% filter(Provenance=="Plantation") %>% filter(name=="y") %>% pull(value), col="steelblue")+
  geom_text(aes(x=prov_clim_adjusted_bi_long %>% filter(Provenance=="Plantation") %>% filter(name=="y") %>% pull(value) + 12, y=12.5, label="plantation"), col="steelblue")+
...
ggarrange(relht_provclim_coef_plot, relht_transfer_coef_plot, ncol=2, common.legend = TRUE)
######
...
surv_mods_2014_coeffs_all %>% 
  mutate(type = "provenance climate")) %>% 
  bind_rows(data.frame(var = "2D PCA", estimate = confint(surv2014_climdist_mod)[2,3], lwr = confint(surv2014_climdist_mod)[2,1], upr = confint(surv2014_climdist_mod)[2,2], model="with Gila NF", type="transfer distance")) %>% 
  bind_rows(data.frame(var = "2D PCA", estimate = confint(surv2014_climdist_mod_nog)[2,3], lwr = confint(surv2014_climdist_mod_nog)[2,1], upr = confint(surv2014_climdist_mod_nog)[2,2], model="without Gila NF", type="transfer distance"))
...
```{r}
predictdata_lat_dbh %>% colnames()
predictdata_lat_dbh_transfer%>% colnames()

predictdata_lat_dbh_gr%>% colnames()
predictdata_lat_dbhg_transfer%>% colnames()

predictdata_lat_dbh_relgrowth%>% colnames()
predictdata_lat_dbh_relgrowth_transfer%>% colnames()

predictdata_lat_ht %>% colnames()
predictdata_y_htgrowth_nog%>% colnames()
predictdata_y_relht_all%>% colnames()

survy_bothmod_data %>% colnames()
surv2014_y_bothmod_data %>% colnames()

dbh_lat_data<- bind_rows(predictdata_lat_dbh %>% mutate(metric="absolute", trait="DBH"),predictdata_lat_dbh_gr %>% mutate(metric="increment", trait="DBH"),predictdata_lat_dbh_relgrowth %>% mutate(metric="relative increment", trait="DBH") )

ht_lat_data <- bind_rows(predictdata_lat_ht %>% rename(value=climvar.value) %>% mutate(metric="absolute", trait="Height"),predictdata_y_htgrowth_nog %>% mutate(metric="increment", trait="Height"),predictdata_y_relht_all %>% mutate(metric="relative increment", trait="Height")) %>% 
  filter(! model %in% c("with Gila NF"))
...
lat_rug_data <- expand.grid(value=provs_bylat$y, Provenance=provs_bylat$Provenance, metric=c("absolute","increment","relative increment"), trait=c("DBH","Height","Survival")) %>% 
  mutate(fit = case_when(trait == "DBH" & metric == "absolute" ~ 9,
                         ...
                         trait == "Height" & metric == "absolute" ~ .9,
