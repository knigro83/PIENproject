...
cor_means <- cor_clean %>% 
  ...
  summarise(mean.cor=mean(cor), sd.cor = sd(cor)) %>% 
  mutate(time_season = paste(time, season, sep="")) %>% 
...
ggplot(cor_means %>% filter(!time_season=="priorsp"), aes(x=factor(time_season, levels=c("priorsp","priorsm","priorfa","priorwt","NAsp","NAsm"), labels= c("priorsp","priorsm","priorfa","priorwt","sp","sm")), y=mean.cor, shape=pvalue<0.05, col=mean.cor>0))+
...
allanovas %>% filter(pvalue<0.05) #prior_ppt_wt, cwd_sp, prior_spei_wt have significant differences between provenances
...
#prior_spei_wt
prior_spei_wt_RWI_mod <- lm(cor ~ Provenance, data=cor_clean %>% filter(climvar == "prior_spei_wt"))
Anova(prior_spei_wt_RWI_mod, type="2")
lsmeans_prior_spei_wt_RWI_mod <- lsmeans(prior_spei_wt_RWI_mod, ~Provenance)
cld(lsmeans_prior_spei_wt_RWI_mod)
...
ggplot(data=sig.cors.byprov, aes(x=factor(code, levels=provs_bylat$code), y=estimate))+
    geom_jitter(data= cor_clean %>% filter(!time_season=="priorsp")%>% filter(!var=="spei") %>% left_join(prov_names), aes(x=factor(code, levels=provs_bylat$code), y=cor, color=p.value<0.05),size=2, height=0, width=0.1, alpha=0.2)+
    geom_text(data=cor_means %>% filter(!time_season=="priorsp")%>% filter(!var=="spei"), aes(x="RoNF", y=0.95,label=paste0("mean r = ",round(mean.cor,2)," \u00B1 ",round(sd.cor,2)), col=mean.cor<0))+
    geom_text(data=sig_trees_per_clim %>% filter(!time_season=="priorsp")%>% filter(!var=="spei"), aes(x="RoNF", y=0.7,label=paste0("N[p<0.05] = ",n_sig_trees)))+
  scale_color_manual(values=c("gray20","red"))+
  ggnewscale::new_scale_color()+
