...
survmod_byyear<- glmmTMB(cbind(alive, died) ~ scale(year)*Provenance, data=moniger_survival_byyr_binom, family=binomial) ##this is not converging, I think because of all the 0's. 
...
surv_eFFP_mod_glm <- glmmTMB(cbind(alive,died) ~ scale(climvar.value), data=moniger_survival_long %>% filter(climvar.name=="eFFP"), family="binomial")
...
surv_eFFP_mod_nog_glm <- glmmTMB(cbind(alive,died) ~ scale(climvar.value), data=moniger_survival_long %>% filter(climvar.name=="eFFP") %>% filter(!Provenance=="Gila NF"), family="binomial")
summary(surv_eFFP_mod_nog)

##predictions
surv_eFFP_data_nog <- moniger_survival_long %>% filter(climvar.name=="eFFP") %>% filter(!Provenance=="Gila NF")

surveFFP_nog_newdata1 <- expand.grid(climvar.value = seq(min(surv_eFFP_data_nog$climvar.value),max(surv_eFFP_data_nog$climvar.value),length.out=50))

surveFFP_nog_newdata2 <- cbind(surveFFP_nog_newdata1, predict(surv_eFFP_mod_nog_glm, newdata = surveFFP_nog_newdata1, type = "link", se=TRUE))
...
surv_mods_quad %>% arrange(pvalue2_nog) ##Eref and CMD are significant both with and without Gila, but they are positively correlated so I will just use Eref since it has a larger r2.
...
surv_ind_plot_data<- bind_rows(
surveFFP_bothmod_data %>% mutate(climvar.name="eFFP"),
...
surv_ind_plot<- ggplot(surv_ind_plot_data, aes(x = climvar.value, y = PredictedProb)) + 
  ...
  geom_rug(data=moniger_survival_long %>% filter(climvar.name%in%c("Eref","FFP","smrpb")) %>% mutate(PredictedProb=.7), sides="b", linewidth=1)+
  scale_fill_manual(values=c("black","gray"))+
  ylab("Survival Probability")+
  xlab("Provenance Climate Value")+
  facet_wrap(~factor(climvar.name, levels=c("eFFP","smrpb","Eref"), labels=c("end of frost-free period (days)","Summer Precip Ratio","Hargreave's reference evaporation")), scales="free")+
  theme_bw()
surv_ind_plot
