...
plantation_clim_long_currentyr <- plantation_clim_long %>% 
  filter(name1=="prior") %>% 
  mutate(year = year-1) %>% 
  bind_rows(plantation_clim_long %>% filter(name3=="sp")) %>% 
...
cwd_sp_CMD_plot <- ggplot()+
  ...
  scale_fill_manual(values=c("darkgrey","black"))+
  geom_hline(yintercept=0)+
  geom_vline(xintercept=plantation_only$CMD, color="steelblue", linewidth=1)+
  xlab("Provenance CMD")+
  ylab("Pearson's r")+
  theme_bw()+
...
cwd_sp_MCMT_plot <- ggplot()+
  ...
  scale_fill_manual(values=c("darkgrey","black"))+
  geom_hline(yintercept=0)+
  geom_vline(xintercept=plantation_only$MCMT, color="steelblue", linewidth=1)+
