...
anova(aov_DBHrel) ##basal area growth differs by provenance and year, but not by the interaction of the two. 
```
...
dbh_relgro_mods_quad %>% arrange(pvalue2_nog) #nothing significant
```
...
ht_growth_clim_long_transfer <- ht_data_year_clim_long %>%
  left_join(plantation_climNA, by=c("climvar.name"="var")) %>% 
  mutate(diff_plant = abs(plant_value - value))

ht_growth_clim_long_transfer_nog <- ht_growth_clim_long_transfer %>% filter(!Provenance=="Gila NF")

ht_mods_transfer<- data.frame()
for(i in 1:length(provenance_climvars)){
mod <- lmer(value ~ diff_plant + (1|Provenance)+ (1|BLK) + (1|year), data=ht_growth_clim_long_transfer %>% filter(climvar.name==provenance_climvars[i]))
pvalue<- as.data.frame(summary(mod)$coefficients)[2,]$`Pr(>|t|)`
Mr2 <- r2(mod)$R2_marginal

mod_nog <- lmer(value ~ diff_plant + (1|Provenance)+ (1|BLK) + (1|year), data=ht_growth_clim_long_transfer_nog %>% filter(climvar.name==provenance_climvars[i]) %>% filter(!Provenance=="Gila NF"))
pvalue_nog<- as.data.frame(summary(mod_nog)$coefficients)[2,]$`Pr(>|t|)`
...
ht_mods_transfer %>% arrange(pvalue_nog)

##distance in 2d climate space

ht_climdist_df <- ht_growth_clim_long_transfer %>% 
...
ht_climdist_mod <- lmer(ht ~ dist_2d + (1|Provenance)+ (1|BLK) + (1|year), data=ht_climdist_df)
plot(ht_climdist_mod)
qqmath(ht_climdist_mod)
summary(ht_climdist_mod) ##looks like trees further away from the plantation in climate space grow taller?
...
bestmod_relht_pca_lm <- lm(value ~  PC2, data=ht_growth_rel_clim_noG)

newdata_relht <-  expand.grid(PC2=seq(min(ht_growth_rel_clim_noG$PC2), max(ht_growth_rel_clim_noG$PC2), length.out=50))
...
ggplot()+
  ...
  geom_rug(data=ht_growth_rel_clim_noG %>% mutate(fit=10), aes(x=PC2,y=fit), linewidth=1, col="black", sides="b")+
  xlab("Provenance PC2")+
...
ht_growth_rel_clim_long_transfer <- ht_growth_rel_clim_long %>%
  ...
  left_join(prov_vars %>% filter(Provenance=="Plantation") %>% dplyr::select(var,value) %>% rename(plant_value = value), by=c("name"="var")) %>% 
  mutate(diff_plant = abs(plant_value - value))

ht_growth_rel_clim_long_transfer_nog <- ht_growth_rel_clim_long_transfer %>% filter(!Provenance=="Gila NF")

provenance_climvars_transfer <- provenance_climvars[-c(1:2)]

relht_mods_transfer<- data.frame()
for(i in 1:length(provenance_climvars)){
mod <- lmer(relht ~ diff_plant + (1|Provenance)+ (1|BLK) + (1|year), data=ht_growth_rel_clim_long_transfer %>% filter(name==provenance_climvars_transfer[i]))
pvalue<- as.data.frame(summary(mod)$coefficients)[2,]$`Pr(>|t|)`
Mr2 <- r2(mod)$R2_marginal

mod_nog <- lmer(relht ~ diff_plant + (1|Provenance)+ (1|BLK) + (1|year), data=ht_growth_rel_clim_long_transfer_nog %>% filter(name==provenance_climvars_transfer[i]) %>% filter(!Provenance=="Gila NF"))
pvalue_nog<- as.data.frame(summary(mod_nog)$coefficients)[2,]$`Pr(>|t|)`
Mr2_nog <- r2(mod_nog)$R2_marginal

relht_mods_transfer <- bind_rows(relht_mods_transfer, data.frame(var=provenance_climvars_transfer[i], pvalue = pvalue, Mr2=Mr2, pvalue_nog = pvalue_nog, Mr2_nog=Mr2_nog))

}

relht_mods_transfer %>% arrange(pvalue_nog)
...
resil_0102mod_pc_noG <- lmer(log(resilience) ~ PC1*PC2 + I(PC1^2) + I(PC2^2) + (1|Provenance) + (1|BLK), data=RR0102_clim %>% filter(!Provenance=="Gila NF"))
