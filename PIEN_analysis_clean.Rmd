...
#smrpb
cone_smrpb_mod_nog <- glmmTMB(cbind(n_cones,total-n_cones) ~ diff_plant*height + (1|year) + (1|code)+(1|BLK), data=cone_clim_long_transfer_nog %>% filter(climvar.name=="smrpb"), family=binomial)
summary(cone_smrpb_mod_nog) #provenances with smrpb more different from the plantation have higher probability of cones. 
r2(cone_smrpb_mod_nog)
...
surv_mods %>% arrange(pvalue_nog) ##length of growing season seems to be influential both with and without Gila, also smrpb & PPTsp
surv_mods %>% 
  filter(pvalue_nog<0.05) %>% 
  arrange(desc(Mr2_nog))
...
surv_mods_2014 %>% arrange(pvalue_nog) ##length of growing season seems to be influential both with and without Gila, also smrpb
surv_mods_2014 %>% 
  filter(pvalue_nog<0.05) %>% 
  arrange(desc(Mr2_nog))

#eFFP
surv_eFFP_mod2014 <- glmmTMB(cbind(alive,died) ~ scale(climvar.value) + (1|Provenance)+ (1|BLK), data=moniger_survival2014_long %>% filter(climvar.name=="eFFP"), family="binomial")
summary(surv_eFFP_mod2014)

surv_eFFP_data2014 <- moniger_survival2014_long %>% filter(climvar.name=="eFFP")

newdata_eFFP_surv2014 <- data.frame(
    climvar.value = seq(min(surv_eFFP_data2014$climvar.value), 
                      max(surv_eFFP_data2014$climvar.value), 
                      length.out = 100
                      ))

##predictions
surv2014eFFP_newdata1 <-  expand.grid(climvar.value = seq(min(newdata_eFFP_surv2014$climvar.value),max(newdata_eFFP_surv2014$climvar.value),length.out=50))

surv2014_eFFP_mod_glm <- glmmTMB(cbind(alive,died) ~ scale(climvar.value), data=moniger_survival2014_long %>% filter(climvar.name=="eFFP"), family="binomial")

surv2014_eFFP_newdata2 <- cbind(surv2014eFFP_newdata1, predict(surv2014_eFFP_mod_glm, newdata = surv2014eFFP_newdata1, type = "link", se=TRUE))
surv2014_eFFP_newdata3 <- within(surv2014_eFFP_newdata2, {
    PredictedProb <- plogis(fit)
    LL <- plogis(fit - (1.96 * se.fit))
    UL <- plogis(fit + (1.96 * se.fit))
})
...
surv2014_eFFP_mod_nog_glm <- glmmTMB(cbind(alive,died) ~ scale(climvar.value), data=moniger_survival2014_long %>% filter(climvar.name=="eFFP") %>% filter(!Provenance=="Gila NF"), family="binomial")

##predictions
surv2014_eFFP_data_nog <- moniger_survival2014_long %>% filter(climvar.name=="eFFP") %>% filter(!Provenance=="Gila NF")

surv2014_effp_nog_newdata1 <- expand.grid(climvar.value = seq(min(surv2014_eFFP_data_nog$climvar.value),max(surv2014_eFFP_data_nog$climvar.value),length.out=50))

surv2014_effp_nog_newdata2 <- cbind(surv2014_effp_nog_newdata1, predict(surv2014_eFFP_mod_nog_glm, newdata = surv2014_effp_nog_newdata1, type = "link", se=TRUE))
surv2014_effp_nog_newdata3 <- within(surv2014_effp_nog_newdata2, {
    PredictedProb <- plogis(fit)
    LL <- plogis(fit - (1.96 * se.fit))
    UL <- plogis(fit + (1.96 * se.fit))
})

surv2014_effp_bothmod_data<- bind_rows(surv2014_eFFP_newdata3 %>% mutate(model="with Gila NF"),
surv2014_effp_nog_newdata3 %>% mutate(model="without Gila NF"))

ggplot(surv2014_effp_bothmod_data, aes(x = climvar.value, y = PredictedProb, linetype=model)) + geom_ribbon(aes(ymin = LL,
    ymax = UL), alpha = 0.2) + 
  geom_line(linewidth = 1)+
  xlab("eFFP")
#########
...
surv2014_smrpb_nog_newdata2 <- cbind(surv2014_smrpb_nog_newdata1, predict(surv2014_smrpb_mod_nog, newdata = surv2014_smrpb_nog_newdata1, type = "link", se=TRUE))
...
##y
surv2014_y_mod <- glmmTMB(cbind(alive,died) ~ scale(climvar.value) + I(scale(climvar.value)^2) + (1|Provenance) + (1|BLK), data=moniger_survival2014_long %>% filter(climvar.name=="y"), family="binomial")
summary(surv2014_y_mod)
r2(surv2014_y_mod)

surv2014_y_mod_glm <- glmmTMB(cbind(alive,died) ~ scale(climvar.value) + I(scale(climvar.value)^2), data=moniger_survival2014_long %>% filter(climvar.name=="y"), family="binomial")

surv2014_y_data <- moniger_survival2014_long %>% filter(climvar.name=="y")

##predictions
surv2014_y_newdata1 <- expand.grid(climvar.value = seq(min(surv2014_y_data$climvar.value),max(surv2014_y_data$climvar.value),length.out=50))
...
#without Gila is marginally significant
surv2014_y_mod_nog <- glmmTMB(cbind(alive,died) ~ scale(climvar.value) + I(scale(climvar.value)^2) + (1|Provenance) + (1|BLK), data=moniger_survival2014_long %>% filter(climvar.name=="y") %>% filter(!Provenance=="Gila NF"), family="binomial")
summary(surv2014_y_mod_nog)
r2(surv2014_y_mod_nog)

surv2014_y_mod_nog_glm <- glmmTMB(cbind(alive,died) ~ scale(climvar.value) + I(scale(climvar.value)^2), data=moniger_survival2014_long %>% filter(climvar.name=="y") %>% filter(!Provenance=="Gila NF"), family="binomial")

##predictions
surv2014_y_newdata1_nog <- expand.grid(climvar.value = seq(min(moniger_survival2014_long %>% filter(climvar.name=="y") %>% filter(!Provenance=="Gila NF") %>% pull(climvar.value)),max(moniger_survival2014_long %>% filter(climvar.name=="y") %>% filter(!Provenance=="Gila NF") %>% pull(climvar.value)),length.out=50))

surv2014_y_newdata2_nog <- cbind(surv2014_y_newdata1_nog, predict(surv_y_mod_nog_glm, newdata = surv2014_y_newdata1_nog, type = "link", se=TRUE))
surv2014_y_newdata3_nog <- within(surv2014_y_newdata2_nog, {
    PredictedProb <- plogis(fit)
    LL <- plogis(fit - (1.96 * se.fit))
    UL <- plogis(fit + (1.96 * se.fit))
})

surv2014_y_bothmod <- bind_rows(surv2014_y_newdata3 %>% mutate(model="with Gila NF"),surv2014_y_newdata3_nog %>% mutate(model="without Gila NF"))

ggplot(surv2014_y_bothmod, aes(x = climvar.value, y = PredictedProb, linetype=model)) + geom_ribbon(aes(ymin = LL,
    ymax = UL), alpha = 0.2) + 
  geom_line(linewidth = 1)+
  xlab("y")+
    ylab("Predicted Probability")+
...
surv0_clim_long_transfer <- moniger_survival2014_long %>%
  left_join(plantation_climNA, by=c("climvar.name"="var")) %>% 
  mutate(diff_plant = abs(plant_value - climvar.value))
