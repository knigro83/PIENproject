...
##sm tmin
mod_smtmin <- lm(value ~ year, data=plantation_clim_all_studyperiod %>% filter(name=="tmin_sm"))
par(mfrow=c(2,2))
plot(mod_smtmin)
summary(mod_smtmin)

##fa tmin
mod_fatmin <- lm(value ~ year, data=plantation_clim_all_studyperiod %>% filter(name=="tmin_fa"))
par(mfrow=c(2,2))
plot(mod_fatmin)
summary(mod_fatmin)

##aet
mod_aet <- lm(value ~ year, data=plantation_clim_all_studyperiod %>% filter(name=="aet_fa"))
par(mfrow=c(2,2))
plot(mod_aet)
summary(mod_aet)

ggplot(plantation_clim_all_studyperiod %>% filter(name %in% c("aet_fa","tmin_sm","tmin_fa")), aes(x=year, y=value, col=name, group=name))+
  geom_point()+
