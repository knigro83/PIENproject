...
plantation_clim_long_currentyr <- plantation_clim_long %>% 
  ...
  dplyr::select(c(CN, year, name2, name3, value, statistic, type))
...
ggplot(plantation_clim_long_currentyr, aes(x=year, y=value, col=name2))+
  ggnewscale::new_scale_color()+
  geom_line(aes(color=factor(statistic, levels=c("sum","max","mean","min"))))+
  geom_smooth(method="lm", aes(color=factor(statistic, levels=c("sum","max","mean","min"))))+
...
spppt_mod <- lm(value~year, data=plantation_clim_long %>% filter(name=="ppt_sp"))
plot(spppt_mod)
summary(spppt_mod) #NS

##summer ppt

smppt_mod <- lm(value~year, data=plantation_clim_long %>% filter(name=="ppt_sm"))
plot(smppt_mod)
summary(smppt_mod) #NS

##fall ppt

atppt_mod <- lm(value~year, data=plantation_clim_long %>% filter(name=="prior_ppt_fa"))
plot(atppt_mod)
summary(atppt_mod) #NS

##winter ppt

wtppt_mod <- lm(value~year, data=plantation_clim_long %>% filter(name=="prior_ppt_wt"))
plot(wtppt_mod)
summary(wtppt_mod) #NS

##spring tmean

sptmean_mod <- lm(value~year, data=plantation_clim_long %>% filter(name=="tmean_sp"))
plot(sptmean_mod)
summary(sptmean_mod) #NS

sptmin_mod <- lm(value~year, data=plantation_clim_long %>% filter(name=="tmin_sp"))
plot(sptmin_mod)
summary(sptmin_mod) #significant

sptmax_mod <- lm(value~year, data=plantation_clim_long %>% filter(name=="tmax_sp"))
plot(sptmax_mod)
summary(sptmax_mod) #NS

##summer tmean

smtmean_mod <- lm(value~year, data=plantation_clim_long %>% filter(name=="tmean_sm"))
plot(smtmean_mod)
summary(smtmean_mod) #Significant

smtmin_mod <- lm(value~year, data=plantation_clim_long %>% filter(name=="tmin_sm"))
plot(smtmin_mod)
summary(smtmin_mod) #significant

smtmax_mod <- lm(value~year, data=plantation_clim_long %>% filter(name=="tmax_sm"))
plot(smtmax_mod)
summary(smtmax_mod) #significant

##fall tmean

attmean_mod <- lm(value~year, data=plantation_clim_long %>% filter(name=="prior_tmean_fa"))
plot(attmean_mod)
summary(attmean_mod) #Significant

fatmin_mod <- lm(value~year, data=plantation_clim_long %>% filter(name=="prior_tmin_fa"))
plot(fatmin_mod)
summary(fatmin_mod) #significant

fatmax_mod <- lm(value~year, data=plantation_clim_long %>% filter(name=="prior_tmax_fa"))
plot(fatmax_mod)
summary(fatmax_mod) #significant

##winter tmean

wttmean_mod <- lm(value~year, data=plantation_clim_long %>% filter(name=="prior_tmean_wt"))
plot(wttmean_mod)
summary(wttmean_mod) #NS

wttmin_mod <- lm(value~year, data=plantation_clim_long %>% filter(name=="prior_tmin_wt"))
plot(wttmin_mod)
summary(wttmin_mod) #NS

wttmax_mod <- lm(value~year, data=plantation_clim_long %>% filter(name=="prior_tmax_wt"))
plot(wttmax_mod)
...
ggplot(plantation_clim_all_studyperiod, aes(x=year, y=value, col=name2, group=name2))+
  geom_point()+
...
ggplot(plantation_clim_long_qs %>% 
  ...
  geom_line(data=plantation_clim_long_qs, aes(x=year, y=out.low), col="gray20")+
  geom_line(data=plantation_clim_long_qs, aes(x=year, y=out.hi), col="gray20")+
  theme_bw()+
...
ggplot()+
  ...
  geom_vline(xintercept = plant_pca_scores %>% filter(period=="1970s") %>% pull(PC1), col="steelblue", linewidth=2)+
  theme_bw(),
ggplot()+
  geom_histogram(data=pien_clim_niche, aes(x=PC2), fill="gray", col="black")+
  geom_vline(xintercept = plant_pca_scores %>% filter(period=="1970s") %>% pull(PC2), col="steelblue", linewidth=2)+
  theme_bw())
...
cones_TD_plot<- ggplot() + 
  ...
  geom_vline(xintercept=plantation_only$TD, col="steelblue", linewidth=1)+
...
cones_pptsm_plot<- ggplot(cone_ht_pptsm_quad_newdata3_all, aes(x = climvar.value, y = PredictedProb)) + 
  ...
  scale_fill_manual(values=c("darkgray","black"))+
  scale_color_manual(values=c("darkgray","black"))+
  geom_vline(xintercept=plantation_only$PPTsm, col="steelblue", linewidth=1)+
  ylim(c(0,1))+
  xlab("Provenance summer precip")+
...
ggplot(all_lat_data, aes(x=value, y=fit))+
  ...
  scale_color_manual(values=c("darkgrey","black"))+
  scale_fill_manual(values=c("darkgrey","black"))+
  geom_vline(xintercept=plantation_only$y, col="steelblue", linewidth=1)+
