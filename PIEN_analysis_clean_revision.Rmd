...
dbh_numeric_time <- dbh_numeric %>% 
  ...
  mutate(PC1transfer = plant_PC1 - PC1, PC2transfer = plant_PC2 - PC2) %>% 
...
dbh_clim_long_transfer <- dbh_numeric_time_long %>%
  ...
  mutate(transfer = plant_value - value) %>% 
...
#with Gila
mod_DBHgr_pca <- lmer(sqrt(growth) ~ PC1transfer*PC2transfer + I(PC1transfer^2) + I(PC2transfer^2) + pcadist2d + (1|Provenance) + (1|BLK) + (1|year), data=dbh_growth_clim)
plot(mod_DBHgr_pca)
qqmath(mod_DBHgr_pca)
summary(mod_DBHgr_pca) #nothing significant

#no gila

mod_DBHgr_pca_nog <- lmer(sqrt(growth) ~ PC1transfer*PC2transfer + I(PC1transfer^2) + I(PC2transfer^2) + pcadist2d + (1|Provenance) + (1|BLK) + (1|year), data=dbh_growth_clim_noG)
...
dbh_growth_clim_long <- dbh_growth_clim %>% 
  dplyr::select(c(SORT,LOC,BLK,REP,Provenance,code,Series,year,growth,all_of(provenance_climvars))) %>% 
  pivot_longer(CMD:monsoonality)

dbh_growth_clim_long_transfer <- dbh_growth_clim_long %>%
  left_join(plantation_climNA, by=c("name"="var")) %>% 
  mutate(transfer = plant_value - value) %>% 
  bind_rows(dbh_growth_clim[,c("SORT","LOC","BLK","REP","Provenance","code","Series","year","growth","PC1transfer","PC2transfer","pcadist2d")] %>% 
  pivot_longer(PC1transfer:pcadist2d, names_to="name", values_to="transfer"))

dbh_growth_clim_long_transfer_nog <- dbh_growth_clim_long_transfer %>% filter(!Provenance=="Gila NF")

transfer_vars <- unique(dbh_growth_clim_long_transfer_nog$name)

growth_mods_transfer<- data.frame()
for(i in 1:length(transfer_vars)){
mod <- lmer(growth ~ transfer + (1|Provenance)+ (1|BLK) + (1|year), data=dbh_growth_clim_long_transfer %>% filter(name==transfer_vars[i]))
pvalue<- as.data.frame(summary(mod)$coefficients)[2,]$`Pr(>|t|)`
Mr2 <- r2(mod)$R2_marginal

mod_nog <- lmer(growth ~ transfer + (1|Provenance)+ (1|BLK) + (1|year), data=dbh_growth_clim_long_transfer_nog %>% filter(name==transfer_vars[i]) %>% filter(!Provenance=="Gila NF"))
pvalue_nog<- as.data.frame(summary(mod_nog)$coefficients)[2,]$`Pr(>|t|)`
Mr2_nog <- r2(mod_nog)$R2_marginal

growth_mods_transfer <- bind_rows(growth_mods_transfer, data.frame(var=transfer_vars[i], pvalue = pvalue, Mr2=Mr2, pvalue_nog = pvalue_nog, Mr2_nog=Mr2_nog))

}

growth_mods_transfer %>% arrange(pvalue_nog) #nothing significant without Gila

#quadratic

growth_mods_transfer_quad<- data.frame()

for(i in 1:length(transfer_vars)){
mod <- lmer(growth ~ transfer + I(transfer^2) + (1|Provenance) + (1|BLK) + (1|year), data=dbh_growth_clim_long_transfer %>% filter(name==transfer_vars[i]))
pvalues<- as.data.frame(summary(mod)$coefficients)[2:3,]$`Pr(>|t|)`
Cr2 <- performance::r2(mod)$R2_conditional
Mr2 <- performance::r2(mod)$R2_marginal
 
mod_nog <- lmer(growth ~ transfer + I(transfer^2) + (1|Provenance) + (1|BLK) + (1|year), data=dbh_growth_clim_long_transfer_nog %>% filter(name==transfer_vars[i]) %>% filter(!Provenance=="Gila NF"))
pvalues_nog<- as.data.frame(summary(mod_nog)$coefficients)[2:3,]$`Pr(>|t|)`
Cr2_nog <- performance::r2(mod_nog)$R2_conditional
Mr2_nog <- performance::r2(mod_nog)$R2_marginal

growth_mods_transfer_quad <- bind_rows(growth_mods_transfer_quad, data.frame(var=transfer_vars[i], pvalue1 = pvalues[1], pvalue2 = pvalues[2], cr2= Cr2, mr2 = Mr2, pvalue1_nog = pvalues_nog[1], pvalue2_nog = pvalues_nog[2], cr_nog = Cr2_nog, mr2_nog = Mr2_nog))

}

growth_mods_transfer_quad %>% arrange(pvalue2_nog) #nothing significant
...
```{r}
###analyze basal area growth data
aov_DBHrel <- lmer(sqrt(relgrowth) ~ year*Provenance + (1|BLK), data=dbh_growth_data_rel)
plot(aov_DBHrel)
qqmath(aov_DBHrel)
...
mod_relgrowth_pca_nog <- lmer(sqrt(relgrowth) ~ PC1transfer*PC2transfer + I(PC1transfer^2) + I(PC2transfer^2) + pcadist2d + (1|Provenance) + (1|BLK) + (1|year), data=dbh_growth_rel_clim_noG)
...
ggarrange(ggplot(data=nursery_ht_clim_long %>% filter(name=="smrpb"), aes(x=value, y=nursery_ht_m))+
  geom_point()+
  geom_smooth(method="lm")+
  theme_bw()+
...
ht_ranks<- ht_data_aves_forfig %>% 
  ...
  dplyr::select(-c("seRG","seTH","meangrowth")) %>% 
...
dbh_ht_ranks<- dbh_ranks %>% 
  mutate(trait="dbh") %>% 
  bind_rows(ht_ranks %>% mutate(trait="ht"))

ggplot(data=dbh_ht_ranks)+
  #geom_point(aes(x=year, y=rank, col=factor(Provenance, levels=provs_bylat$Provenance)))+
  #geom_line(aes(x=year, y=rank, col=factor(Provenance, levels=provs_bylat$Provenance)))+
  geom_ribbon(alpha = 0.7, aes(x=year, ymin=rank-.5, ymax=rank+.5, fill=factor(Provenance, levels=provs_bylat$Provenance)))+
  scale_color_manual(values=unname(stepped()), name="Provenance")+
  scale_fill_manual(values=unname(stepped()), name="Provenance")+
  facet_grid(trait~region)+
  theme_bw()
...
#with Gila
mod_ht_pca <- lmer(log(ht) ~ PC1transfer*PC2transfer + I(PC1transfer^2) + I(PC2transfer^2) + pcadist2d + (1|Provenance) + (1|BLK) + (1|year), data=ht_data_year_clim)
plot(mod_ht_pca)
qqmath(mod_ht_pca)
summary(mod_ht_pca) #PC2transfer^2 significant with Gila
...
ht_data_year_clim_long <- ht_data_year_clim %>% 
  dplyr::select(c(LOC,BLK,REP,Provenance,nursery_ht_m,year,ht,growth.m,rel.growth,all_of(provenance_climvars))) %>% 
  pivot_longer(CMD:monsoonality)

ht_data_year_clim_long_transfer <- ht_data_year_clim_long %>%
  left_join(plantation_climNA, by=c("name"="var")) %>% 
  mutate(transfer = plant_value - value) %>% 
  bind_rows(ht_data_year_clim[,c("LOC","BLK","REP","Provenance","nursery_ht_m","year","ht","growth.m","rel.growth","PC1transfer","PC2transfer","pcadist2d")] %>% 
  pivot_longer(PC1transfer:pcadist2d, names_to="name", values_to="transfer"))
...
for(i in 1:length(transfer_vars)){
mod <- lmer(ht ~ transfer + (1|Provenance)+ (1|BLK) + (1|year), data=ht_data_year_clim_long_transfer %>% filter(name==transfer_vars[i]))
pvalue<- as.data.frame(summary(mod)$coefficients)[2,]$`Pr(>|t|)`
Mr2 <- r2(mod)$R2_marginal

mod_nog <- lmer(ht ~ transfer + (1|Provenance)+ (1|BLK) + (1|year), data=ht_data_year_clim_long_transfer_nog %>% filter(name==transfer_vars[i]))
pvalue_nog<- as.data.frame(summary(mod_nog)$coefficients)[2,]$`Pr(>|t|)`
Mr2_nog <- r2(mod_nog)$R2_marginal
...
for(i in 1:length(transfer_vars)){
mod <- lmer(ht ~ transfer + I(transfer^2) + (1|Provenance) + (1|BLK) + (1|year), data=ht_data_year_clim_long_transfer %>% filter(name==transfer_vars[i]))
pvalues<- as.data.frame(summary(mod)$coefficients)[2:3,]$`Pr(>|t|)`
Cr2 <- performance::r2(mod)$R2_conditional
Mr2 <- performance::r2(mod)$R2_marginal
 
mod_nog <- lmer(ht ~ transfer + I(transfer^2) + (1|Provenance) + (1|BLK) + (1|year), data=ht_data_year_clim_long_transfer_nog %>% filter(name==transfer_vars[i])) 
pvalues_nog<- as.data.frame(summary(mod_nog)$coefficients)[2:3,]$`Pr(>|t|)`
Cr2_nog <- performance::r2(mod_nog)$R2_conditional
Mr2_nog <- performance::r2(mod_nog)$R2_marginal
...
ht_3meas <- ht_growth_data %>% filter(!is.na(HT14)) %>% 
  dplyr::select(c(Provenance, name, value)) %>% 
  group_by(Provenance, name) %>% 
  summarise(meangrowth=mean(value)) %>% 
  mutate(name=substr(name,7,8)) %>% 
  right_join(ht_matrix %>% 
  as.data.frame() %>% 
  rownames_to_column(var="Provenance") %>% 
  pivot_longer(nursery_ht_m:HT14, values_to="meanht")%>% 
  mutate(name=substr(name,3,4)), by=c("Provenance","name")) %>% 
  mutate(year=case_when(name=="06" ~ 2006,
                        name=="14" ~ 2014,
                        name=="79" ~ 1979,
                        name=="85" ~1985,
                        name=="91" ~ 1991,
...
height_growth_forplot <- ht_growth_data %>% 
  dplyr::select(LOC, BLK, REP, Provenance, name, value) %>% 
  mutate(year= ifelse(name %in% c("growth79","growth85","growth91","growth96"), as.numeric(paste0("19",substr(name, 7,8))), as.numeric(paste0("20",substr(name, 7,8)))))
  
ggplot()+
  geom_bar(data=ht_3meas, aes(x=year, y=meangrowth*10, fill = factor(Provenance, levels=provs_bylat$Provenance)), stat="identity")+
  ...
  scale_y_continuous(name = "Height (m)",sec.axis = sec_axis( transform=~./10, name="Height Growth Rate (m/yr)")) +
...
ht_growth_clim<- ht_growth_data %>% 
  left_join(prov_pcapts_big) %>% 
  rename(htgrowth=value) %>% 
  mutate(plant_PC1 = plant_pca_scores$PC1[1], plant_PC2 = plant_pca_scores$PC2[1]) %>% 
  mutate(PC1transfer = plant_PC1 - PC1, PC2transfer = plant_PC2 - PC2) %>% 
  mutate(absPC1transfer = abs(PC1transfer), absPC2transfer = abs(PC2transfer)) %>% 
  mutate(pcadist2d = sqrt(absPC1transfer^2 + absPC2transfer^2))

ht_growth_clim_noG <- ht_growth_clim %>% 
  filter(!Provenance=="Gila NF")

###using year as a random effect

#with Gila
mod_htgrowth_pca <- lmer(htgrowth ~ PC1transfer*PC2transfer + I(PC1transfer^2) + I(PC2transfer^2) + pcadist2d + (1|Provenance) + (1|BLK) + (1|name), data=ht_growth_clim)
plot(mod_htgrowth_pca)
qqmath(mod_htgrowth_pca)
summary(mod_htgrowth_pca) #PC2transfer^2 significant with Gila

#no gila

mod_htgrowth_pca_noG <- lmer(htgrowth ~ PC1transfer*PC2transfer + I(PC1transfer^2) + I(PC2transfer^2) + pcadist2d + (1|Provenance) + (1|BLK) + (1|name), data=ht_growth_clim_noG)
plot(mod_htgrowth_pca_noG)
qqmath(mod_htgrowth_pca_noG)
summary(mod_htgrowth_pca_noG) #nothing significant
options(na.action="na.fail")
mod_htgrowth_pca_noG_dredge<- dredge(mod_htgrowth_pca_noG)
mod_htgrowth_pca_noG_dredge #null is best
```
...
for(i in 1:length(transfer_vars)){
mod <- lmer(growth.m ~ transfer + (1|Provenance)+ (1|BLK) + (1|year), data=ht_data_year_clim_long_transfer %>% filter(name==transfer_vars[i]))
pvalue<- as.data.frame(summary(mod)$coefficients)[2,]$`Pr(>|t|)`
Mr2 <- r2(mod)$R2_marginal

mod_nog <- lmer(growth.m ~ transfer + (1|Provenance)+ (1|BLK) + (1|year), data=ht_data_year_clim_long_transfer_nog %>% filter(name==transfer_vars[i]))
pvalue_nog<- as.data.frame(summary(mod_nog)$coefficients)[2,]$`Pr(>|t|)`
Mr2_nog <- r2(mod_nog)$R2_marginal

htgrowth_mods_transfer <- bind_rows(htgrowth_mods_transfer, data.frame(var=transfer_vars[i], pvalue = pvalue, Mr2=Mr2, pvalue_nog = pvalue_nog, Mr2_nog=Mr2_nog))

}

htgrowth_mods_transfer %>% arrange(pvalue_nog) #nothing significant without Gila
...
for(i in 1:length(transfer_vars)){
mod <- lmer(growth.m ~ transfer + I(transfer^2) + (1|Provenance) + (1|BLK) + (1|year), data=ht_data_year_clim_long_transfer %>% filter(name==transfer_vars[i]))
pvalues<- as.data.frame(summary(mod)$coefficients)[2:3,]$`Pr(>|t|)`
Cr2 <- performance::r2(mod)$R2_conditional
Mr2 <- performance::r2(mod)$R2_marginal
 
mod_nog <- lmer(growth.m ~ transfer + I(transfer^2) + (1|Provenance) + (1|BLK) + (1|year), data=ht_data_year_clim_long_transfer_nog %>% filter(name==transfer_vars[i])) 
pvalues_nog<- as.data.frame(summary(mod_nog)$coefficients)[2:3,]$`Pr(>|t|)`
Cr2_nog <- performance::r2(mod_nog)$R2_conditional
Mr2_nog <- performance::r2(mod_nog)$R2_marginal

htgrowth_mods_quad <- bind_rows(htgrowth_mods_quad, data.frame(var=transfer_vars[i], pvalue1 = pvalues[1], pvalue2 = pvalues[2], cr2= Cr2, mr2 = Mr2, pvalue1_nog = pvalues_nog[1], pvalue2_nog = pvalues_nog[2], cr_nog = Cr2_nog, mr2_nog = Mr2_nog))

}

htgrowth_mods_quad %>% arrange(pvalue2_nog) #nothing significant
```
...
for(i in 1:length(transfer_vars)){
mod <- lmer(rel.growth ~ transfer + (1|Provenance)+ (1|BLK) + (1|year), data=ht_data_year_clim_long_transfer %>% filter(name==transfer_vars[i]))
pvalue<- as.data.frame(summary(mod)$coefficients)[2,]$`Pr(>|t|)`
Mr2 <- r2(mod)$R2_marginal

mod_nog <- lmer(rel.growth ~ transfer + (1|Provenance)+ (1|BLK) + (1|year), data=ht_data_year_clim_long_transfer_nog %>% filter(name==transfer_vars[i]))
pvalue_nog<- as.data.frame(summary(mod_nog)$coefficients)[2,]$`Pr(>|t|)`
Mr2_nog <- r2(mod_nog)$R2_marginal
...
relhtgrowth_mods_transfer %>% arrange(pvalue_nog) #SMRPB significant without Gila
...
for(i in 1:length(transfer_vars)){
mod <- lmer(rel.growth ~ transfer + I(transfer^2) + (1|Provenance) + (1|BLK) + (1|year), data=ht_data_year_clim_long_transfer %>% filter(name==transfer_vars[i]))
pvalues<- as.data.frame(summary(mod)$coefficients)[2:3,]$`Pr(>|t|)`
Cr2 <- performance::r2(mod)$R2_conditional
Mr2 <- performance::r2(mod)$R2_marginal
 
mod_nog <- lmer(rel.growth ~ transfer + I(transfer^2) + (1|Provenance) + (1|BLK) + (1|year), data=ht_data_year_clim_long_transfer_nog %>% filter(name==transfer_vars[i])) 
pvalues_nog<- as.data.frame(summary(mod_nog)$coefficients)[2:3,]$`Pr(>|t|)`
Cr2_nog <- performance::r2(mod_nog)$R2_conditional
Mr2_nog <- performance::r2(mod_nog)$R2_marginal
...
relhtgrowth_mods_quad %>% arrange(pvalue2_nog) #PC1transfer^2 significant


##significant vars
#####PC1
PC1_relht_mod_lm_nog <- lm(rel.growth ~ transfer + I(transfer^2), data=ht_data_year_clim_long_transfer_nog %>% filter(name=="PC1transfer"))
summary(PC1_relht_mod_lm_nog)

####make plot 
newdata_PC1_relht_nog <-  expand.grid(transfer=seq(min(ht_data_year_clim_long_transfer_nog %>% filter(name=="PC1transfer") %>% pull(transfer)), max(ht_data_year_clim_long_transfer_nog %>% filter(name=="PC1transfer") %>% pull(transfer)), length.out=50))
PC1_relht_predict_nog <- predict(PC1_relht_mod_lm_nog, newdata = newdata_PC1_relht_nog, interval="confidence")

predictdata_PC1_relht_nog <- cbind(newdata_PC1_relht_nog, PC1_relht_predict_nog) %>% mutate(var="PC1")
###

PC1_relht_plot <- ggplot()+
  #geom_vline(xintercept = prov_clim_adjusted_bi_long %>% filter(Provenance=="Plantation") %>% filter(name=="PC1") %>% pull(value), col="darkorange2")+
  #geom_text(aes(x=prov_clim_adjusted_bi_long %>% filter(Provenance=="Plantation") %>% filter(name=="PC1") %>% pull(value) + 12, y=12.5, label="plantation"), col="darkorange2")+
  geom_ribbon(data=predictdata_PC1_relht_nog, aes(x=transfer, ymin=lwr, ymax=upr), alpha=0.2)+
  geom_line(data=predictdata_PC1_relht_nog, aes(x=transfer,y=fit))+
  geom_rug(data=ht_growth_rel_clim %>% dplyr::select(c("PC1transfer","Provenance")) %>% mutate(fit=10) %>% filter(!Provenance=="Gila NF"), aes(x=PC1transfer,y=fit, col=factor(Provenance, levels=provs_bylat$Provenance)), linewidth=1, sides="b")+
...
##significant vars
#####smrpb
smrpb_relht_mod_lm_nog <- lm(rel.growth ~ transfer + I(transfer^2), data=ht_data_year_clim_long_transfer_nog %>% filter(name=="smrpb"))
summary(smrpb_relht_mod_lm_nog)

####make plot 
newdata_smrpb_relht_nog <-  expand.grid(transfer=seq(min(ht_data_year_clim_long_transfer_nog %>% filter(name=="smrpb") %>% pull(transfer)), max(ht_data_year_clim_long_transfer_nog %>% filter(name=="smrpb") %>% pull(transfer)), length.out=50))
smrpb_relht_predict_nog <- predict(smrpb_relht_mod_lm_nog, newdata = newdata_smrpb_relht_nog, interval="confidence")

predictdata_smrpb_relht_nog <- cbind(newdata_smrpb_relht_nog, smrpb_relht_predict_nog) %>% mutate(var="smrpb")
###

smrpb_relht_plot <- ggplot()+
  #geom_vline(xintercept = prov_clim_adjusted_bi_long %>% filter(Provenance=="Plantation") %>% filter(name=="smrpb") %>% pull(value), col="darkorange2")+
  #geom_text(aes(x=prov_clim_adjusted_bi_long %>% filter(Provenance=="Plantation") %>% filter(name=="smrpb") %>% pull(value) + 12, y=12.5, label="plantation"), col="darkorange2")+
  geom_ribbon(data=predictdata_smrpb_relht_nog, aes(x=transfer, ymin=lwr, ymax=upr), alpha=0.2)+
  geom_line(data=predictdata_smrpb_relht_nog, aes(x=transfer,y=fit))+
  #geom_rug(data=ht_growth_rel_clim %>% dplyr::select(c("smrpb","Provenance")) %>% mutate(fit=10) %>% filter(!Provenance=="Gila NF"), aes(x=smrpb,y=fit, col=factor(Provenance, levels=provs_bylat$Provenance)), linewidth=1, sides="b")+
  scale_color_manual(values=unname(stepped())[-1], name="Provenance")+
  xlab("smrpb Transfer")+
  ylab("Relative Height Growth Rate (%/year)")+
  facet_grid(~var)+
...
mod_cones_pca_nog_dredge #null is best

##predictions
#no gila
best.mod.cone.clim_nog <- glmmTMB(cbind(n_cones, total-n_cones) ~ height + pcadist + (1|code) + (1|year) + (1|BLK), data= moniger_subset_noG, family = binomial)
...
newmod <- glmmTMB(cbind(n_cones, total-n_cones) ~  height + pcadist, data= moniger_subset_noG, family = binomial)
summary(newmod)
...
png("figure_summary.png", width=275, height=225, unit="mm", res=600)
...
####PC only mods
```{r}
survival_byyr_pc <- moniger_survival_byyr_binom %>% 
  mutate(plant_PC1 = plant_pca_scores$PC1[1], plant_PC2 = plant_pca_scores$PC2[1]) %>% 
  mutate(PC1transfer = plant_PC1 - PC1, PC2transfer = plant_PC2 - PC2) %>% 
  mutate(absPC1transfer = abs(PC1transfer), absPC2transfer = abs(PC2transfer)) %>% 
  mutate(pcadist2d = sqrt(absPC1transfer^2 + absPC2transfer^2))

survival_byyr_pc_nog <- survival_byyr_pc %>% filter(!Provenance%in%c("Gila NF"))

##no gila
survmod_pc_nog<- glmmTMB(cbind(alive,died) ~ PC1transfer*PC2transfer + I(PC1transfer^2) + I(PC2transfer^2) + pcadist2d + (1|Provenance) + (1|BLK) + (1|year), data=survival_byyr_pc_nog, family=binomial)

summary(survmod_pc_nog) #nothing significant
...
survival_byyr_transfer <- survival_byyr_pc %>% 
  left_join(prov_pcapts_big) %>% 
  dplyr::select(c(Provenance, BLK, year, alive, died, all_of(provenance_climvars))) %>% 
  pivot_longer(CMD:monsoonality) %>% 
  left_join(plantation_climNA, by=c("name"="var")) %>% 
  mutate(transfer = plant_value - value) %>% 
  bind_rows(survival_byyr_pc[,c("BLK","Provenance","year","alive","died","PC1transfer","PC2transfer","pcadist2d")] %>% 
  pivot_longer(PC1transfer:pcadist2d, names_to="name", values_to="transfer")) 

survival_byyr_transfer_nog <- survival_byyr_transfer %>% filter(!Provenance=="Gila NF")

surv_mods<- data.frame()

for(i in 1:length(transfer_vars)){
mod <- glmmTMB(cbind(alive,died) ~ transfer + (1|Provenance) + (1|BLK) + (1|year), data=survival_byyr_transfer %>% filter(name==transfer_vars[i]), family=binomial)
pvalue<- as.data.frame(summary(mod)$coefficients$cond)[2,]$`Pr(>|z|)`
Mr2=r2(mod)$R2_marginal

mod_nog <- glmmTMB(cbind(alive,died) ~ transfer + (1|Provenance)+ (1|BLK) + (1|year), data=survival_byyr_transfer_nog %>% filter(name==transfer_vars[i]), family=binomial)
pvalue_nog<- as.data.frame(summary(mod_nog)$coefficients$cond)[2,]$`Pr(>|z|)`
Mr2_nog=r2(mod_nog)$R2_marginal


surv_mods <- bind_rows(surv_mods, data.frame(var=transfer_vars[i], pvalue = pvalue, Mr2=Mr2, pvalue_nog = pvalue_nog, Mr2_nog = Mr2_nog))}

surv_mods %>% arrange(pvalue_nog) ##NFFD & smrpb 

#NFFD
#without Gila is still significant
surv_NFFD_mod_nog <- glmmTMB(cbind(alive,died) ~ scale(transfer) + (1|Provenance)+(1|BLK) + (1|year), data=survival_byyr_transfer_nog %>% filter(name=="NFFD"), family="binomial")
summary(surv_NFFD_mod_nog)

surv_NFFD_mod_nog_glm <- glmmTMB(cbind(alive,died) ~ scale(transfer), data=survival_byyr_transfer_nog %>% filter(name=="NFFD"), family="binomial")
summary(surv_NFFD_mod_nog)

##predictions
surv_NFFD_data_nog <- survival_byyr_transfer_nog %>% filter(name=="NFFD") 

survNFFD_nog_newdata1 <- expand.grid(transfer = seq(min(surv_NFFD_data_nog$transfer),max(surv_NFFD_data_nog$transfer),length.out=50))

survNFFD_nog_newdata2 <- cbind(survNFFD_nog_newdata1, predict(surv_NFFD_mod_nog_glm, newdata = survNFFD_nog_newdata1, type = "link", se=TRUE))
survNFFD_nog_newdata3 <- within(survNFFD_nog_newdata2, {
    PredictedProb <- plogis(fit)
    LL <- plogis(fit - (1.96 * se.fit))
    UL <- plogis(fit + (1.96 * se.fit))
})

ggplot(survNFFD_nog_newdata3, aes(x = transfer, y = PredictedProb)) + geom_ribbon(aes(ymin = LL,
    ymax = UL), alpha = 0.2) + 
  geom_line(linewidth = 1)+
  xlab("NFFD")
#########
...
surv_smrpb_mod_nog <- glmmTMB(cbind(alive,died) ~ scale(transfer) + (1|Provenance)+(1|BLK) + (1|year), data=survival_byyr_transfer_nog %>% filter(name=="smrpb"), family="binomial")
summary(surv_smrpb_mod_nog)

##predictions
surv_smrpb_data_nog <- survival_byyr_transfer_nog %>% filter(name=="smrpb") 
survsmrpb_nog_newdata1 <- expand.grid(transfer = seq(min(surv_smrpb_data_nog$transfer),max(surv_smrpb_data_nog$transfer),length.out=50))

surv_smrpb_mod_nog_glm <- glmmTMB(cbind(alive,died) ~ scale(transfer), data=survival_byyr_transfer_nog %>% filter(name=="smrpb"), family="binomial")
...
ggplot(survsmrpb_nog_newdata3, aes(x = transfer, y = PredictedProb)) + geom_ribbon(aes(ymin = LL,
    ymax = UL), alpha = 0.2) + 
  geom_line(linewidth = 1)+
  xlab("smrpb")
#########
...
for(i in 1:length(transfer_vars)){
mod <- glmmTMB(cbind(alive,died) ~ scale(transfer) + I(scale(transfer)^2) + (1|Provenance) + (1|BLK) + (1|year), data=survival_byyr_transfer %>% filter(name==transfer_vars[i]), family="binomial")
pvalues<- as.data.frame(summary(mod)$coefficients$cond)[2:3,]$`Pr(>|z|)`
Mr2 <- r2(mod)$R2_marginal

mod_nog <- glmmTMB(cbind(alive,died) ~ scale(transfer) + I(scale(transfer)^2) + (1|Provenance) + (1|BLK) + (1|year), data=survival_byyr_transfer_nog %>% filter(name==transfer_vars[i]), family="binomial")
pvalues_nog<- as.data.frame(summary(mod_nog)$coefficients$cond)[2:3,]$`Pr(>|z|)`
Mr2_nog <- r2(mod_nog)$R2_marginal

surv_mods_quad <- bind_rows(surv_mods_quad, data.frame(var=transfer_vars[i], pvalue1 = pvalues[1], pvalue2 = pvalues[2], Mr2 = Mr2, pvalue1_nog = pvalues_nog[1], pvalue2_nog = pvalues_nog[2], Mr2_nog = Mr2_nog))

}

surv_mods_quad %>% arrange(pvalue2_nog) ##cmd

##CMD
#without Gila is still significant
surv_CMD_mod2_nog <- glmmTMB(cbind(alive,died) ~ scale(transfer) + I(scale(transfer)^2) + (1|Provenance) + (1|BLK) + (1|year), data=survival_byyr_transfer_nog %>% filter(name=="CMD"), family="binomial")
summary(surv_CMD_mod2_nog)
r2(surv_CMD_mod2_nog)

surv_CMD_mod2_nog_glm <- glmmTMB(cbind(alive,died) ~ scale(transfer) + I(scale(transfer)^2), data=survival_byyr_transfer_nog %>% filter(name=="CMD"), family="binomial")

##predictions
survCMD2_newdata1_nog <- expand.grid(transfer = seq(min(survival_byyr_transfer %>% filter(name=="CMD") %>% filter(!Provenance=="Gila NF") %>% pull(transfer)),max(survival_byyr_transfer %>% filter(name=="CMD") %>% filter(!Provenance=="Gila NF") %>% pull(transfer)),length.out=50))
...
ggplot(survCMD2_newdata3_nog, aes(x = transfer, y = PredictedProb)) + geom_ribbon(aes(ymin = LL,
    ymax = UL), alpha = 0.2) + 
  geom_line(linewidth = 1)+
  xlab("CMD")+
    ylab("Predicted Probability")+
...
#NFFD
surv_NFFD_mod2014 <- glmmTMB(cbind(alive,died) ~ scale(climvar.value) + (1|Provenance)+ (1|BLK), data=moniger_survival2014_long %>% filter(climvar.name=="NFFD"), family="binomial")
summary(surv_NFFD_mod2014)

surv_NFFD_data2014 <- moniger_survival2014_long %>% filter(climvar.name=="NFFD")

newdata_NFFD_surv2014 <- data.frame(
    climvar.value = seq(min(surv_NFFD_data2014$climvar.value), 
                      max(surv_NFFD_data2014$climvar.value), 
                      length.out = 100
                      ))

##predictions
surv2014NFFD_newdata1 <-  expand.grid(climvar.value = seq(min(newdata_NFFD_surv2014$climvar.value),max(newdata_NFFD_surv2014$climvar.value),length.out=50))

surv2014_NFFD_mod_glm <- glmmTMB(cbind(alive,died) ~ scale(climvar.value), data=moniger_survival2014_long %>% filter(climvar.name=="NFFD"), family="binomial")

surv2014_NFFD_newdata2 <- cbind(surv2014NFFD_newdata1, predict(surv2014_NFFD_mod_glm, newdata = surv2014NFFD_newdata1, type = "link", se=TRUE))
surv2014_NFFD_newdata3 <- within(surv2014_NFFD_newdata2, {
    PredictedProb <- plogis(fit)
    LL <- plogis(fit - (1.96 * se.fit))
    UL <- plogis(fit + (1.96 * se.fit))
})
...
surv2014_NFFD_mod_nog_glm <- glmmTMB(cbind(alive,died) ~ scale(climvar.value), data=moniger_survival2014_long %>% filter(climvar.name=="NFFD") %>% filter(!Provenance=="Gila NF"), family="binomial")

##predictions
surv2014_NFFD_data_nog <- moniger_survival2014_long %>% filter(climvar.name=="NFFD") %>% filter(!Provenance=="Gila NF")

surv2014_NFFD_nog_newdata1 <- expand.grid(climvar.value = seq(min(surv2014_NFFD_data_nog$climvar.value),max(surv2014_NFFD_data_nog$climvar.value),length.out=50))
...
surv2014_NFFD_bothmod_data<- bind_rows(surv2014_NFFD_newdata3 %>% mutate(model="with Gila NF"),
surv2014_NFFD_nog_newdata3 %>% mutate(model="without Gila NF"))

ggplot(surv2014_NFFD_bothmod_data, aes(x = climvar.value, y = PredictedProb, linetype=model)) + geom_ribbon(aes(ymin = LL,
    ymax = UL), alpha = 0.2) + 
  geom_line(linewidth = 1)+
  xlab("NFFD")+
  theme_bw()
...
surv2014_mods_quad %>% arrange(pvalue2_nog) ##CMD, MCMT
...
surv2014_mods_transfer %>% arrange(p.value_nog) #RH, CMD, NFFD, MWMT 
...
summary(surv2014_NFFD_mod_nog) #provenances with NFFD more similar to the plantation have higher probability of survival
r2(surv2014_NFFD_mod_nog)
plot(ggpredict(surv2014_NFFD_mod_nog, terms = "diff_plant"))+
  xlab("NFFD difference from plantation")+
  ylab("Survival Probability")+ggtitle("NFFD")
