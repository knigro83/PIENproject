...
write.csv(prov_clim_adjusted_bi, "PIEN_published_code/prov_clim_adjusted_bi.csv")
...
##plot NFFD x CMD and smrpb x CMD

ggplot(prov_clim_adjusted_bi, aes(x=CMD, y=NFFD))+
  geom_point()
```
...
write.csv(monthly_clim, "monthly_clim.csv")
...
prov_climvars_plot<- ggplot(prov_vars, aes(x=factor(code, levels=provs_bylat$code), y=value, col=factor(code, levels=provs_bylat$code)))+
  geom_point(size=4)+
  ...
  theme_bw()+
  theme(axis.text.x = element_text(angle=55,hjust=1, size=10), legend.position = 'none', strip.text=element_text(size=12), axis.title.x = element_blank(), axis.text.y = element_text(size=12), axis.title.y=element_text(size=12))

png("figures/final/prov_climvars_plot.png", width=20, height=9, units="in", res=500)
prov_climvars_plot
dev.off()

ggplot(prov_vars %>% filter(!Provenance=="Gila NF") %>% filter(var=="smrpb"), aes(x=factor(Provenance, levels=provs_bylat$Provenance), y=value, col=factor(Provenance, levels=provs_bylat$Provenance)))+
  geom_point(size=4)+
...
ggplot(prov_vars %>% filter(var=="MCMT" & !Provenance=="Plantation"), aes(x=factor(code, levels=provs_bylat$code), y=value, col=factor(code, levels=provs_bylat$code)))+
    geom_hline(data=prov_vars %>% filter(Provenance=="Plantation"& var=="MCMT"),aes(yintercept = value, group=var), linetype=2, linewidth=1)+
  geom_point(size=4)+
  facet_wrap(~var, scales="free_y")+
  scale_color_manual(values=unname(stepped()), name="Provenance")+
  theme_bw()+
...
ggplot(prov_vars %>% filter(var=="smrpb" & !Provenance=="Plantation"), aes(x=factor(code, levels=provs_bylat$code), y=value, col=factor(code, levels=provs_bylat$code)))+
    geom_hline(data=prov_vars %>% filter(Provenance=="Plantation"& var=="smrpb"),aes(yintercept = value, group=var), linetype=2, linewidth=1)+
  geom_point(size=4)+
  facet_wrap(~var, scales="free_y")+
  scale_color_manual(values=unname(stepped()), name="Provenance")+
  theme_bw()+
...
prov_vars_english <- prov_vars %>% filter(var %in% c("smrpb","NFFD","MAP","MCMT")) %>% 
  mutate(value = case_when(var=="MCMT" ~ value*(9/5) + 32,
                           var=="MAP" ~ value/25.4,
                          TRUE ~ value))

ggplot(prov_vars_english %>% filter(!Provenance=="Plantation"), aes(x=factor(code, levels=provs_bylat$code), y=value, col=factor(code, levels=provs_bylat$code)))+
    geom_hline(data=prov_vars_english %>% filter(Provenance=="Plantation"),aes(yintercept = value, group=var), linetype=2, linewidth=1)+
  geom_point(size=4)+
  facet_wrap(~factor(var, levels=c("MCMT","MAP","NFFD","smrpb"),
                     labels = c("Mean Temp Coldest Month (F)", "Mean Annual Precip (in)", "# Frost-free Days","Summer-Spring Precip Ratio")), scales="free_y")+
  scale_color_manual(values=unname(stepped()), name="Provenance")+
  ylab("")+
  theme_bw()+
  theme(axis.text.x = element_text(angle=55,hjust=1, size=10), legend.position = 'none', strip.text=element_text(size=12), axis.title.x = element_blank(), axis.text.y = element_text(size=12), axis.title.y=element_text(size=12))
...
ppt_comb_plantation <- bind_rows(as.data.frame(ppt_data_1969) %>% filter(CN == "Plantation") %>%
  mutate(date=as.character(as.Date(paste(month,"01",year,sep="/"), "%m/%d/%Y")), ppt=as.numeric(ppt), year=as.integer(year), month=as.integer(month)), ppt_comb_plantation) 

tmax_comb_plantation <- bind_rows(as.data.frame(tmax_data_1969) %>% filter(CN == "Plantation") %>%
  mutate(date=as.character(as.Date(paste(month,"01",year,sep="/"), "%m/%d/%Y")), tmax=as.numeric(tmax), year=as.integer(year), month=as.integer(month)), tmax_comb_plantation)

tmean_comb_plantation <- bind_rows(as.data.frame(tmean_data_1969) %>% filter(CN == "Plantation") %>%
  mutate(date=as.character(as.Date(paste(month,"01",year,sep="/"), "%m/%d/%Y")), tmean=as.numeric(tmean), year=as.integer(year), month=as.integer(month)), tmean_comb_plantation) 

tmin_comb_plantation <- bind_rows(as.data.frame(tmin_data_1969) %>% filter(CN == "Plantation") %>%
  mutate(date=as.character(as.Date(paste(month,"01",year,sep="/"), "%m/%d/%Y")), tmin=as.numeric(tmin), year=as.integer(year), month=as.integer(month)), tmin_comb_plantation) 

vpdmax_comb_plantation <- bind_rows(as.data.frame(vpdmax_data_1969) %>% filter(CN == "Plantation") %>%
  mutate(date=as.character(as.Date(paste(month,"01",year,sep="/"), "%m/%d/%Y")), vpdmax=as.numeric(vpdmax), year=as.integer(year), month=as.integer(month)), vpdmax_comb_plantation) 
...
plantation_seasonal_plot<- ggplot(plantation_clim_long_currentyr %>% filter(name2 %in% c("ppt","tmin","tmean","tmax")), aes(x=currentyr, y=value, col=type))+
...
png("figures/final/plantation_clim.png", width=6, height=4, res=300, units="in")
plantation_seasonal_plot
dev.off()
...
drought_xtreme<- ggplot(plantation_clim_long_qs %>% 
  mutate(extreme = case_when(name2 %in% c("ppt","aet") & value < out.low ~ "yes",
                             name2 %in% c("cwd","spei","tmax","tmean","tmin","vpdmax") & value > out.hi ~"yes",
  ...
  theme_bw()+
  xlab("year")+
  theme(strip.text=element_text(size=12), legend.text=element_text(size=14), legend.title=element_text(size=14), axis.text.x=element_text(size=12), axis.text.y=element_text(size=12), axis.title.x=element_text(size=12), axis.title.y=element_text(size=12))

png("figures/final/drought_xtreme.png", width=11, height=9, res=300, units="in")
drought_xtreme
dev.off()
...
pc1_pc2_niche_plot <- ggplot(as.data.frame(clim_pca_big$x[-(1:5),]), aes(x=PC1, y=PC2))+
  ...
  geom_text(data = as.data.frame(loadings_pc1pc2 * scale) %>% 
              mutate(name=rownames(loadings_pc1pc2)),
               ...
               color = "black", size=2, hjust=c(.1,.1,.1,.1,.8,.1,1,-.3,-.2,1.2), vjust=c(1.2,1.2,-1,1.4,1.2,1.2,1,1,1,1.2))+
...
```{r}
##plot of pien climate niche with plantation points

ggplot(clim_pien, aes(x=NFFD, y=CMD))+
  geom_density_2d_filled()+
  geom_point(data=plantation_all %>% filter(period=="1970s"), size=3, color="white")+
  #geom_point(data=plantation_all %>% filter(period=="2080s"), size=3, color="pink3")+
  #geom_point(data=prov_clim_adjusted_bi %>% filter(!Provenance=="Plantation"), size=2, color="gray70")+
  #geom_point(data=prov_clim_adjusted_bi %>% filter(Provenance %in% c("Larimer 2","Routt NF","Gunnison NF","Dixie NF","Wallowa-Whitman NF")), size=2, color="orange")+
  theme_bw()+
  xlab("# Frost-free days")+
  ylab("Climate Moisture Deficit")
...
png("figures/final/map_and_bigpca_bluewhite.png", width=9, height=7, res=500, units = "in")
ggarrange(map,ggarrange(pc1_pc2_niche_plot, contrib.plot, nrow=2, ncol=1, labels=c("B)","C)")), labels=c("A)",""), ncol=2, nrow=1)
dev.off()

##map without white and blue spruce ranges
...
png("figures/map_simple.png", width=3, height=3, res=300, units = "in")
map_simple
dev.off()
...
ggplot(dbh_numeric14, aes(x=smrpb, y=DBH))+
  geom_point()+
  theme_bw()+
  xlab("Provenance Summer to Spring Precip Ratio")+
  ylab("DBH (cm)")+
  ggtitle("2014 Diameter at Breast Height (DBH)")
...
ht_data <- moniger_data_comb %>% 
  ...
  dplyr::select(c(LOC,BLK,REP,Provenance,Notes85,HT74:HT14,Seedling.Ht..from.Nursery)) %>% 
...
ht_data_aves <- ht_data %>% filter(!name=="HT74") %>% 
  ...
  summarise(meantotht = mean(value), seTH = sd(value)/sqrt(n()), n=n())

ht_data_FD <- moniger_data_comb %>% 
  filter(! (LOC==30 & BLK=="D" & REP==3)) %>%  ##excluding Gila tree that was outlier for height in 1996
  dplyr::select(c(LOC,BLK,REP,Provenance,Notes85,HT74:HT14,Seedling.Ht..from.Nursery)) %>% 
  pivot_longer(HT74:HT14) %>% 
  mutate(nursery_ht_m = Seedling.Ht..from.Nursery/39.37) %>% 
...
ggplot(data=ht_data_FD %>% filter(name=="HT14"), aes(x=name, y=value, fill=Provenance))+
  geom_boxplot()
...
ht_data_unique_tree<- ht_data_year %>% 
  mutate(tree = paste0(LOC,BLK,REP))
  

ggplot(data=ht_data_unique_tree, aes(x=year, y=value, color=tree))+
  geom_point()+
  geom_line()+
...
ggplot(data= moniger_data_comb %>% 
   group_by(Provenance, Form85) %>% 
   summarise(n=n()) %>% 
     filter(!is.na(Form85)), aes(x=factor(Form85), y=n, fill=factor(Provenance, levels=provs_bylat$Provenance)))+
     geom_bar(stat="identity", position="dodge")+
   scale_fill_manual(values=unname(stepped()))
  
DT_trees <- moniger_data_comb %>% 
  pivot_longer(Form85:Form06) %>% 
  filter(value == 3) %>% 
  mutate(tree=paste0(LOC,BLK,REP)) %>% 
  pull(tree) %>% 
  unique()

no_DT_aveht <- ht_data_unique_tree %>% 
  filter(year==2014) %>% 
  filter(!tree %in% DT_trees) %>% 
  group_by(Provenance) %>% 
  summarise(mean=mean(value), se = sd(value)/sqrt(n()), n=n()) %>% 
  mutate(percent = n/48)

ggplot(no_DT_aveht,aes(x=factor(Provenance, levels=provs_bylat$Provenance), col=factor(Provenance, levels=provs_bylat$Provenance), y=mean))+
  geom_point(size=3)+
  geom_errorbar(aes(ymin=mean-se, ymax=mean+se), linewidth=1)+
  scale_color_manual(values=unname(stepped()), name="Provenance")+
  theme_bw()+
  theme(axis.text.x=element_text(angle=45, hjust=1), legend.position="none")+
  ylab("Mean 2014 Height (m)")+
  xlab("")+
  ggtitle("Trees with no record of `dead top`")

ggplot(ht_data_aves %>% filter(name=="HT14"),aes(x=factor(Provenance, levels=provs_bylat$Provenance), col=factor(Provenance, levels=provs_bylat$Provenance), y=meantotht  ))+
  geom_point(size=3)+
  geom_errorbar(aes(ymin=meantotht  -seTH     , ymax=meantotht  +seTH     ), linewidth=1)+
  scale_color_manual(values=unname(stepped()), name="Provenance")+
  theme_bw()+
  theme(axis.text.x=element_text(angle=45, hjust=1), legend.position="none")+
  ylab("Mean 2014 Height (m)")+
...
ggplot(no_DT_aveht, aes(x=factor(Provenance, levels=provs_bylat$Provenance), y=percent, fill=factor(Provenance, levels=provs_bylat$Provenance)))+
  geom_bar(stat="identity", position="dodge")+
  scale_fill_manual(values=unname(stepped()))+
  ylab("% trees with no dead tops recorded")+
  xlab("")+
  theme_bw()+
  theme(axis.text.x=element_text(angle=45, hjust=1), legend.position = "none")


View(ht_data_unique_tree %>% 
  filter(year==2014) %>% 
  filter(!tree %in% DT_trees) %>% 
    filter(Provenance=="Gila NF"))
...
ggplot(data=nursery_ht_clim_long %>% filter(name=="smrpb") %>% mutate(model="with Gila"), aes(x=value, y=nursery_ht_m, color=model))+
  geom_smooth(method="lm")+
    geom_point(data=nursery_ht_clim_long %>% filter(name=="smrpb") %>% mutate(model="with Gila"))+
  geom_smooth(method="lm", data=nursery_ht_clim_long_nog %>% filter(name=="smrpb") %>% mutate(model="without Gila"), aes(x=value, y=nursery_ht_m, color=model))+
  scale_color_manual(values=c("grey","black"))+
  geom_point(data=nursery_ht_clim_long_nog %>% filter(name=="smrpb") %>% mutate(model="without Gila"))+
  theme_bw()+
  xlab("Summer to Spring Precip Ratio")+
  ylab("Nursery Height (m)")+
  ggtitle("Average Provenance Nursery Height")
...
nursery_ht_plot<- ggplot()+
  geom_text(data=nursery_stats, aes(x=x, y=max-0.1, label=paste0("p = ", round(p,2))), size=4)+
...
png("figures/final/nursery_ht_plot.png", height=6, width=9, res=500, units="in")
nursery_ht_plot
dev.off()
```
...
png("figures/final/rank_plot.png",height=5.5, width=8.5, res=300, units="in")
rank_plot
dev.off()
```
...
ggplot(ht_data_year_clim_long_transfer %>% filter(name=="smrpb", year==2014), aes(x=value, y=ht))+
  geom_point()+
  theme_bw()+
  xlab("Provenance Summer to Spring Precip Ratio")+
  ylab("Height (m)")+
  ggtitle("2014 Height (m)")
```
...
###no dead tops
NODT_ht_growth_rel_data_forplot<- ht_growth_rel_data %>% 
  mutate(tree=paste0(LOC,BLK,REP)) %>% 
  filter(!tree %in% DT_trees) %>% 
  dplyr::select(Provenance, name, value) %>% 
    mutate(year= ifelse(name %in% c("growth79rel","growth85rel","growth91rel","growth96rel"), as.numeric(paste0("19",substr(name, 7,8))), as.numeric(paste0("20",substr(name, 7,8))))) %>% 
  group_by(Provenance, year, name) %>% 
  summarise(meangrowth=mean(value))

ggplot()+
  geom_bar(data=NODT_ht_growth_rel_data_forplot, aes(x=year, y=meangrowth/5, fill = factor(Provenance, levels=provs_bylat$Provenance)), stat="identity")+
  #geom_point(data=ht_3meas, aes(x=year, y=meanht), col="black", size=2)+
  #geom_line(data=ht_3meas, aes(x=year, y=meanht), col="black")+
  #geom_boxplot(data=height_growth_forplot, aes(x=year, y=value*10, fill=factor(Provenance, levels=provs_bylat$Provenance), group=year), outliers=FALSE)+
  scale_y_continuous(name = "Height (m)",sec.axis = sec_axis( transform=~.*5, name="Relative Height Growth Rate (%/yr)")) +
...
relgrowth_NODT<- NODT_ht_growth_rel_data_forplot %>% 
  group_by(Provenance) %>% 
  summarise(mean=mean(meangrowth))

ggplot(data=relgrowth_NODT, aes(x=factor(Provenance, levels=provs_bylat$Provenance), y=mean, col=factor(Provenance, levels=provs_bylat$Provenance)))+
  geom_point(size=3)+
  scale_color_manual(values=unname(stepped()))+
  theme_bw()+
  ylab("Mean relative height growth rate (% per year)")+
  theme(axis.text.x = element_text(angle=45, hjust=1), legend.position = "none")+
  xlab("")+
  ggtitle("Trees with no record of 'dead top'")
...
cone_effects_plot<- ggarrange(cone_plot_height,cones_pptsm_plot, cones_TD_plot, ncol=3, labels=c("A)","B)","C)"),nrow=1)

png("figures/final/cone_effects_plot.png", width=9, height=3.5, units="in", res=500)
cone_effects_plot
dev.off()


##plot with vertical line
ggplot(cone_ht_pptsm_quad_nog_newdata3, aes(x = transfer, y = PredictedProb)) +
  geom_ribbon(aes(ymin = LL,ymax = UL), alpha = 0.2) + 
  geom_line(linewidth = 1)+
  geom_rug(data=cone_clim_long_transfer_nog %>% filter(name=="PPTsm") %>% mutate(PredictedProb=0.5), sides="b", linewidth=1, color="black", alpha=0.5)+
...
###checking Shepperd paper numbers
moniger_data_comb %>% filter(Stat79 ==1) %>% group_by(Provenance) %>% summarise(n=n()) %>% mutate(percent=n/48*100) %>% pull(percent) %>% mean()
####
...
transfer_effects_plot<- ggarrange(cmd2_surv14_plot,smrpb_surv14_plot,nffd_surv14_plot, pcadist2d_relht_plot, ncol=2, nrow=2, labels=c("A)","B)","C)","D)"))

png("figures/final/transfer_effects_plot.png", width=7.5, height=7, units="in", res=500)
transfer_effects_plot
dev.off()
...
recent_means_fig<- ggplot(recent_means, aes(x=factor(Provenance, levels=provs_bylat$Provenance, labels=provs_bylat$code), y=value, col=factor(Provenance, levels=provs_bylat$Provenance, labels=provs_bylat$code)))+
  geom_point(size=3)+
...
png("figures/final/recent_means_fig.png", width=8, height=5.5, units="in", res=500)
recent_means_fig
dev.off()

###english units

recent_means_english<- survival_means %>% filter(year==2014) %>% dplyr::select(Provenance, prop.surv.since.1970) %>% 
  left_join(ht_data_aves_forfig %>% ungroup() %>% filter(name=="HT14") %>% dplyr::select(Provenance, meantotht, seTH)) %>% 
  left_join(dbh_aves %>% filter(year==2014) %>% dplyr::select(Provenance, meanDBH, se_DBH)) %>% 
  left_join(cone_sum_forfig %>% filter(year==2006) %>% dplyr::select(Provenance, prop.w.cones)) %>% 
  pivot_longer(c(prop.surv.since.1970,meantotht,meanDBH,prop.w.cones)) %>% 
  mutate(seTH = ifelse(name=="meantotht", seTH, NA), se_DBH = ifelse(name=="meanDBH", se_DBH, NA)) %>%  
  mutate(value = case_when(name=="meantotht" ~ value*3.281,
                           name=="meanDBH" ~ value/2.54,
                           TRUE ~ value),
         seTH = seTH*3.281,
         se_DBH = se_DBH/2.54) %>% 
  mutate(name = case_when(name=="meanDBH" ~ "DBH (in) - 2014",
                          name=="meantotht" ~ "Height (ft) - 2014",
                          name=="prop.surv.since.1970" ~ "Proportion Survival 1970 - 2014",
                          name=="prop.w.cones" ~ "Proportion with Cones - 2006")) 

recent_means_fig_english<- ggplot(recent_means_english, aes(x=factor(Provenance, levels=provs_bylat$Provenance, labels=provs_bylat$code), y=value, col=factor(Provenance, levels=provs_bylat$Provenance, labels=provs_bylat$code)))+
  geom_point(size=3)+
  geom_errorbar(aes(ymax=value+se_DBH, ymin=value-se_DBH))+
  geom_errorbar(aes(ymax=value+seTH, ymin=value-seTH))+
  facet_wrap(~name, scales="free_y")+
  ...
  xlab("Provenance")+
  theme_bw()+
  theme(axis.text.x = element_text(angle=45, hjust=1, color="black"), axis.title.y=element_blank(),
        axis.text.y = element_text(color="black"),
        strip.text = element_text(size=12, color="black"))
  
png("figures/recent_means_fig_english.png", width=7.8, height=5.5, units="in", res=500)
recent_means_fig_english
dev.off()


##figure out a transfer limit

surv_means_transfer <- moniger_survival_byyr_binom_zeros_2014 %>% 
  group_by(Provenance, year) %>% 
  summarise(alive=sum(alive), died=sum(died)) %>% 
  mutate(prop.surv=alive/(alive+died)) %>% 
  left_join(survival14_transfer %>% 
  ungroup() %>% 
  dplyr::select(Provenance, name, value, transfer) %>% 
  unique(), by="Provenance") %>% 
  mutate(surv.percentile = prop.surv/0.9375)

ggplot(surv_means_transfer %>% filter(name=="NFFD") %>% filter(!Provenance=="Gila NF"))+
  geom_point(aes(x=transfer, y=surv.percentile))



surv2014_NFFD_nog_newdata3_scaled<- surv2014_NFFD_nog_newdata3 %>% 
  mutate(scaled_surv = PredictedProb/max(surv2014_NFFD_nog_newdata3$PredictedProb))

nffd_surv14_plot_scaled <- ggplot(surv2014_NFFD_nog_newdata3_scaled, aes(x = transfer, y = scaled_surv)) + #geom_ribbon(aes(ymin = LL,
 #   ymax = UL), alpha = 0.2) + 
  geom_line(linewidth = 1)+
    geom_vline(xintercept = 0, color="black", linetype=2)+
    geom_rug(data=survival14_transfer %>% filter(name=="NFFD") %>% filter(!Provenance=="Gila NF") %>%  mutate(fit=0.7), aes(x=transfer,y=fit), linewidth=1, sides="b")+
  ...
  geom_vline(xintercept = 39.706, col="red")+
  xlab("NFFD transfer")+
      ylab("Predicted Probability of Survival")+
  theme_bw()

nffd_surv14_plot_scaled ##to be in the top 10% of survival, NFFD transfer can be +40FFD.
...
rwi_by_prov<- ggplot(growth.detrended.byprov, aes(x=year, y=meanrwi-1))+
  ...
  geom_text(data=gini_sum, x=2011, y=0.7, aes(label=paste0("gini = ", round(mean,2))))+
  facet_wrap(~factor(Provenance, levels=provs_bylat$Provenance))+
  theme_bw()+
  theme(axis.text = element_text(color="black",size=10), axis.title = element_text(color="black", size=12), panel.grid = element_blank(), strip.text = element_text(size=12, color="black"), legend.text=element_text(size=14), legend.title=element_text(size=16))

png("figures/final/rwi_byprov.png", width=18, height=10, res=500, units="in")
rwi_by_prov
dev.off()
...
rwi_simple_plot<- ggplot(growth.detrended.ave, aes(x=year, y=meanrwi-1))+
  ...
  geom_hline(yintercept=0, linetype=2)+
  theme_bw()+
  theme(axis.text = element_text(color="black",size=10), axis.title = element_text(color="black", size=12), panel.grid = element_blank(), legend.text=element_text(size=11))

png("figures/final/rwi_simple_plot.png", width=7, height=5, units="in", res=500)
rwi_simple_plot
dev.off()
...
cor_means_fig <- ggplot(cor_means, aes(x=factor(time_season, levels=c("priorsp","priorsm","priorfa","priorwt","NAsp","NAsm"), labels= c("priorsp","priorsm","priorfa","priorwt","sp","sm")), y=estimate,col=mean.cor>0))+
...
png("figures/final/cor_means_fig.png", width=6.5, height=5, units="in", res=500)
cor_means_fig
dev.off()

###smaller version
vpd_cor_fig<- ggplot(cor_means %>% filter(var=="vpdmax"), aes(x=factor(time_season, levels=c("priorsp","priorsm","priorfa","priorwt","NAsp","NAsm"), labels= c("priorsp","priorsm","priorfa","priorwt","sp","sm")), y=estimate,col=mean.cor>0))+
  geom_point(position=position_dodge(1), size=3)+
  geom_errorbar(aes(ymin=lwr, ymax=upr),width=0.5)+
  geom_text(aes(y=0.25, label=sig), size=5)+
  scale_color_manual(values=c("red","black"))+
  scale_shape_manual(values=c(1,13))+
  geom_hline(yintercept=0)+
  facet_wrap(~factor(var, labels=c("Max Vapor Pressure Deficit")), nrow=3)+
  theme_bw()+
  ylim(c(-0.15, 0.3))+
  xlab("")+ylab("Mean Pearson's r")+
...
png("figures/cor_means_fig_justvpd.png", width=4, height=3, units="in", res=500)
vpd_cor_fig
dev.off()
...
gc_cors_allprovs<- ggplot(data=sig.cors.byprov, aes(x=factor(code, levels=provs_bylat$code), y=estimate))+
...
png("figures/final/gc_cors_allprovs.png", width=19, height=12, units="in", res=500)
gc_cors_allprovs
dev.off()
```
...
gc_cor_fig<- ggarrange(cwd_sp_MCMT_plot, cwd_sp_CMD_plot, cwd_sp_pcadist2d_plot, nrow=1, common.legend = TRUE)

png("figures/final/gc_cor_fig.png", width=8, height=3.5, units="in", res=500)
gc_cor_fig
dev.off()
```
