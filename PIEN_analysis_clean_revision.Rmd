...
provenance_climvars <- c(colnames(big_pca_data),"PC1","PC2")
...
dbh_gr_mods %>% arrange(pvalue_nog) #nothing significant
...
dbh_gr_mods_quad %>% arrange(pvalue2_nog) #nothing significant
```
...
dbh_relgro_mods %>% arrange(pvalue_nog) #nothing significant
...
dbh_relgrowth_mods_transfer %>% arrange(pvalue_nog) #nothing significant without Gila
...
nursery_climvars <- c(colnames(big_pca_data),"PC1","PC2")
...
nursery_lms %>% arrange(pvalue_nog)  #smrpb significant with and without Gila NF
...
nursery_lms_quad %>% arrange(pvalue_nog2) #nothing significant
...
ht_mods %>% arrange(pvalue_nog) ##nothing significant
...
ht_mods_transfer %>% arrange(pvalue_nog) #nothing significant
...
htgrowth_mods_transfer %>% arrange(pvalue_nog) #nothing significant
...
#####PC2
summary(lmer(relht ~ value + (1|Provenance)+ (1|BLK) + (1|year), data=ht_growth_rel_clim_long %>% filter(name=="PC2")))
summary(lmer(relht ~ value + (1|Provenance)+ (1|BLK) + (1|year), data=ht_growth_rel_clim_long %>% filter(name=="PC2") %>% filter(!Provenance=="Gila NF")))
r2(lmer(relht ~ value + (1|Provenance)+ (1|BLK) + (1|year), data=ht_growth_rel_clim_long %>% filter(name=="PC2")))
r2(lmer(relht ~ value + (1|Provenance)+ (1|BLK) + (1|year), data=ht_growth_rel_clim_long %>% filter(name=="PC2") %>% filter(!Provenance=="Gila NF") %>% filter(!Provenance=="Wenatchee NF")))

####make plot 
PC2_relht_mod_lm <- lm(relht ~  value, data=ht_growth_rel_clim_long %>% filter(name=="PC2"))

newdata_PC2_relht <-  expand.grid(value=seq(min(ht_growth_rel_clim$PC2), max(ht_growth_rel_clim$PC2), length.out=50))
PC2_relht_predict <- predict(PC2_relht_mod_lm, newdata = newdata_PC2_relht, interval="confidence")

predictdata_PC2_relht <- cbind(newdata_PC2_relht, PC2_relht_predict) %>% mutate(var="PC2")

#no gila
PC2_relht_mod_lm_nog <- lm(relht ~  value, data=ht_growth_rel_clim_long_nog %>% filter(name=="PC2") %>% filter(!Provenance=="Gila NF"))

newdata_PC2_relht_nog <-  expand.grid(value=seq(min(ht_growth_rel_clim_noG$PC2), max(ht_growth_rel_clim_noG$PC2), length.out=50))
PC2_relht_predict_nog <- predict(PC2_relht_mod_lm_nog, newdata = newdata_PC2_relht_nog, interval="confidence")

predictdata_PC2_relht_nog <- cbind(newdata_PC2_relht_nog, PC2_relht_predict_nog) %>% mutate(var="PC2")
###

#no wenatchee - still significant
PC2_relht_mod_lm_now <- lm(relht ~  value, data=ht_growth_rel_clim_long %>% filter(name=="PC2") %>% filter(!Provenance=="Wenatchee NF"))
summary(PC2_relht_mod_lm_now)
###

predictdata_PC2_relht_all <- predictdata_PC2_relht %>% 
  mutate(model="with Gila NF") %>% 
  bind_rows(predictdata_PC2_relht_nog %>% mutate(model="without Gila NF"))

PC2_relht_plot <- ggplot()+
  geom_vline(xintercept = prov_clim_adjusted_bi_long %>% filter(Provenance=="Plantation") %>% filter(name=="PC2") %>% pull(value), col="darkorange2")+
  geom_text(aes(x=prov_clim_adjusted_bi_long %>% filter(Provenance=="Plantation") %>% filter(name=="PC2") %>% pull(value) + 12, y=12.5, label="plantation"), col="darkorange2")+
  geom_ribbon(data=predictdata_PC2_relht_all, aes(x=value, ymin=lwr, ymax=upr, fill=model), alpha=0.2)+
  geom_line(data=predictdata_PC2_relht_all, aes(x=value,y=fit, linetype=model))+
  geom_rug(data=ht_growth_rel_clim %>% dplyr::select(c("PC2","Provenance"))%>% mutate(fit=10), aes(x=PC2,y=fit, col=factor(Provenance, levels=provs_bylat$Provenance)), linewidth=1, sides="b")+
...
r2(lmer(relht ~ value + I(value^2) + (1|Provenance) + (1|BLK) + (1|year), data=ht_growth_rel_clim_long %>% filter(name=="MCMT")))
r2(lmer(relht ~ value + I(value^2) + (1|Provenance) + (1|BLK) + (1|year), data=ht_growth_rel_clim_long_nog %>% filter(name=="MCMT") %>% filter(!Provenance=="Gila NF")))
...
relht_quad_mod_plot<- ggplot()+
  ...
  ylab("Relative Height Growth Rate (%/year)")+
  facet_wrap(~factor(var, levels=c("MCMT","PPTsp")), nrow=1, scales="free_x")+
  theme_bw()+
  theme(legend.position="right")
relht_quad_mod_plot
...
ggplot(relht_allmods %>% filter(!var %in% c("x","y")) %>% filter(model=="without Gila NF"), aes(y = factor(var), x=estimate, color=model))+
  ylab("")+
