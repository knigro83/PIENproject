...
ht_growth_clim_long <- ht_growth_clim %>%
  mutate(year = substr(name,7,8)) %>% 
  dplyr::select(c(LOC,BLK,REP,Provenance,year,htgrowth,all_of(transfer_vars))) %>% 
  pivot_longer(CMD:pcadist2d) 

ht_growth_clim_long_transfer <- ht_growth_clim_long %>%
  left_join(plantation_climNA, by=c("name"="var")) %>% 
  mutate(transfer = value - plant_value) %>% 
...
for(i in 1:length(transfer_vars)){
mod <- lmer(htgrowth ~ transfer + (1|Provenance)+ (1|BLK) + (1|year), data= ht_growth_clim_long_transfer %>% filter(name==transfer_vars[i]))
pvalue<- as.data.frame(summary(mod)$coefficients)[2,]$`Pr(>|t|)`
Mr2 <- r2(mod)$R2_marginal

mod_nog <- lmer(htgrowth ~ transfer + (1|Provenance)+ (1|BLK) + (1|year), data=ht_growth_clim_long_transfer_nog %>% filter(name==transfer_vars[i]))
pvalue_nog<- as.data.frame(summary(mod_nog)$coefficients)[2,]$`Pr(>|t|)`
Mr2_nog <- r2(mod_nog)$R2_marginal
...
for(i in 1:length(transfer_vars)){
mod <- lmer(htgrowth ~ transfer + I(transfer^2) + (1|Provenance) + (1|BLK) + (1|year), data=ht_growth_clim_long_transfer %>% filter(name==transfer_vars[i]))
pvalues<- as.data.frame(summary(mod)$coefficients)[2:3,]$`Pr(>|t|)`
Cr2 <- performance::r2(mod)$R2_conditional
Mr2 <- performance::r2(mod)$R2_marginal
 
mod_nog <- lmer(htgrowth ~ transfer + I(transfer^2) + (1|Provenance) + (1|BLK) + (1|year), data=ht_growth_clim_long_transfer_nog %>% filter(name==transfer_vars[i])) 
pvalues_nog<- as.data.frame(summary(mod_nog)$coefficients)[2:3,]$`Pr(>|t|)`
Cr2_nog <- performance::r2(mod_nog)$R2_conditional
Mr2_nog <- performance::r2(mod_nog)$R2_marginal
...
pcadist2d_relht_plot <- ggplot()+
  ...
  geom_rug(data=ht_growth_rel_clim_long_transfer_nog %>% filter(name=="pcadist2d") %>% mutate(fit=12) %>% filter(!Provenance=="Gila NF"), aes(x=transfer,y=fit, col=factor(Provenance, levels=provs_bylat$Provenance)), linewidth=1, sides="b")+
  scale_color_manual(values=unname(stepped())[-1], name="Provenance")+
  xlab("pcadist2d Transfer")+
  ylab("Relative Height Growth Rate (%/year)")+
  facet_grid(~var)+
  theme_bw()+
  theme(legend.position="right")
...
#NFFD
surv_NFFD_mod2014 <- glmmTMB(cbind(alive,died) ~ transfer + (1|Provenance)+ (1|BLK), data=survival14_transfer %>% filter(name=="NFFD"), family="binomial")
summary(surv_NFFD_mod2014)

surv_NFFD_data2014 <- survival14_transfer %>% filter(name=="NFFD")

newdata_NFFD_surv2014 <- data.frame(
    transfer = seq(min(surv_NFFD_data2014$transfer), 
                      max(surv_NFFD_data2014$transfer), 
                      length.out = 100
                      ))
...
smrpb_surv14_plot<- ggplot(surv2014_smrpb_nog_newdata3, aes(x = transfer, y = PredictedProb)) + geom_ribbon(aes(ymin = LL,
  ...
  geom_line(linewidth = 1)+
    geom_vline(xintercept = 0, color="black", linetype=2)+
  ylim(c(0.5,1))+
  xlab("Summer:Spring Precip transfer distance")+
  ylab("Predicted Probability")+
  theme_bw()
#########
...
cmd2_surv14_plot<- ggplot(surv2014_CMD_newdata3_nog, aes(x = transfer, y = PredictedProb)) + geom_ribbon(aes(ymin = LL,
  ...
  geom_vline(xintercept = 0, color="black", linetype=2)+
    ylim(c(0.5,1))+
  xlab("CMD transfer distance")+
    ylab("Predicted Probability")+
  theme_bw()
#########

ggarrange(cmd2_surv14_plot,smrpb_surv14_plot,nffd_surv14_plot, ncol=3)
```
