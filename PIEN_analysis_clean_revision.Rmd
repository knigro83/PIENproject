...
#NFFD
surv_NFFD_mod <- glmmTMB(cbind(alive,died) ~ scale(climvar.value) + (1|Provenance)+ (1|BLK) + (1|year), data=moniger_survival_long %>% filter(climvar.name=="NFFD"), family="binomial")
...
##predictions
survNFFD_newdata1 <-  expand.grid(climvar.value = seq(min(newdata_NFFD_surv$climvar.value),max(newdata_NFFD_surv$climvar.value),length.out=50))

surv_NFFD_mod_glm <- glmmTMB(cbind(alive,died) ~ scale(climvar.value), data=moniger_survival_long %>% filter(climvar.name=="NFFD"), family="binomial")
...
ggplot(survNFFD_newdata3, aes(x = climvar.value, y = PredictedProb)) + geom_ribbon(aes(ymin = LL,
    ymax = UL), alpha = 0.2) + 
  geom_line(linewidth = 1)+
  xlab("NFFD")

#without Gila is still significant
surv_NFFD_mod_nog <- glmmTMB(cbind(alive,died) ~ scale(climvar.value) + (1|Provenance)+(1|BLK) + (1|year), data=moniger_survival_long %>% filter(climvar.name=="NFFD") %>% filter(!Provenance=="Gila NF"), family="binomial")
summary(surv_NFFD_mod_nog)

surv_NFFD_mod_nog_glm <- glmmTMB(cbind(alive,died) ~ scale(climvar.value), data=moniger_survival_long %>% filter(climvar.name=="NFFD") %>% filter(!Provenance=="Gila NF"), family="binomial")
summary(surv_NFFD_mod_nog)

##predictions
surv_NFFD_data_nog <- moniger_survival_long %>% filter(climvar.name=="NFFD") %>% filter(!Provenance=="Gila NF")

survNFFD_nog_newdata1 <- expand.grid(climvar.value = seq(min(surv_NFFD_data_nog$climvar.value),max(surv_NFFD_data_nog$climvar.value),length.out=50))
...
survNFFD_bothmod_data<- bind_rows(survNFFD_newdata3 %>% mutate(model="with Gila NF"),
survNFFD_nog_newdata3 %>% mutate(model="without Gila NF"))

ggplot(survNFFD_bothmod_data, aes(x = climvar.value, y = PredictedProb, linetype=model)) + geom_ribbon(aes(ymin = LL,
    ymax = UL), alpha = 0.2) + 
  geom_line(linewidth = 1)+
  xlab("NFFD")
#########
...
surv_mods_transfer %>% arrange(p.value_nog) #MWMT, NFFD, RH,  CMD
surv_mods_transfer %>% filter(p.value_nog<0.05) %>% arrange(Mr2_nog) 

clim_pien_cor[c("MWMT", "NFFD",  "RH",  "CMD"),c("MWMT", "NFFD",  "RH",  "CMD")]
...
#NFFD 
surv_NFFD_mod_nog <- glmmTMB(cbind(alive,died) ~ diff_plant + (1|year) + (1|Provenance)+(1|BLK), data=surv_clim_long_transfer_nog %>% filter(climvar.name=="NFFD"), family=binomial)
summary(surv_NFFD_mod_nog) #provenances with NFFD more similar to the plantation have higher probability of survival
r2(surv_NFFD_mod_nog)
plot(ggpredict(surv_NFFD_mod_nog, terms = "diff_plant"))
