...
tree_clim_cors %>% 
  ...
  group_by(Provenance) %>% 
  summarise(meancor=mean(cor,na.rm=TRUE), sd=sd(cor, na.rm=TRUE)) %>% 
...
##linear ring widths
ggplot(data=rwi %>% filter(Year>1994 & Year < 2010) %>% filter(! Tree %in% off_trees) %>% filter(!Tree=="X30L4"), aes(x=Year, y=RWI, col=Tree))+
  geom_point()+
  geom_line()+
  facet_wrap(~Provenance)+
  theme(legend.position = "none")
...
TukeyHSD(spline_aov_resil)
##now Gila is not an outlier in terms of resilience. Okanogan is just significantly greater than Inlet Creek.
...
summary(rawmodlist_resistance[[2]])
...
summary(rawmodlist_resilience[[2]])
summary(rawmodlist_resilience[[3]])
summary(rawmodlist_resilience[[4]]) ##only 5 year periods is significant (p < 0.05). 3 year periods marginally significant

##can also experiment with which year this centers on & choosing even numbered length periods
```
