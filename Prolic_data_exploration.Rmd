...
for(i in 1:length(climvars)){
  clim <- growth_climate_df_ne %>% dplyr::select(climvars[i])
  gc.cor <- cor(clim, growth_climate_df_ne$rw_spline)
  row <- cbind(climvars[i], gc.cor) %>% as.data.frame()
...
gc_cor_df_ne %>% arrange(V2)
  ...
  group_by(tree) %>% 
  summarise(cor = cor(rw_spline, ppt_gs))

lm(rw_spline ~ data=growth.detrended.ne.df)
...
for(i in 1:length(climvars_raw)){
  clim <- growth_climate_df_raw %>% dplyr::select(climvars_raw[i])
  gc.cor <- cor(clim, growth_climate_df_raw$value)
  row <- cbind(climvars[i], gc.cor) %>% as.data.frame()
  gc_cor_df_raw <- bind_rows(gc_cor_df_raw, row)
}

gc_cor_df_raw %>% arrange(V2)

##why are none of these correlating very well with growth? look at what other correlations have been.
