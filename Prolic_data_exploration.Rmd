...
adjbi_prov_clim_scaled <- prov_clim_adjusted_bi_wide %>% dplyr::select(-Provenance) %>% as.data.frame() %>% scale()
adjbi_prov_clim_eucdist <- vegdist(prov_clim_adjusted_bi_wide %>% dplyr::select(-Provenance) %>% as.data.frame(), method="euclidean")


set.seed(83)
env.euc<-vegdist(adjbi_prov_clim_scaled,"euclidian")
env.clust.ward<-hclust(env.euc,"ward.D2")
plot(env.clust.ward, hang=-1)
...
pam6$silinfo$avg.width #6 groups best by this metric
...
# years_new <- as.character(seq(1981,2023,1))
...
```{r}
all_climate <- left_join(ppt_comb_plantation,tmax_comb_plantation, by=c("CN","year","month","date")) %>% 
...
hist(engelmann_clim$MAT, freq=FALSE)
quantile(engelmann_clim$MAT)
median(engelmann_clim$MAT)

prov_clim_states$MAT
...
dbh %>% 
 dplyr::select(Series,Provenance,DBH14) %>% 
  pull(DBH14) %>% 
...
diam14 <- dbh %>% 
 ...
 dplyr::select(Series, Provenance, inner_diam_mm)
...
prov_good <- prov_all %>% as.data.frame() %>% 
 dplyr::select(-all_of(off_trees %>% substr(2, nchar(off_trees))))
...
full_join( dbh_age_good %>%dplyr::select(tree) %>% unique(),
           dbh %>% 
             filter(!Series=="") %>% 
              mutate(tree=paste("X",Series,sep="")) %>% 
              filter(!tree %in% off_trees) %>% 
            dplyr::select(tree) %>% unique()) == dbh_age_good %>%dplyr::select(tree) %>% unique() 
...
rw_clim_wide_formodel<- rw_clim_wide %>%dplyr::select(colnames(rw_clim_wide)[c(2:4,6:27,32,37,38)])
...
for(i in 1:length(provs)){
  provn <- rw.detrend.df %>% 
   dplyr::select(c(year,fdsi.cor.df.pops %>% filter(Provenance == provs[i]) %>% pull(tree))) %>% 
    na.omit()
  
rownames(provn) = provn$year

provn_noyr = provn %>%dplyr::select(-year)
...
linear_calcs <- left_join(drought_rw_linear,predrought_rw_linear) %>% left_join(postdrought_rw_linear) %>% 
  ...
  left_join(dbh_age %>%dplyr::select(tree,Provenance) %>% unique(), by=c("Tree" = "tree"))
...
prop.recovered.order<- unique(ne.prop.recovered03 %>%dplyr::select(Provenance,prop.recovered, prop.nored)) %>% arrange(prop.recovered)

prop.nored.order<- unique(ne.prop.recovered03 %>%dplyr::select(Provenance,prop.recovered, prop.nored)) %>% arrange(prop.nored)
...
plantation_clim_minus1<- plantation_clim %>% mutate(year = year+1) %>% 
 dplyr::select(c(CN:year,ppt_gs:vpdmax_sp, FDSI, aet_sum, cwd_wy, spei_sum))
...
growth_climate_df_ne <- growth.detrended.ne.df %>% 
  left_join(plantation_clim) %>% 
 dplyr::select(c(CN:year,ppt_gs:ppt_sp, vpdmax_sm:vpdmax_sp, FDSI, aet_sum, cwd_wy, spei_sum)) %>% 
  left_join(plantation_clim_minus1, by=c("year" = "prior_year", "CN" = "prior_CN")) %>% 
  ungroup() %>% 
  filter(year>1970)
...
for(i in 1:length(climvars)){
  ...
  tree <- growth_climate_df_ne %>% dplyr::filter(tree == tree_ids_ne[j]) %>%dplyr::select(rw_spline)
...
gc_cor_df_ne %>% 
  ...
  left_join(gc_cor_df_ne, by=c("maxcor"="rw_spline", "V2")) %>% 
  group_by(V1) %>% 
  summarise(n=n()) %>% print(n=50)
...
fdsi_cor_ne_provs<- gc_cor_df_ne %>% 
  filter(V1 == "FDSI") %>% 
  left_join(dbh_age_good %>%dplyr::select(tree, Provenance) %>% unique(), by=c("V2"="tree")) %>% 
...
for(i in 1:length(climvars_raw)){
  ...
  tree <- growth_climate_df_raw %>% dplyr::filter(name == tree_ids[j]) %>%dplyr::select(value)
  gc.cor <- cor(clim, tree)
...
fdsi_cor_provs<- gc_cor_df_raw %>% 
  ...
  left_join(dbh_age_good %>%dplyr::select(tree, Provenance) %>% unique()) %>% 
...
for(i in 1:length(climvars_raw)){
  ...
  tree <- growth_climate_df_raw %>% dplyr::filter(name == tree_ids[j]) %>% filter(year > 1991) %>%dplyr::select(value)
  gc.cor <- cor(clim, tree)
  row <- cbind(climvars_raw[i], tree_ids[j], gc.cor) %>% as.data.frame()
...
ggplot(data=growth_climate_df_raw %>% mutate(tree=paste("X",tree,sep="")) %>% left_join(dbh_age_good %>%dplyr::select(tree,Provenance) %>% unique(), by="tree"))+
...
testdata_big <- growth_climate_df_raw %>% 
  filter(name=="32I2") %>% 
 dplyr::select(year, value) %>% 
  filter(year > 1991) %>% 
  left_join(growth_climate_df_raw) %>% 
  mutate(age = year - 1970) 

testbigmod <- lm(value ~ ppt_wt + ppt_fa + ppt_gs + value_sm_tmax +  value_sp_tmax + value_fa_tmax  + age, data=testdata_big)

testdata_big %>% 
dplyr::select(c(ppt_wt , ppt_fa , ppt_gs , ppt_sp , value_sm_tmin , value_sm_tmax , value_sp_tmin , value_sp_tmax , value_wt_tmin , value_wt_tmax , value_fa_tmin , value_fa_tmax)) %>% 
  cor() %>% 
  ggcorrplot(type="lower", insig = "blank", lab=TRUE)

summary(testbigmod)
vif(testbigmod)

testbigmod_prior <- lm(value ~ prior_ppt_wt +  prior_ppt_fa +  prior_ppt_gs +  prior_value_sm_tmax +   prior_value_sp_tmax +  prior_value_fa_tmax  + age, data=testdata_big)

summary(testbigmod_prior)
vif(testbigmod_prior)

testbigmod_both <- lm(value ~ ppt_wt + ppt_fa + ppt_gs + value_sm_tmax +  value_sp_tmax + value_fa_tmax + prior_ppt_wt +  prior_ppt_fa +  prior_ppt_gs +  prior_value_sm_tmax +   prior_value_sp_tmax +  prior_value_fa_tmax  + age, data=testdata_big)

summary(testbigmod_both)
vif(testbigmod_both)

testdata_big %>% 
dplyr::select(c(ppt_wt , ppt_fa , ppt_gs , value_sm_tmax ,  value_sp_tmax , value_fa_tmax , prior_ppt_wt ,  prior_ppt_fa ,  prior_ppt_gs ,  prior_value_sm_tmax ,   prior_value_sp_tmax ,  prior_value_fa_tmax)) %>% 
  cor() %>% 
  ggcorrplot(type="lower", insig = "blank", lab=TRUE)

##vpd model
testvpdmod <- lm(value ~ vpdmax_sm + vpdmax_fa + vpdmax_wt + age, data=testdata_big)
summary(testvpdmod)
vif(testvpdmod)

testvpdmod_both <- lm(value ~ vpdmax_sm + vpdmax_fa + vpdmax_wt  +  prior_vpdmax_sm + prior_vpdmax_fa + prior_vpdmax_wt + age, data=testdata_big)
summary(testvpdmod_both)
vif(testvpdmod_both)

testdata_big %>% 
dplyr::select(c(vpdmax_sm , vpdmax_fa , vpdmax_wt , vpdmax_sp ,  prior_vpdmax_sm , prior_vpdmax_fa , prior_vpdmax_wt , prior_vpdmax_sp )) %>% 
  cor() %>% 
  ggcorrplot(type="lower", insig = "blank", lab=TRUE)

##model with aggregate variables
testdata_big %>% 
dplyr::select(c(FDSI, aet_sum, cwd_wy, spei_sum)) %>% 
  cor() %>% 
  ggcorrplot(type="lower", insig = "blank", lab=TRUE)

testmod_agg <- lm(value ~ FDSI + aet_sum + cwd_wy + spei_sum + age, data=testdata_big)

summary(testmod_agg)

##try with detrended data
testdata_ne <- growth_climate_df_ne %>% 
  filter(tree=="X32I2") %>% 
 dplyr::select(year, rw_spline) %>% 
  left_join(growth_climate_df_ne) %>% 
  mutate(age = year - 1970) 

testmod_agg_ne <- lm(rw_spline ~ FDSI + aet_sum + cwd_wy, data=testdata_ne)
summary(testmod_agg_ne)
vif(testmod_agg_ne)

testmod_agg_ne_prior <- lm(rw_spline ~ prior_FDSI + prior_aet_sum + prior_cwd_wy, data=testdata_ne)
summary(testmod_agg_ne_prior)
vif(testmod_agg_ne_prior)

testmod_agg_ne_both <- lm(rw_spline ~ FDSI + aet_sum + cwd_wy + prior_FDSI + prior_aet_sum + prior_cwd_wy, data=testdata_ne)
summary(testmod_agg_ne_both)
vif(testmod_agg_ne_both)

##vpd
testvpdmod_ne <- lm(rw_spline ~ vpdmax_sm + vpdmax_fa + vpdmax_wt  + age, data=testdata_ne)
summary(testvpdmod_ne)
vif(testvpdmod_ne)

testvpdmod_both_ne <- lm(rw_spline ~ vpdmax_sm + vpdmax_fa + vpdmax_wt  +  prior_vpdmax_sm + prior_vpdmax_fa + prior_vpdmax_wt  + age, data=testdata_ne)
summary(testvpdmod_both_ne)
vif(testvpdmod_both_ne)

##do these detrended models for pre drought
testdata_ne_predrought<- testdata_ne %>% 
  filter(year<2002)

#agg
testmod_agg_ne_pre <- lm(rw_spline ~ FDSI + aet_sum + cwd_wy, data=testdata_ne_predrought)
summary(testmod_agg_ne_pre)
vif(testmod_agg_ne_pre)

#vpd
testvpdmod_ne_pre <- lm(rw_spline ~ vpdmax_sm + vpdmax_fa + vpdmax_wt  + age, data=testdata_ne_predrought)
summary(testvpdmod_ne_pre)
vif(testvpdmod_ne_pre)

#vpd both
testvpdmod_both_ne_pre <- lm(rw_spline ~ vpdmax_sm + vpdmax_fa + vpdmax_wt  +  prior_vpdmax_sm  + prior_vpdmax_wt + age, data=testdata_ne_predrought)
summary(testvpdmod_both_ne_pre)
vif(testvpdmod_both_ne_pre)
```


##detrending whole series

```{r}
###negative exponential detrending with dplr
prov_good_allyrs <- prov_all %>%dplyr::select(-all_of(substr(off_trees, 2,nchar(off_trees))))

growth.detrended.ne.allyrs = matrix(nrow=nrow(prov_good_allyrs), ncol=length(colnames(prov_good_allyrs)))
for(i in 1:length(prov_good_allyrs)){
obj<- detrend.series(y = prov_good_allyrs %>% pull(colnames(prov_good_allyrs)[i]), y.name="growth", method="ModNegExp")
growth.detrended.ne.allyrs[,i]<- obj
}

colnames(growth.detrended.ne.allyrs) = colnames(prov_good_allyrs)
growth.detrended.ne.allyrs.df <- growth.detrended.ne.allyrs %>% 
  as.data.frame() %>% 
  mutate(year = rownames(prov_good_allyrs)) %>% 
  pivot_longer(!year, values_to = "rw_spline", names_to="tree") %>% 
  mutate(tree=paste("X",tree,sep=""), year=as.integer(year)) %>% 
  left_join(rwi %>% dplyr::select(Provenance, Tree) %>% unique(), by=c("tree"="Tree"))


growth_climate_df_ne_allyrs <- growth.detrended.ne.allyrs.df %>% 
  left_join(plantation_clim) %>% 
  dplyr::select(c(CN:year,ppt_gs:vpdmax_sp, FDSI, aet_sum, cwd_wy, spei_sum)) %>% 
  left_join(plantation_clim_minus1, by=c("year" = "prior_year", "CN" = "prior_CN")) %>% 
  ungroup() %>% 
...
predrought_nedata_allyrs<- growth_climate_df_ne_allyrs %>% 
 dplyr::select(tree, year, rw_spline, FDSI, prior_FDSI) %>% 
  filter(year<2002 & year>1971) %>% 
  filter(!is.na(rw_spline)) %>% 
  left_join(dbh_age_good %>%dplyr::select(tree,Provenance) %>% unique(), by="tree") 


postdrought_nedata_allyrs<- growth_climate_df_ne_allyrs %>% 
 dplyr::select(tree, year, rw_spline, FDSI, prior_FDSI) %>% 
  filter(year>2001 & year < 2016) %>% 
  left_join(dbh_age_good %>%dplyr::select(tree,Provenance) %>% unique(), by="tree") %>% 
  filter(!is.na(rw_spline))

treenames<- unique(predrought_nedata_allyrs$tree)

modresultdf_ne_allyrs<- data.frame()
for(i in 1:length(treenames)){
  predata = predrought_nedata_allyrs %>% filter(tree==treenames[i])
  postdata = postdrought_nedata_allyrs %>% filter(tree==treenames[i])
  mod = lm(rw_spline ~ FDSI + prior_FDSI, data=predata)
  adjrsquared<- summary(mod)$adj.r.squared
  postclim <- growth_climate_df_ne_allyrs %>%dplyr::select(year,FDSI,prior_FDSI) %>% filter(year>2001 & year < 2016) %>% unique()
  newdata <-  predata %>% bind_cols(prediction= predict(mod)) %>% 
  bind_rows(postdata %>% left_join(data.frame(year=seq(2002,2015,1), prediction=predict(mod, postclim)))) %>% 
    mutate(adjrsquared = adjrsquared)
modresultdf_ne_allyrs<- bind_rows(modresultdf_ne_allyrs,newdata)
}

modresultdf_ne_allyrs %>% 
  filter(adjrsquared>0.2) %>% 
  pull(tree) %>% unique()

ggplot(data=modresultdf_ne_allyrs %>% filter(adjrsquared>0.2), aes(x=year, y=rw_spline))+
  geom_point()+
  geom_line(col="black")+
  geom_line(data=modresultdf_ne_allyrs %>% filter(adjrsquared>0.2), aes(x=year, y=prediction), col="red")+
  geom_point(data=modresultdf_ne_allyrs %>% filter(adjrsquared>0.2), aes(x=year, y=prediction), col="red")+
  geom_bar(stat="identity", aes(x=year, y=FDSI, fill=Provenance))+
  geom_text(data= data.frame(tree=modresultdf_ne_allyrs %>% filter(adjrsquared>0.2) %>% pull(tree) %>% unique(), year=1993,rw_spline=max(modresultdf_ne_allyrs %>% filter(adjrsquared>0.2) %>% pull(rw_spline)), label=modresultdf_ne_allyrs %>% filter(adjrsquared>0.2) %>% pull(adjrsquared) %>% unique()), aes(label=paste("adj r2 =", round(label,1))))+
  geom_vline(xintercept=2002, col="darkred")+
  facet_wrap(~tree)
...
```{r}
##try these models for all data with random nested effect of tree within provenance
gc_alltrees_ne <- growth_climate_df_ne_allyrs %>% 
  mutate(age = year - 1970) %>% 
  filter(!is.na(rw_spline))

aggmod_ne <- lmer(rw_spline ~ FDSI + aet_sum + cwd_wy + (1|Provenance) + (1|Provenance:tree), data=gc_alltrees_ne)
summary(aggmod_ne)
plot(aggmod_ne)
vif(aggmod_ne)

vpdmod_ne <- lmer(rw_spline ~ vpdmax_sm + vpdmax_fa + vpdmax_wt + (1|Provenance) + (1|Provenance:tree), data=gc_alltrees_ne)
summary(vpdmod_ne)
vif(vpdmod_ne)

##FDSI models with all years and all trees
fdsi_mod_ne <- lmer(rw_spline ~ FDSI + prior_FDSI + (1|Provenance) + (1|Provenance:tree), data=gc_alltrees_ne)
summary(fdsi_mod_ne)
fdsi_mod_ne_prov <- lmer(rw_spline ~ FDSI + prior_FDSI + (1|Provenance), data=gc_alltrees_ne)
summary(fdsi_mod_ne_prov)
fdsi_mod_ne_tree <- lmer(rw_spline ~ FDSI + prior_FDSI + (1|tree), data=gc_alltrees_ne)
summary(fdsi_mod_ne_tree) ##the fact that all the random effects produce singular models with no variance attributed to tree or provenance makes me think that the relationship between growth and FDSI does not vary significantly between provenances or between trees (i.e., they all respond similarly to FDSI and prior FDSI)

fdsi_mod_ne_lm <- lm(rw_spline ~ FDSI + prior_FDSI, data=gc_alltrees_ne)
summary(fdsi_mod_ne_lm)

##SPEI models with all years and all trees
gc_alltrees_ne %>%dplyr::select(spei_sum, prior_spei_sum) %>% cor()

spei_mod_ne <- lmer(rw_spline ~ spei_sum + prior_spei_sum + (1|Provenance) + (1|Provenance:tree), data=gc_alltrees_ne)
summary(spei_mod_ne)
spei_mod_ne_prov <- lmer(rw_spline ~ spei_sum + prior_spei_sum + (1|Provenance), data=gc_alltrees_ne)
summary(spei_mod_ne_prov)
spei_mod_ne_tree <- lmer(rw_spline ~ spei_sum + prior_spei_sum + (1|tree), data=gc_alltrees_ne)
summary(spei_mod_ne_tree) ##the fact that all the random effects produce singular models with no variance attributed to tree or provenance makes me think that the relationship between growth and spei does not vary significantly between provenances or between trees (i.e., they all respond similarly to spei and prior spei)

spei_mod_ne_lm <- lm(rw_spline ~ spei_sum*prior_spei_sum, data=gc_alltrees_ne)
summary(spei_mod_ne_lm)

ggplot(gc_alltrees_ne %>% mutate(prior_cat = case_when(
  prior_spei_sum < -1.99 ~ "extremely dry",
  prior_spei_sum < -.99 & prior_spei_sum > -2 ~ "dry",
  prior_spei_sum < 1 & prior_spei_sum > -1 ~ "normal",
  prior_spei_sum < 2 & prior_spei_sum > 0.99 ~ "wet",
  prior_spei_sum > 2 ~ "extremely wet")), 
  aes(x=spei_sum, y=rw_spline, col=prior_cat))+
  geom_point()+
  geom_smooth(method="lm")

ggplot(gc_alltrees_ne %>% filter(year<2002),aes(x=spei_sum, y=rw_spline, col=prior_spei_sum>0))+
  geom_point()+
  geom_smooth(method="lm")

##try as many clim variables as possible

##ppt & temp
TPmod_ne_both <- lmer(rw_spline ~ ppt_wt + ppt_fa + ppt_gs + value_sm_tmax +  value_sp_tmax + value_fa_tmax + prior_ppt_wt +  prior_ppt_fa +  prior_ppt_gs +  prior_value_sm_tmax +   prior_value_sp_tmax +  prior_value_fa_tmax  + age + (1|Provenance) + (1|Provenance:tree), data=gc_alltrees_ne)
summary(TPmod_ne_both) #singular so random effects not needed

TPmod_ne_both_lm <- lm(rw_spline ~ ppt_wt*prior_ppt_wt + ppt_fa*prior_ppt_fa + ppt_gs*prior_ppt_gs + value_sm_tmax*prior_value_sm_tmax +  value_sp_tmax*prior_value_sp_tmax + value_fa_tmax*prior_value_fa_tmax + age, data=gc_alltrees_ne)
summary(TPmod_ne_both_lm)

options(na.action = "na.fail")
TPmod_ne_both_lm_dredge <- dredge(TPmod_ne_both_lm)
TPmod_ne_both_lm_dredge %>% as.data.frame() %>% filter(delta<2)
...
```{r}
predrought_nedata<- growth_climate_df_ne %>% 
 dplyr::select(tree, year, rw_spline) %>% 
  filter(year > 1991) %>% 
  left_join(growth_climate_df_ne %>% 
 dplyr::select(year, FDSI) %>% unique() %>% 
  mutate(year = year+1) %>% 
...
postdrought_nedata<- growth_climate_df_ne %>% 
 dplyr::select(tree, year, rw_spline) %>% 
  filter(year > 1991) %>% 
  left_join(growth_climate_df_ne %>% 
 dplyr::select(year, FDSI) %>% unique() %>% 
...
ggplot(predrought_nedata, aes(x=FDSI_prior, y=rw_spline, col=tree))+
  geom_point()+
  geom_smooth(method="lm")

treenames<- unique(predrought_nedata$tree)

modresultdf_ne<- data.frame()
for(i in 1:length(treenames)){
  predata = predrought_nedata %>% filter(tree==treenames[i])
  postdata = postdrought_nedata %>% filter(tree==treenames[i])
  mod = lm(rw_spline ~ FDSI_prior, data=predata)
  adjrsquared<- summary(mod)$adj.r.squared
  newdata <-  predata %>% bind_cols(prediction= predict(mod)) %>% 
  bind_rows(postdata %>% left_join(data.frame(year=seq(2003,2010,1), prediction=predict(mod, postdata)))) %>% 
    mutate(adjrsquared = adjrsquared)
modresultdf_ne<- bind_rows(modresultdf_ne,newdata)
}
...
ggplot(data=modresultdf_ne %>% filter(adjrsquared>0.2), aes(x=year, y=rw_spline))+
  geom_point()+
  geom_line(col="black")+
  geom_line(data=modresultdf_ne %>% filter(adjrsquared>0.2), aes(x=year, y=prediction), col="red")+
  geom_point(data=modresultdf_ne %>% filter(adjrsquared>0.2), aes(x=year, y=prediction), col="red")+
  geom_bar(stat="identity", aes(x=year, y=FDSI_prior, fill=Provenance))+
  geom_text(data= data.frame(tree=modresultdf_ne %>% filter(adjrsquared>0.2) %>% pull(tree) %>% unique(), year=1993,rw_spline=max(modresultdf_ne %>% filter(adjrsquared>0.2) %>% pull(rw_spline)), label=modresultdf_ne %>% filter(adjrsquared>0.2) %>% pull(adjrsquared) %>% unique()), aes(label=paste("adj r2 =", round(label,1))))+
  geom_vline(xintercept=2003, col="darkred")+
  facet_wrap(~tree)
...
```{r}
climvars <- colnames(gc_alltrees_ne %>% dplyr::select(-c(CN:year,age)))
tree_ids_ne <- unique(gc_alltrees_ne$tree)

gc_cor_df_ne_allyrs<-data.frame()
for(i in 1:length(climvars)){
  for(j in 1:length(tree_ids_ne)){
  clim <- gc_alltrees_ne %>% dplyr::filter(tree == tree_ids_ne[j]) %>% dplyr::select(climvars[i])
  tree <- gc_alltrees_ne %>% dplyr::filter(tree == tree_ids_ne[j]) %>%dplyr::select(rw_spline)
  gc.cor <- cor(clim, tree)
  p.value <- cor.test(clim %>% pull(1), tree %>% pull(1))$p.value
  row <- cbind(climvars[i], tree_ids_ne[j], gc.cor, p.value) %>% as.data.frame()
  gc_cor_df_ne_allyrs <- bind_rows(gc_cor_df_ne_allyrs, row)
  }}
...
gc_cor_df_ne_allyrs %>% 
  group_by(V2) %>% 
  summarise(maxcor = as.numeric(max(rw_spline, na.rm=T))) %>% View()

gc_cor_df_ne_allyrs %>% 
  group_by(V2) %>% 
  summarise(maxcor = max(rw_spline, na.rm=T)) %>% 
  left_join(gc_cor_df_ne_allyrs, by=c("maxcor"="rw_spline", "V2")) %>% 
  group_by(V1) %>% 
  summarise(n=n()) %>% print(n=50)

cor_clean <- gc_cor_df_ne_allyrs %>% 
  rename(climvar=V1, tree=V2, cor=rw_spline) %>%
  mutate(cor = as.numeric(cor)) %>% 
  left_join(gc_alltrees_ne %>%dplyr::select(tree,Provenance) %>% unique(), by="tree") %>% 
  filter(!is.na(cor))
  
cor_summary<- gc_cor_df_ne_allyrs %>% 
  rename(climvar=V1, tree=V2, cor=rw_spline) %>%
  mutate(cor = as.numeric(cor)) %>% 
  left_join(gc_alltrees_ne %>%dplyr::select(tree,Provenance) %>% unique(), by="tree") %>% 
  filter(!is.na(cor)) %>% 
  group_by(Provenance, climvar) %>% 
  summarise(meancor = mean(cor), se=sd(cor)/sqrt(n())) %>% 
  ungroup()

ggplot(cor_summary %>% filter(abs(meancor)>0.2), aes(x=climvar, y=meancor, fill=climvar))+
  geom_bar(stat="identity")+
...
cor_summary %>% 
  group_by(Provenance) %>% 
  summarise(maxmeancor = max(meancor)) %>% 
  arrange(maxmeancor)

ggplot(cor_clean %>% inner_join(cor_summary %>% filter(abs(meancor)>0.2)), aes(x=climvar, y=cor, fill=climvar))+
  geom_boxplot()+
  geom_point()+
  facet_grid(~Provenance)+
  coord_flip()+
...
allanovas <- bind_rows(allanovas, data.frame(climvar=climvars[i],pvalue=pvalue))
}

allanovas %>% filter(pvalue<0.05)

ggplot(cor_clean %>% filter(climvar %in% c(allanovas %>% filter(pvalue<0.05) %>% pull(climvar))), aes(x=Provenance, y=cor))+
  geom_boxplot()+
  facet_wrap(~climvar)+
  theme(axis.text.x = element_text(angle=45, hjust=1))

##ANOVA aet_sum
anova_aetsum <- aov(cor ~ Provenance, data=cor_clean %>% filter(climvar == "aet_sum"))
as.data.frame(TukeyHSD(anova_aetsum)$Provenance) %>% filter(`p adj` < 0.05)

##ANOVA ppt
anova_ppt <- aov(cor ~ Provenance, data=cor_clean %>% filter(climvar == "ppt"))
as.data.frame(TukeyHSD(anova_ppt)$Provenance) %>% filter(`p adj` < 0.05)

##ANOVA ppt_wt
anova_ppt_wt <- aov(cor ~ Provenance, data=cor_clean %>% filter(climvar == "ppt_wt"))
summary(anova_ppt_wt)
as.data.frame(TukeyHSD(anova_ppt_wt)$Provenance) %>% filter(`p adj` < 0.1)

##ANOVA prior_ppt
anova_prior_ppt <- aov(cor ~ Provenance, data=cor_clean %>% filter(climvar == "prior_ppt"))
as.data.frame(TukeyHSD(anova_prior_ppt)$Provenance) %>% filter(`p adj` < 0.1)

##ANOVA sp_tmax
anova_sp_tmax <- aov(cor ~ Provenance, data=cor_clean %>% filter(climvar == "value_sp_tmax"))
as.data.frame(TukeyHSD(anova_sp_tmax)$Provenance) %>% filter(`p adj` < 0.05)

######testing relationships between growth and climate across all trees with provenance as a random effect

all_lms<- data.frame()
for(i in 1:length(climvars)){
mod <- lmer(gc_alltrees_ne %>% pull(rw_spline) ~ gc_alltrees_ne %>% pull(climvars[i]) + (1|Provenance), data=gc_alltrees_ne)
...
cor_provclim<- cor_clean %>% left_join(
prov_clim_adjusted_bi %>% 
  mutate(Provenance = ifelse(Provenance=="Grand Mesa-Uncompahgre NF","GM-U NF",Provenance)) %>% 
  dplyr::select(c(Provenance,MAT:pas,smrpb)), by="Provenance") %>% 
  left_join(data.frame(cluster=pam7$clustering) %>% mutate(Provenance=rownames(data.frame(cluster=pam7$clustering))) %>% mutate(Provenance = ifelse(Provenance=="Grand Mesa-Uncompahgre NF","GM-U NF",Provenance))) 

provmatmod<- lmer(cor ~ MAT + (1|Provenance), data=cor_provclim %>% filter(climvar=="prior_FDSI"))
summary(provmatmod)

clustermod <- aov(cor ~ factor(cluster), data=cor_provclim %>% filter(climvar=="prior_FDSI"))
summary(clustermod)
TukeyHSD(clustermod)

ggplot(cor_provclim %>% filter(climvar=="prior_FDSI"), aes(x=factor(cluster), y=cor))+
  geom_boxplot()

cor_provclim_long <- pivot_longer(cor_provclim, MAT:smrpb)
...
#FDSI x cmd
ggplot(data=cor_provclim_long %>% filter(climvar=="FDSI"& name=="cmd"), aes(x=value, y=cor, col=Provenance))+
  geom_point()+
  ggtitle("FDSI")
...
#cwd_wy x NFFD
ggplot(cor_provclim_long %>% filter(climvar=="cwd_wy"& name=="nffd"), aes(x=value, y=cor, col=Provenance))+
  geom_point()+
  geom_smooth(method="lm")+
  ggtitle("cwd_wy")

cwd_nffd<- cor_provclim_long %>% filter(climvar=="cwd_wy" & name=="nffd") 

cwd_nffd_mod <- lmer(cor ~ value + (1|Provenance), data=cwd_nffd)
summary(cwd_nffd_mod) #is significant if Gila is excluded

# aet x td
ggplot(data=cor_provclim_long %>% filter(climvar=="aet_sum"& name=="TD"), aes(x=value, y=cor, col=Provenance))+
  geom_point()+
  ggtitle("TD x aet_sum cor")

aet_td<- cor_provclim_long %>% filter(climvar=="aet_sum" & name=="TD") 

aet_td_mod <- lmer(cor ~ value + (1|Provenance), data=aet_td)
summary(aet_td_mod)

#TD x spei
ggplot(data=cor_provclim_long %>% filter(climvar=="spei_sum"& name=="TD"), aes(x=value, y=cor, col=Provenance))+
  geom_point()+
  ggtitle("TD x spei_sum cor")

spei_td<- cor_provclim_long %>% filter(climvar=="spei_sum" & name=="TD") 

spei_td_mod <- lmer(cor ~ value + (1|Provenance), data=spei_td)
summary(aet_td_mod)

#prior ppt x cmd
ggplot(data=cor_provclim_long %>% filter(climvar=="prior_ppt"& name=="cmd"), aes(x=value, y=cor, col=Provenance))+
  geom_point()+
  ggtitle("cmd x prior_ppt cor")

prippt_cmd<- cor_provclim_long %>% filter(climvar=="prior_ppt" & name=="cmd") 

prippt_cmd_mod <- lmer(cor ~ value + (1|Provenance), data=prippt_cmd)
summary(prippt_cmd_mod)

#prior fa tmin x bFFP
ggplot(data=cor_provclim_long %>% filter(climvar=="prior_value_fa_tmin"& name=="bFFP"), aes(x=value, y=cor, col=Provenance))+
  geom_point()+
  ggtitle("bFFP x prior_value_fa_tmin cor")
...
#nffd x tmin
ggplot(data=cor_provclim_long %>% filter(climvar=="prior_value_fa_tmin"& name=="nffd"), aes(x=value, y=cor, col=Provenance))+
  geom_point()+
  ggtitle("nffd x prior_value_fa_tmin cor")

prifatmin_nffd<- cor_provclim_long %>% filter(climvar=="prior_value_fa_tmin" & name=="nffd") 

prifatmin_nffd_mod <- lmer(cor ~ value + (1|Provenance), data=prifatmin_nffd)
summary(prifatmin_nffd_mod)

#EMT x tmin
ggplot(data=cor_provclim_long %>% filter(climvar=="prior_value_fa_tmin"& name=="EMT"), aes(x=value, y=cor, col=Provenance))+
  geom_point()+
  ggtitle("EMT x prior_value_fa_tmin cor")
...
summary(prifatmin_EMT_mod) ##places with lower extreme minimum temperatures respond negatively to prior fall being hotter. 

#prior sp tmean x msp
ggplot(data=cor_provclim_long %>% filter(climvar=="prior_value_sp_tmean"& name=="msp"), aes(x=value, y=cor, col=Provenance))+
  geom_point()+
  ggtitle("msp x prior_value_sp_tmean cor")

prisptmean_msp<- cor_provclim_long %>% filter(climvar=="prior_value_sp_tmean" & name=="msp") 

prisptmean_msp_mod <- lmer(cor ~ value + (1|Provenance), data=prisptmean_msp)
summary(prisptmean_msp_mod) #significant

#prior wt vpdmax x msp
ggplot(data=cor_provclim_long %>% filter(climvar=="prior_vpdmax_wt"& name=="msp"), aes(x=value, y=cor, col=Provenance))+
  geom_point()+
  ggtitle("msp x prior_vpdmax_wt cor")

priwtvpd_msp<- cor_provclim_long %>% filter(climvar=="prior_vpdmax_wt" & name=="msp") 

priwtvpd_msp_mod <- lmer(cor ~ value + (1|Provenance), data=priwtvpd_msp)
summary(priwtvpd_msp_mod) #significant
...
sig.cors20<- gc_cor_df_ne_allyrs %>% 
  filter(p.value<0.05) %>% 
  group_by(V1) %>% 
  summarise(n=n()) %>% 
  filter(n>20) %>% 
...
cwd_pc1_mod <- lmer(cor~PC1 + (1|Provenance), data=pca_cor_data %>% filter(climvar=="cwd_wy"))
summary(cwd_pc1_mod)

fdsi_pc2_mod<- lmer(cor~PC2+(1|Provenance), data=pca_cor_data %>% filter(climvar=="FDSI"))
summary(fdsi_pc2_mod)
...
pcadist_mod <- lmer(cor ~ pcadist + (1|Provenance), data=pca_cor_data %>% filter(climvar%in%sig.cors20$V1) %>% filter(!Provenance=="Gila NF") %>% filter(climvar=="FDSI"))
summary(pcadist_mod)

pcadist_mod_cwd <- lmer(cor ~ pcadist + (1|Provenance), data=pca_cor_data %>% filter(climvar%in%sig.cors20$V1) %>% filter(!Provenance=="Gila NF") %>% filter(climvar=="cwd_wy"))
summary(pcadist_mod_cwd)

pcadist_mod_vpd <- lmer(cor ~ pcadist + (1|Provenance), data=pca_cor_data %>% filter(climvar%in%sig.cors20$V1) %>% filter(!Provenance=="Gila NF") %>% filter(climvar=="vpdmax_sm"))
