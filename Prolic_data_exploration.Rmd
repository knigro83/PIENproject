...
adjbi_prov_clim_scaled <- prov_clim_adjusted_bi_wide %>% select(-Provenance) %>% as.data.frame() %>% scale()
...
plantation_clim_minus1<- plantation_clim %>% mutate(year = year+1) %>% 
  select(c(CN:year,ppt_gs:vpdmax_sp, FDSI, aet_sum, cwd_wy, spei_sum))
...
testdata_big %>% 
select(c(ppt_wt , ppt_fa , ppt_gs , ppt_sp , value_sm_tmin , value_sm_tmax , value_sp_tmin , value_sp_tmax , value_wt_tmin , value_wt_tmax , value_fa_tmin , value_fa_tmax)) %>% 
  cor() %>% 
...
testdata_big %>% 
select(c(ppt_wt , ppt_fa , ppt_gs , value_sm_tmax ,  value_sp_tmax , value_fa_tmax , prior_ppt_wt ,  prior_ppt_fa ,  prior_ppt_gs ,  prior_value_sm_tmax ,   prior_value_sp_tmax ,  prior_value_fa_tmax)) %>% 
  cor() %>% 
  ggcorrplot(type="lower", insig = "blank", lab=TRUE)
...
testdata_big %>% 
select(c(vpdmax_sm , vpdmax_fa , vpdmax_wt , vpdmax_sp ,  prior_vpdmax_sm , prior_vpdmax_fa , prior_vpdmax_wt , prior_vpdmax_sp )) %>% 
  cor() %>% 
  ggcorrplot(type="lower", insig = "blank", lab=TRUE)

##model with aggregate variables
testdata_big %>% 
select(c(FDSI, aet_sum, cwd_wy, spei_sum)) %>% 
  cor() %>% 
  ggcorrplot(type="lower", insig = "blank", lab=TRUE)
...
gc_alltrees_ne %>% select(spei_sum, prior_spei_sum) %>% cor()
...
```{r}
###negative exponential detrending with dplr
prov_good_allyrs <- prov_all %>% select(-all_of(substr(off_trees, 2,nchar(off_trees))))
...
predrought_nedata_allyrs<- growth_climate_df_ne_allyrs %>% 
  select(tree, year, rw_spline, FDSI, prior_FDSI) %>% 
...
for(i in 1:length(treenames)){
  ...
  adjrsquared<- summary(mod)$adj.r.squared
  postclim <- growth_climate_df_ne_allyrs %>% select(year,FDSI,prior_FDSI) %>% filter(year>2001 & year < 2016) %>% unique()
  newdata <-  predata %>% bind_cols(prediction= predict(mod)) %>% 
...
for(i in 1:length(climvars)){
  ...
  tree <- gc_alltrees_ne %>% dplyr::filter(tree == tree_ids_ne[j]) %>% select(rw_spline)
  gc.cor <- cor(clim, tree)
  row <- cbind(climvars[i], tree_ids_ne[j], gc.cor) %>% as.data.frame()
...
cor_clean <- gc_cor_df_ne_allyrs %>% 
  ...
  left_join(gc_alltrees_ne %>% select(tree,Provenance) %>% unique(), by="tree") %>% 
  filter(!is.na(cor))
